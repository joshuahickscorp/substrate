...
def collapse_proof_paths() -> set[str]:
    out = subprocess.run(["git", "ls-tree", "-r", "--name-only", COLLAPSE_TAG, "--", "proof"],
...
def build():
    ...
    for p in sorted((ROOT / "proof").rglob("*")):
        ...
        if p.suffix == ".json":
            try:
                ...
                if isinstance(doc, dict):
                    meta = {k: doc.get(k) for k in ("schema", "classification", "verdict", "program", "lane", "domain")
        ...
        artifacts.append({
            "logical_id": rel, "original_path": rel, "canonical_path": f"evidence_store/{ch}",
            "content_hash": ch, "bytes": len(payload), "set": classify(rel, collapse),
            "schema": meta.get("schema"), "program": meta.get("program") or meta.get("lane"),
            "evidence_class": cls,
    ...
    for name in ("collapse", "substrate", "campaign", "historical"):
        ...
        sets[name] = {"count": len(rows), "bytes": sum(r["bytes"] for r in rows),
    ...
    index = {
        "schema": "mop-evidence-fabric/v1", "pack": "integrated-v1",
        "artifacts": artifacts, "sets": {k: {i: v[i] for i in ("count", "bytes", "merkle_root")} for k, v in sets.items()},
        "union": {"count": len(artifacts), "unique_objects": len(uniq),
                  "unique_bytes": sum(uniq.values()), "duplicate_bytes_eliminated": dup_bytes,
                  "merkle_root": union_root},
        "by_claim": {}, "by_experiment": {}, "by_null": [a["logical_id"] for a in artifacts if a["is_null"]],
    }
    for a in artifacts:
        if a["evidence_class"]:
...
def verify(index) -> dict:
    ...
    checks["substrate_or_campaign_present"] = (index["sets"]["substrate"]["count"] +
                                               index["sets"]["campaign"]["count"]) > 0
...
if __name__ == "__main__":
    ...
    mut = mutations(index)
    for name, obj in [
        ("MOP_PROOF_SCOPE_AUTHORITY.json", {"schema": "mop-proof-scope-authority/v1",
        ...
        ("MOP_INTEGRATED_EVIDENCE_ACCOUNTING.json", {"schema": "mop-integrated-evidence-accounting/v1",
    ...
    print("artifacts", index["union"]["count"], "unique", index["union"]["unique_objects"],
          "dup_bytes", index["union"]["duplicate_bytes_eliminated"])
