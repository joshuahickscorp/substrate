...
class GRU(nn.Module):
    def __init__(s, ch, no):
        super().__init__(); s.g = nn.GRU(ch, HIDDEN, batch_first=True); s.h = nn.Linear(HIDDEN, no)
...
class Bag(nn.Module):
    def __init__(s, ch, no):
        super().__init__(); s.n = nn.Sequential(nn.Linear(ch * 3, HIDDEN), nn.ReLU(), nn.Linear(HIDDEN, HIDDEN), nn.ReLU(), nn.Linear(HIDDEN, no))
...
def train_eval(Model, tr, te, X, Y, seed, transform=None):
    torch.manual_seed(seed); m = Model(X.shape[2], len(WORDS))
    opt = torch.optim.Adam(m.parameters(), 1e-3); rng = np.random.default_rng(seed)
    ...
    m.eval(); xe = X[te] if transform is None else transform(X[te], rng)
    with torch.no_grad():
...
def main():
    t0 = time.time(); X, Y, S = load()
    spk = np.unique(S); rng0 = np.random.default_rng(0)
    tr_spk = set(rng0.choice(spk, int(0.7 * len(spk)), replace=False).tolist())
    tr = np.where(np.isin(S, list(tr_spk)))[0]; te = np.where(~np.isin(S, list(tr_spk)))[0]
    ...
    out = {"schema": "mop-substrate-audio-preflight/v2-melbank", "domain": "SpeechCommands_v0.02",
           ...
           "rights": "Speech Commands v0.02, public (CC BY 4.0), Google", "source_commit": commit,
           "units": "speaker-disjoint (speaker hash in filename)", "n_clips": int(len(X)),
           "frames": int(X.shape[1]), "bins": int(X.shape[2]), "classes": len(WORDS),
           "n_speakers": int(len(spk)), "seeds": SEEDS,
           "gru_correct": round(float(gc.mean()), 4), "gru_shuffled": round(float(sh.mean()), 4),
           "bag_order_free": round(float(bg.mean()), 4),
           "temporal_headroom_gru_vs_bag": round(th, 4), "temporal_headroom_lcb": round(th_lcb, 4),
           "order_matters_gru_vs_shuffled": round(om, 4), "verdict": verdict,
    ...
    print(f"[audio] {verdict} | gru {gc.mean():.3f} bag {bg.mean():.3f} shuffled {sh.mean():.3f} "
