...
class Proj(nn.Module):
    def __init__(s, ch):
        super().__init__(); s.c = nn.Conv1d(ch, 32, 5, padding=2); s.l = nn.Linear(32, LATENT)
...
class SharedCore(nn.Module):
    ...
    def __init__(s):
        super().__init__(); s.core = nn.GRU(LATENT, LATENT, batch_first=True)

    def forward(s, z):
        o, _ = s.core(z); return o[:, -1]
...
def har_data(seed):
    ...
    X = torch.cat([t["x"] for t in tasks]); Y = torch.cat([t["y"] for t in tasks])
    te = [t["test"] for t in tasks]
    Xte = torch.cat([t[0] for t in te]); Yte = torch.cat([t[1] for t in te])
    return X, Y, Xte, Yte, ch, nout


def fit(core, proj, head, X, Y, seed, steps=STEPS, train_core=True):
    params = list(proj.parameters()) + list(head.parameters()) + (list(core.parameters()) if train_core else [])
    opt = torch.optim.Adam(params, 1e-3); rng = np.random.default_rng(seed)
    proj.train(); head.train(); core.train()
    for _ in range(steps):
        bi = rng.choice(len(X), min(64, len(X)), replace=False)
        logits = head(core(proj(X[bi])))
        opt.zero_grad(); F.cross_entropy(logits, Y[bi]).backward(); opt.step()


@torch.no_grad()
def acc(core, proj, head, X, Y):
    core.eval(); proj.eval(); head.eval()
...
def run(arm, seed):
    ...
    spk = np.unique(sS); rng = np.random.default_rng(seed)
    held = set(rng.choice(spk, max(1, len(spk) // 5), replace=False).tolist())
    tr = np.where(~np.isin(sS, list(held)))[0]; ho = np.where(np.isin(sS, list(held)))[0]
    sno = int(sY.max()) + 1
    ...
    return {"har_after_stage1": har_after_1, "speech_acquisition": speech_acq,
            "har_return_recovery": har_return, "future_adaptation": future_adapt,
            "negative_transfer": har_after_1 - har_return}


ARMS = ["fresh_core", "projection_only", "slow_core_transfer", "frozen_transferred_core",
        "finetuned_transferred_core", "full_persistent_substrate"]
...
def main():
    ...
    for a in ARMS:
        ...
        r = [per[a][i]["har_return_recovery"] - per[base][i]["har_return_recovery"] for i in range(len(SEEDS))]
        eff[a] = {"speech_acq_vs_fresh": {"mean": round(float(np.mean(e)), 4), "lower_95_cb": round(lcb(e), 4)},
                  "har_return_vs_fresh": {"mean": round(float(np.mean(r)), 4), "lower_95_cb": round(lcb(r), 4)}}
    pos = [a for a, v in eff.items()
    ...
    doc = {"schema": "mop-integrated-cross-domain/v1", "source_commit": commit,
           "sequence": "HAR sensor -> Speech Commands audio -> return HAR -> held-out speaker adaptation",
           "domains_valid": {"HAR_raw": "temporal_headroom_present", "SpeechCommands": "temporal_headroom_present (mel filterbank)"},
           "seeds": SEEDS, "arms": ARMS, "means": means, "effects_vs_fresh_core": eff,
           "positive_arms": pos, "verdict": verdict, "SESOI": 0.05,
