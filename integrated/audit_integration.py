...
def files(ref):
    out = subprocess.run(["git", "ls-tree", "-r", "--name-only", ref], cwd=ROOT, capture_output=True, text=True)
...
def main():
    ...
    for p in src:
        text = p.read_text()
        for m in re.finditer(r"from\s+mop\.evidence\s+import\s+\(([^)]*)\)|from\s+mop\.evidence\s+import\s+([^\n(]+)", text):
    ...
    pyproject = json.dumps(subprocess.run(["git", "show", f"{INTEGRATED}:pyproject.toml"], cwd=ROOT,
    ...
    seal("MOP_INTEGRATED_AUTHORITY.json", {
        "schema": "mop-integrated-authority/v1", "integrated_commit": INTEGRATED, "successor_head": head,
        "successor_branch": "agent/mop-integrated-substrate-forge",
        "collapse_source": subprocess.run(["git", "rev-parse", COLLAPSE], cwd=ROOT, capture_output=True, text=True).stdout.strip(),
        "forge_source": subprocess.run(["git", "rev-parse", FORGE], cwd=ROOT, capture_output=True, text=True).stdout.strip(),
        "merge_commit": "d023d9d", "code_tree_replacement_commits": ["9de20f0", "64d2801"],
        "preserved_substrate_dirs": list(SUBSTRATE_DIRS), "preserved_proof_dirs": ["proof"],
        "test_invariant_changes": test_seam,
        "registry_config_replacements": ["registry/experiments.yaml", "configs/"],
        "excluded_branches": [FORGE, "agent/mop-autonomous-substrate-evolution", "agent/mop-substrate-genesis-v2",
                              "agent/mop-evidence-salvage", COLLAPSE],
        "rollback": [f"git checkout {INTEGRATED}", f"git checkout {FORGE}", f"git checkout {COLLAPSE}"],
        "audits": audits,
    })
    seal("MOP_INTEGRATED_ANCESTRY.json", {
        "schema": "mop-integrated-ancestry/v1",
        "chain": ["collapse condensation -> dd5498a", "substrate/evidence lineage -> d33dc69 (forge boundary)",
                  "merge d023d9d (-X theirs)", "wholesale code adoption 9de20f0", "registry/config + invariant scoping 64d2801"],
        "counts": {"integrated_files": len(integ), "collapse_files": len(coll), "forge_files": len(forg),
                   "collapse_retained": len(collapse_retained), "substrate_retained": len(substrate_retained),
                   "generated": len(generated), "proof_preserved_beyond_collapse": len(proof_preserved)},
    })
    seal("MOP_INTEGRATED_ARTIFACT_MAP.json", {
        ...
        "registries": registries, "config_roots": config_roots, "clis": sorted(set(clis)),
    })
    print("unexpected:", len(unexpected), "| audits pass:", audits["all_pass"])
    print("collapse_retained", len(collapse_retained), "substrate_retained", len(substrate_retained),
          "generated", len(generated), "proof_preserved", len(proof_preserved))
    if unexpected[:5]:
