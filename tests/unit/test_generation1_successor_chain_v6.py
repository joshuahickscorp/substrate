...
from mop.studio import generation1_successor_chain_v6 as chain
...
def test_v6_adopts_exact_live_parent_under_fresh_identity_and_supersedes_v5(tmp_path: Path) -> None:
    ...
    assert status["schema"] == chain.STATUS_SCHEMA == "mop-generation1-successor-evidence-chain-status/v6"
    assert status["program_id"] == chain.CHAIN_ID == "generation1-successor-evidence-chain-v6"
    assert status["supersedes"] == "generation1-successor-evidence-chain-v5"
    assert status["state"] == "waiting_legacy"
    assert status["horizon_program"]["program_id"] == "generation1-successor-horizon-v1"
    assert launches == []


def test_v6_state_binds_wrapper_and_superseded_v5_and_inherited_v3_base(tmp_path: Path) -> None:
    spec = _spec(tmp_path)
    parent = _parent(tmp_path, spec, processes=(), launches=[])

    authority = parent.state["parent_implementation"]

    assert authority["path"].endswith("generation1_successor_chain_v6.py")
    assert authority["sha256"] == chain.sha256_file(Path(chain.__file__).resolve())
    assert authority["superseded_predecessor"]["path"].endswith("generation1_successor_chain_v5.py")
...
def test_v6_status_validator_requires_supersedes_v5(tmp_path: Path) -> None:
    spec = _spec(tmp_path)
    _write(spec.status_path, _status(spec))
    ...
    tampered["supersedes"] = "generation1-successor-evidence-chain-v4"
...
def test_tolerable_transient_worker_gate_truth_table(tmp_path: Path) -> None:
    ...
    reparenting = _process(
        tmp_path, pid=45239, label=f"{prefix}0490", command=(f"{prefix}0490",), pgid=45239, ppid=714
    )
    assert parent._tolerable_transient_worker(reparenting, owner, spec) is True

    regrouping = _process(
        tmp_path, pid=45240, label=f"{prefix}0491", command=(f"{prefix}0491",), pgid=714, ppid=1
    )
    assert parent._tolerable_transient_worker(regrouping, owner, spec) is True

    clean_residual = _process(
        tmp_path, pid=45241, label=f"{prefix}0492", command=(f"{prefix}0492",), pgid=714, ppid=714
    )
    ...
    assert parent._tolerable_transient_worker(only_cwd, owner, spec) is False

    foreign_cwd = _process(
        tmp_path,
        pid=45243,
        label=f"{prefix}0494",
        command=(f"{prefix}0494",),
        pgid=45243,
        ppid=714,
...
def test_v6_tolerates_boundary_transient_worker_until_clean_residual(tmp_path: Path) -> None:
    spec = _spec(tmp_path)
    _write(spec.status_path, _status(spec))
...
def test_v6_tolerates_boundary_transient_worker_that_exits_exactly(tmp_path: Path) -> None:
    spec = _spec(tmp_path)
    _write(spec.status_path, _status(spec))
    owner = _process(tmp_path, pid=67790, label=spec.process_label, command=(spec.process_label, ""))
    worker_label = f"{spec.child_label_prefixes[0]}0490"
    ...
    tables = [(owner, transient), (owner,), (owner,)]
    calls = 0
    ...
    parent = _parent(
        ...
        launches=[],
        process_table_fn=process_table,
        identity_probe_fn=lambda identity: "gone" if identity.get("pid") == transient.pid else "alive",
    )
...
def test_v6_persistent_boundary_transient_worker_fails_after_bounded_resamples(tmp_path: Path) -> None:
    spec = _spec(tmp_path)
    _write(spec.status_path, _status(spec))
    owner = _process(tmp_path, pid=67790, label=spec.process_label, command=(spec.process_label, ""))
    worker_label = f"{spec.child_label_prefixes[0]}0490"
    ...
    assert str(transient.pid) in status["problems"][-1]
    assert calls == chain.PROCESS_TRANSITION_ATTEMPTS + 1
    assert len(sleeps) == chain.PROCESS_TRANSITION_ATTEMPTS
    assert launches == []


def test_v6_boundary_transient_worker_that_turns_foreign_fails_closed(tmp_path: Path) -> None:
    spec = _spec(tmp_path)
    _write(spec.status_path, _status(spec))
    owner = _process(tmp_path, pid=67790, label=spec.process_label, command=(spec.process_label, ""))
    worker_label = f"{spec.child_label_prefixes[0]}0490"
...
def test_v6_boundary_transient_worker_pid_reuse_fails_closed(tmp_path: Path) -> None:
    spec = _spec(tmp_path)
    _write(spec.status_path, _status(spec))
    owner = _process(tmp_path, pid=67790, label=spec.process_label, command=(spec.process_label, ""))
    worker_label = f"{spec.child_label_prefixes[0]}0490"
...
def test_v6_foreign_cwd_worker_claiming_ownership_still_fails_closed(
    tmp_path: Path,
    pgid: int,
    ppid: int,
) -> None:
...
def test_v6_title_transition_cannot_mask_hard_same_group_intruder(tmp_path: Path) -> None:
    spec = _spec(tmp_path)
    _write(spec.status_path, _status(spec))
...
def test_v6_clean_worker_set_adopts_without_resample(tmp_path: Path) -> None:
    spec = _spec(tmp_path)
    _write(spec.status_path, _status(spec))
    owner = _process(tmp_path, pid=67790, label=spec.process_label, command=(spec.process_label, ""))
    worker = _process(
...
def test_v6_detached_start_emits_one_exact_v6_run_subcommand(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "generation1-successor-evidence-chain-v6"
    launches: list[tuple[list[str], dict[str, Any]]] = []
    ...
    expected = [
        ...
        str(chain.REPO_ROOT / "scripts/mop_generation1_successor_chain_v6.py"),
        "run",
        "--execute",
        "--root",
        str(root),
