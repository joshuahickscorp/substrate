...
from mop.studio import generation1_successor_chain_v7 as v7
from mop.studio import generation1_successor_extension_chain_v4 as extension_chain
from mop.studio import generation1_successor_recovery_launcher_v2 as recovery
...
def _adopter_status(state: str, *, pid: int, create_time: float | None = None) -> dict[str, Any]:
    capsules = {spec.stage_id: v7.v3._empty_capsule() for spec in v7.legacy_specs(v7.REPO_ROOT)}
    capsules["successor_horizon"] = v7.v3._empty_capsule()
    ...
    implementation = v7._implementation_authority(v7.REPO_ROOT)
    core: dict[str, Any] = {
        "schema": v7.STATUS_SCHEMA,
        "program_id": v7.CHAIN_ID,
        "created_at": NOW,
        "updated_at": NOW,
        "supervisor": {
            "pid": pid,
        ...
        "state": state,
        "horizon_program": v7._horizon_authority(v7.DEFAULT_HORIZON_PROGRAM, v7.REPO_ROOT),
        "capsules": capsules,
        "counts": {
            "complete": completed,
            "total": len(capsules),
...
def _extension_status(state: str, *, pid: int, create_time: float | None = None) -> dict[str, Any]:
    capsules = {
        "predecessor_chain_v7": extension_chain._empty_row(),
    ...
    if complete:
        ...
        capsules["predecessor_chain_v7"].update(
...
def test_recovery_v2_requires_explicit_execution() -> None:
    with pytest.raises(recovery.SuccessorChainRecoveryRefused, match="explicit --execute"):
        recovery.start_recovery(execute=False)


def test_recovery_v2_starts_v7_before_the_observation_only_extension_v4() -> None:
    ...
    assert result["schema"] == "mop-generation1-successor-chain-recovery-start/v2"
    assert result["program_id"] == "generation1-successor-chain-recovery-v2"
...
def test_recovery_v2_is_idempotent_when_both_parents_already_running() -> None:
...
def test_recovery_v2_unsafe_terminal_v7_state_prevents_extension_start(state: str) -> None:
...
@pytest.mark.parametrize("state", ("failure_hold", "integrity_hold", "drained"))
def test_recovery_v2_unsafe_terminal_extension_v4_state_refuses_success_receipt(state: str) -> None:
    with pytest.raises(recovery.SuccessorChainRecoveryRefused, match=f"unsafe state {state}"):
        recovery.start_recovery(
            execute=True,
...
def test_recovery_v2_rejects_self_sealed_adopter_supersedes_forgery() -> None:
    forged = _adopter_status("waiting_legacy", pid=41)
    core = dict(forged)
    core.pop("status_sha256")
    core["supersedes"] = "generation1-successor-evidence-chain-v5"
...
def test_recovery_v2_parser_exposes_one_idempotent_start_command() -> None:
