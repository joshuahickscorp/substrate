...
from mop.studio import generation1_successor_extension_chain_v4 as chain
...
def _predecessor_row(repo_root: Path, capsule_id: str, *, complete: bool) -> dict[str, Any]:
    ...
    if complete:
        artifact_path = (
            repo_root
            / "runs/generation1/generation1-successor-evidence-chain-v7/evidence"
            / f"{capsule_id}.json"
        )
        artifact = {
            "schema": "test-generation1-successor-v7-evidence/v1",
...
def _extension(
    ...
    *,
    ...
    predecessor_path = (
        tmp_path / "runs/generation1/generation1-successor-evidence-chain-v7" / "current_status.json"
    )
...
def test_extv4_predecessor_binding_points_at_v7_and_target_stays_horizon_v2(tmp_path: Path) -> None:
    ...
    assert chain.STATUS_SCHEMA == "mop-generation1-successor-extension-chain-status/v4"
    assert chain.PREDECESSOR_PROGRAM_ID == "generation1-successor-evidence-chain-v7"
    assert chain.PREDECESSOR_STATUS_SCHEMA == "mop-generation1-successor-evidence-chain-status/v7"
    assert chain.TARGET_PROGRAM_ID == "generation1-successor-horizon-v2"
    assert str(chain.DEFAULT_ROOT).endswith(
        "runs/generation1/generation1-successor-extension-chain-v4"
    )
    assert str(chain.DEFAULT_PREDECESSOR_STATUS).endswith(
        "runs/generation1/generation1-successor-evidence-chain-v7/current_status.json"
    )
    assert str(chain.DEFAULT_TARGET_PROGRAM).endswith(
        "configs/campaign/generation1_successor_horizon_v2.json"
    )
    assert chain.predecessor_v7.CHAIN_ID == "generation1-successor-evidence-chain-v7"


def test_extv4_initial_state_binds_v7_predecessor_and_horizon_v2_capsule(tmp_path: Path) -> None:
    extension, _loads, _program = _extension(tmp_path)
    predecessor = extension.state["predecessor"]

    assert predecessor["program_id"] == "generation1-successor-evidence-chain-v7"
    assert predecessor["status_schema"] == "mop-generation1-successor-evidence-chain-status/v7"
    assert predecessor["status_path"].endswith(
        "generation1-successor-evidence-chain-v7/current_status.json"
    )
...
def test_extv4_waits_for_v7_after_validating_target_without_starting_or_signalling(
    ...
    assert status["predecessor"]["program_id"] == "generation1-successor-evidence-chain-v7"
    assert status["predecessor"]["signals_allowed"] is False
    assert status["capsules"]["predecessor_chain_v7"]["status"] == "observed"
...
def test_extv4_terminal_v7_failure_holds_without_starting_target(tmp_path: Path) -> None:
    starts: list[object] = []
    extension, loads, _program = _extension(
        tmp_path,
    ...
    assert status["capsules"]["predecessor_chain_v7"]["status"] == "failure_hold"
    assert loads == [extension.target_program_path]
    assert starts == []


def test_extv4_requires_predecessor_supersedes_v6(tmp_path: Path) -> None:
...
def test_extv4_detached_start_emits_one_exact_v4_run_subcommand(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "generation1-successor-extension-chain-v4"
    ...
    expected = [
        ...
        str(chain.REPO_ROOT / "scripts/mop_generation1_successor_extension_chain_v4.py"),
        "run",
        "--execute",
        "--root",
        str(root),
