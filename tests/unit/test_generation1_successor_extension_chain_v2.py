...
from mop.studio import generation1_successor_extension_chain_v2 as chain
...
def _predecessor_row(
    ...
    *,
    ...
    if complete:
        artifact_path = (
            repo_root
            / "runs/generation1/generation1-successor-evidence-chain-v5/evidence"
            / f"{capsule_id}.json"
        )
        artifact = {
            "schema": "test-generation1-successor-v5-evidence/v1",
...
def _extension(
    ...
    *,
    ...
    predecessor_path = (
        tmp_path / "runs/generation1/generation1-successor-evidence-chain-v5" / "current_status.json"
    )
...
def test_waits_for_v5_after_validating_target_without_starting_or_signalling(
    tmp_path: Path,
    ...
    assert status["capsules"]["predecessor_chain_v5"]["status"] == "observed"
...
def test_terminal_v5_failure_holds_without_starting_target(tmp_path: Path) -> None:
    starts: list[object] = []
    extension, loads, _program = _extension(
        tmp_path,
    ...
    assert status["capsules"]["predecessor_chain_v5"]["status"] == "failure_hold"
    assert loads == [extension.target_program_path]
    assert starts == []


def test_self_sealed_empty_v5_complete_status_is_rejected(tmp_path: Path) -> None:
    ...
    assert status["capsules"]["predecessor_chain_v5"]["status"] == "pending"
    assert "v5 predecessor status fields drifted" in status["problems"][-1]


def test_structurally_complete_fake_v5_status_without_durable_state_is_rejected(
    tmp_path: Path,
) -> None:
    starts: list[object] = []
    extension, _loads, _program = _extension(
    ...
    assert status["capsules"]["successor_horizon_v2"]["status"] == "pending"
    assert starts == []
    assert "v5 predecessor complete snapshot is invalid" in status["problems"][-1]
...
def test_extension_v2_replays_four_v5_snapshots_and_two_v1_admissions_before_one_start(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    trace: list[str] = []
    extension: chain.SuccessorExtensionChain
    stable = {"state": "complete", "status_sha256": "a" * 64}

    def replay_v5(_status: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    def start(_program: Any, **kwargs: Any) -> Mapping[str, Any]:
        trace.append("start")
        assert kwargs == {"execute": True, "use_caffeinate": True}
        persisted = json.loads(extension.state_path.read_text(encoding="utf-8"))
        assert persisted["capsules"]["successor_horizon_v2"]["status"] == "launching"
        return {}
    ...
    extension._validate_complete_predecessor_snapshot = replay_v5  # type: ignore[method-assign]
...
def test_prelaunch_replays_v5_again_after_v1_before_v2_starter(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[str] = []
    ...
    assert status["state"] == "integrity_hold"
    assert status["capsules"]["successor_horizon_v2"]["status"] == "pending"
    assert calls == ["v5", "v1-build", "v1-validate", "v5"]
    assert snapshots == []
    assert starts == []
...
def test_prelaunch_authority_is_rechecked_after_intent_publish(
    ...
    def validate_launch() -> None:
        ...
        if calls.count("validate") == 2:
            raise ValueError("v5 regressed after launch intent")
    ...
    assert starts == []
    assert "v5 regressed after launch intent" in status["problems"][-1]
...
def test_completed_predecessor_evidence_is_monotonic_and_must_remain_exact(
    ...
    first = extension.tick()
    predecessor = first["capsules"]["predecessor_chain_v5"]
    ...
    assert second["state"] == "integrity_hold"
    assert second["capsules"]["predecessor_chain_v5"]["finished_at"] == finished_at
    assert second["capsules"]["predecessor_chain_v5"]["artifacts"] == artifacts
