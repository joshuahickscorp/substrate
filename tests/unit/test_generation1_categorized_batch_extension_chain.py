...
from mop.studies import generation1_successor_categorized_batch_wave as categorized_wave
from mop.studies import generation1_successor_mechanics_queue as mechanics
from mop.studio import generation1_categorized_batch_extension_chain as chain
...
def _program_manifest(
    ...
    *,
    ...
    runner.write_text("# immutable categorized-chain test runner\n")
    ...
    capsule_core: dict[str, Any] = {
        ...
        "artifacts": [
            {
                "path": artifact,
                "schema": "mop-categorized-chain-test-artifact/v1",
                "fields": {"ok": True},
...
def _fixture(
    ...
    *,
    ...
    chain.CategorizedBatchExtensionChain,
    g1.Program,
    g1.Program,
]:
    predecessor_path, predecessor = _program_manifest(
        ...
        manifest_name="generation1_successor_horizon_v2.json",
    )
    ...
    owner = chain.CategorizedBatchExtensionChain(
...
def test_incomplete_predecessor_never_reaches_host_or_target(tmp_path: Path) -> None:
    ...
    assert status["capsules"]["successor_horizon_v2"]["status"] == "observed"
    assert status["capsules"]["categorized_batch_wave_v1"] == chain._empty_row()
    assert admissions == []
    assert starts == []
    assert (
        chain.validate_categorized_extension_status(
            status,
            repo_root=tmp_path,
...
def test_busy_host_never_starts_target(tmp_path: Path) -> None:
    ...
    assert status["capsules"]["successor_horizon_v2"]["status"] == "complete"
    assert status["capsules"]["categorized_batch_wave_v1"]["status"] == "pending"
    assert starts == []
    assert (
        chain.validate_categorized_extension_status(
            status,
...
def test_clean_predecessor_and_two_host_gates_start_target_once(tmp_path: Path) -> None:
    ...
    target_row = status["capsules"]["categorized_batch_wave_v1"]
...
def test_predecessor_change_after_launch_intent_fails_closed(tmp_path: Path) -> None:
    ...
    assert starts == []
    assert any("changed across categorized launch intent" in problem for problem in status["problems"])
...
def test_live_target_is_observed_without_rechecking_or_restarting(tmp_path: Path) -> None:
    ...
    assert status["state"] == "waiting_target"
    assert status["capsules"]["categorized_batch_wave_v1"]["status"] == "running"
    assert admissions == []
    assert starts == []
...
def test_complete_target_rejects_conflicting_visible_supervisor(tmp_path: Path) -> None:
    ...
    assert status["state"] == "integrity_hold"
    assert any(
        "completed categorized target conflicts with visible supervisor" in problem
...
def test_complete_generic_status_replays_artifacts(tmp_path: Path) -> None:
    ...
    except chain.CategorizedBatchExtensionRefused as exc:
...
def test_repeated_detached_start_returns_same_live_parent_without_spawn(
    ...
    first = chain.start_categorized_extension_detached(
        root=root,
        execute=True,
        use_caffeinate=False,
    )
    second = chain.start_categorized_extension_detached(
        root=root,
        execute=True,
        use_caffeinate=False,
    )
...
def test_fresh_detached_start_binds_acknowledged_visible_parent_not_wrapper(
    ...
    result = chain.start_categorized_extension_detached(
        root=root,
        execute=True,
        use_caffeinate=True,
    )
...
def test_fresh_detached_start_rejects_unproven_visible_parent(
    ...
    with pytest.raises(
        chain.CategorizedBatchExtensionRefused,
        match="not provably alive",
    ):
        chain.start_categorized_extension_detached(
            root=root,
            execute=True,
            use_caffeinate=False,
        )
...
def test_partial_category_route_neither_reruns_nor_resurrects_failed_lane(
    ...
    routing = {
        category_id: {
            "continue": False,
            "eligible_lane_ids": [],
            "pruned_lane_ids": list(categorized_wave.CATEGORY_LANES[category_id]),
            "reason": "null_safe_pruning_after_route_or_mechanics_warning",
        }
        for category_id in categorized_wave.CATEGORY_IDS
    }
    ...
    monkeypatch.setattr(categorized_wave, "_execute_pending", execute)
    monkeypatch.setattr(
        categorized_wave,
        "_build_category_result",
        lambda **_kwargs: {"schema": "test-partial-route"},
    )
    monkeypatch.setattr(categorized_wave, "validate_category", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(categorized_wave, "atomic_write_json", lambda *_args, **_kwargs: None)

    categorized_wave.run_category(
        root=tmp_path,
        epoch_index=1,
    ...
    category_payloads = {
        category_id: {
            "lane_results": {
                ...
                for lane_id in categorized_wave.CATEGORY_LANES[category_id]
            },
            "execution": {
    ...
    monkeypatch.setattr(categorized_wave, "_read_object", read_for_classifier)
    monkeypatch.setattr(
        categorized_wave,
        "_validated_context_for_epoch",
        lambda *_args, **_kwargs: categorized_wave._WaveValidationContext(
    ...
    with pytest.raises(ValueError, match="attempted lane resurrection"):
        categorized_wave.build_classification(root=tmp_path, epoch_index=1)
