"""Leg 11B tests. Exercise the pure SEM/recommendation math deterministically (it can fail
on a wrong formula), and run seed_variance end to end at a tiny seed count to assert E1 was
actually rerun and aggregated (non-vacuous: the per-seed gaps and per-S rows must be real)."""
...
def test_aggregate_uses_first_S_seeds_and_flags_sign():
    # gaps 1..3 are far apart (wide spread) then tighten; first two bracket a 0.10 mean.
    per_seed = [{"gap": g} for g in (0.05, 0.15, -0.05, 0.11, 0.10, 0.12)]
    rows = _aggregate(per_seed, (1, 2, 3, 6))
    by_S = {r["S"]: r for r in rows}
    # S=2: both positive -> sign stable; mean is the average of the first two gaps
    assert by_S[2]["sign_stable"] is True
    assert math.isclose(by_S[2]["mean_gap"], 0.10, rel_tol=1e-9)
    # S=3 includes a negative gap -> sign no longer stable
    assert by_S[3]["sign_stable"] is False
    # single seed is never sign-stable (one observation cannot fix a direction)
    assert by_S[1]["sign_stable"] is False
    # SEM shrinks as more seeds land near the mean (S=6 tighter than the wide S=3)
    assert by_S[6]["sem"] < by_S[3]["sem"]


def test_recommend_picks_smallest_qualifying_S():
    # SEM falls below the 0.05 (ranking) tol only at S=5 here; sign stable throughout.
    per_S = [
    ...
    # looser tol (sanity) is satisfied earlier via the flat-mean fallback at S=5,
    # but the strict sign+sem can also pass sooner under a generous tolerance.
...
def test_seed_variance_actually_reruns_e1():
    # Real (toy) run: reuses two seeds, so E1 must execute twice and produce real gaps.
    ...
    for r in out["per_seed"]:
        # gap is exactly protected_bwt - naive_bwt, recomputed independently here
        assert math.isclose(r["gap"], r["protected_bwt"] - r["naive_bwt"], rel_tol=1e-9, abs_tol=1e-9)
    # the requested grid is {1,2} for max_seeds=2, and each row carries the measured stats
    assert [r["S"] for r in out["per_S"]] == [s for s in SEED_GRID if s <= 2]
    assert all(r["S"] >= 0 for r in out["per_S"]) and out["per_S"][-1]["sem"] >= 0.0
    # recommendation is a concrete seed count within the tested grid
