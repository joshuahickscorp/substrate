...
from mop.cognition import plasticity as P
...
def test_fast_adaptation_does_not_touch_shared_parameters():
    ok = P.Adaptation("adapter_update", target="domain_local_adapters", domain="har",
                      checkpoint="proof/.../adapter.json")
...
def test_slow_adaptation_requires_repeated_evidence_and_rollback():
    good = dict(repetitions=4, held_out={"before": 0.70, "after": 0.82},
                retention={"speech": 0.0, "harth": -0.01})
    applied = P.slow_adapt(P.Adaptation("core_update", "selected_core_groups", "har",
                                        checkpoint="proof/.../core.pt"), **good)
    assert applied["applied"] is True and applied["refusals"] == []

    # each of the four requirements alone is enough to refuse
    def refused(**over):
        a = P.Adaptation("core_update", "selected_core_groups", "har",
    ...
    with pytest.raises(P.Refused):
        P.slow_adapt(P.Adaptation("state_update", "persistent_state", "har",
                                  checkpoint="c"), **good)
...
def test_learning_to_learn_requires_cross_task_generalization():
    report = P.learning_to_learn({
        "consolidate_on_verification": {"derived_from": ["har"],
                                        "gains": {"har": 0.30, "speech": 0.09, "harth": 0.11}},
        "consolidate_on_luck": {"derived_from": ["har"],
...
def test_a_permitted_reorganization_still_has_to_beat_simple_routing_after_cost():
    ...
    strong = P.reorganize(permitted, measured={"fixed_routing": 0.70, "simple_routing": 0.74,
                                               "reorganized": 0.90}, cost=0.05)
    ...
    costly = P.reorganize(permitted, measured={"fixed_routing": 0.70, "simple_routing": 0.74,
                                               "reorganized": 0.90}, cost=0.15)
    assert costly["applied"] is False and "cost is charged" in costly["reason"]

    # beating only the weaker control earns nothing
    weak = P.reorganize(permitted, measured={"fixed_routing": 0.70, "simple_routing": 0.88,
                                             "reorganized": 0.80}, cost=0.0)
