...
18 defect classes discovered in the Fast State Plasticity Forge and its predecessors.
...
| D18 | a label permutation control scored against zero difference instead of the majority class rate | with permuted labels no arm may separate from another | two architectures with nothing to learn degenerate differently. The pooled control collapsed onto one class and scored below the majority class rate while the recurrent core spread its predictions and scored at it, leaving a 0.047 difference that is not signal | `mop.method.runs.mutations.e1_mutations` | mutation_attacks |
