...
class LSTMBaseline(nn.Module):
    def __init__(s, ch, no):
        super().__init__()
        s.proj = OwnedProj(ch)
        s.core = nn.LSTM(LATENT, LATENT, batch_first=True)
        s.head = nn.Linear(LATENT, no)

    def forward(s, x):
        o, _ = s.core(s.proj(x))
        return s.head(o[:, -1]), None
...
class ATAttrib(nn.Module):
    ...
    def __init__(s, ch, no, use_fast=True, use_medium=True, use_slow=True):
        super().__init__()
        s.proj = OwnedProj(ch)
        s.fast = nn.GRU(LATENT, FASTH, batch_first=True)
        ...
        s.param_groups = {
            "proj": list(s.proj.parameters()),
            "fast": list(s.fast.parameters()),
...
class FPredCons(nn.Module):
    ...
    def __init__(s, ch, no, objective="multi_horizon"):
        ...
        s.pred = nn.Linear(LATENT, LATENT)
        s.head = nn.Linear(LATENT * 2, no)
        s.register_buffer("consolidated", torch.zeros(LATENT))
        s.objective = objective
        s.param_groups = {
            "proj": list(s.proj.parameters()),
            "core": list(s.core.parameters()),
            "pred": list(s.pred.parameters()),
            "head": list(s.head.parameters()),
        }
...
def aux_loss(internals, objective, rng):
    ...
    for h in horizons:
        if T - h > 0:
            loss = loss + F.mse_loss(o[:, : T - h], z[:, h:].detach())
            n += 1
    return loss / max(1, n)
...
def run_arm(kind, seed, steps=STEPS, objective="multi_horizon", eligibility="all", use=(True, True, True)):
    ...
    mem = SeqMemory(cap=600)
    nT = len(tasks)
    acc = np.zeros((nT, nT))
    prev = None
    for t in range(nT):
        x, y = tasks[t]["x"], tasks[t]["y"]
        model.train()
        for _ in range(steps):
            ...
            if mem.size():
                ...
                if sp:
                    xb = torch.cat([xb, sp[0]])
                    yb = torch.cat([yb, sp[1]])
                ...
                pred = torch.cat(
                    [
                        (
                            model(xt[k : k + 256])[0]
                            if kind == "lstm"
                            else model(xt[k : k + 256], update_medium=False)[0]
                        ).argmax(1)
...
def main():
    ...
    results = {
        a: {
            "means": {k: round(v, 4) for k, v in means[a].items()},
            "util": round(um[a], 4),
            "effect_vs_lstm_gdumb": eff(a),
        }
        for a in ARMS
    }
    ...
    verdicts = {
        "AT1_state_attribution": "null"
        if results["AT1_attrib"]["effect_vs_lstm_gdumb"]["lower_95_cb"] < 0.05
        else "positive",
        "AT2_interference_localized": "null"
        if results["AT2_localized"]["effect_vs_lstm_gdumb"]["lower_95_cb"] < 0.05
        else "positive",
        "F_predictive_consolidation": "null"
        if max(results[a]["effect_vs_lstm_gdumb"]["lower_95_cb"] for a in ("F_multi_horizon", "F_next_step"))
        < 0.05
        else "positive",
    ...
    doc = {
        "schema": "mop-integrated-forge-rounds/v1",
        "bed": "HAR_raw (valid temporal, order matters)",
