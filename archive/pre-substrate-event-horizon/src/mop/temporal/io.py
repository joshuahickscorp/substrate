...
PROCESS_SOURCE_COMMIT = commit()
PROCESS_SOURCE_TREE_OID = subprocess.run(
    ["git", "rev-parse", f"{PROCESS_SOURCE_COMMIT}^{{tree}}"], cwd=ROOT,
    capture_output=True, text=True,
).stdout.strip() if PROCESS_SOURCE_COMMIT else ""
def launch_commit() -> str:
    return os.environ.get("MOP_LAUNCH_SOURCE_COMMIT") or PROCESS_SOURCE_COMMIT
def launch_tree_oid() -> str | None:
    return os.environ.get("MOP_LAUNCH_SOURCE_TREE_OID") or PROCESS_SOURCE_TREE_OID or None
...
def seal(name: str, obj: dict, subdir: str = "") -> Path:
    ...
    obj.setdefault("source_commit", launch_commit())
    if launch_tree_oid():
        obj.setdefault("source_tree_oid", launch_tree_oid())
...
def run_json(name: str, obj: dict, subdir: str = "") -> Path:
    ...
    doc.setdefault("source_commit", launch_commit())
    if launch_tree_oid():
        doc.setdefault("source_tree_oid", launch_tree_oid())
