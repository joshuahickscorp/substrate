...
def main():
    ...
    registries = (
        sorted(str(p.relative_to(ROOT)) for p in (ROOT / "registry").rglob("*") if p.is_file())
        if (ROOT / "registry").is_dir()
        else []
    )
    config_roots = sorted(
        {
    ...
    io.seal(
        "MOP_FAST_STATE_CODE_REPORT.json",
        {
            ...
            "deletions": [],
            "deletion_policy": "inherited runners under substrate_evo, forge, frontier, campaign2, salvage "
            "and "
