...
- **Live disk kill-switch finding**: free disk on this laptop drifted from ~63GB to ~53-58GB over the course of this session from system-level activity unrelated to this repo (confirmed: pytest tmp usage and this repo's own `runs/` growth were both under 200MB combined). `m3pro-local-max`'s `min_free_disk_gb=60` floor genuinely tripped a few times, correctly stopping `local-max` runs and failing the 3 integration tests that exercise the real kill switch against real `disk_usage()` (not mocked). This is the safety floor working as designed, not a bug; on the Studio, `studio-1tb`'s far larger disk budget removes this constraint entirely, but it is worth knowing this specific laptop is now running close to its own floor.
