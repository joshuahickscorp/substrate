...
| RED-successor-horizon-v1-v2 | 8 | verified_reduction | verified | Retire completed successor horizon v1/v2 campaign stacks | audit completed Generation-1 mechanics, context-routing, and final-campaign layers |
| RED-completed-generation1-program | 8 | verified_reduction | verified | Retire the completed Generation-1 program and generated proof surface | delete the next completed experimental vertical slice |
| RED-pre-generation1-campaign-escs | 8 | verified_reduction | verified | Retire the pre-Generation1 campaign, throttle, and ESCS substrate stack | delete the next sealed campaign or unreachable experimental framework |
...
### STARSS23 lifecycle cluster 17 (current checkpoint)

- count interface: sealed and alternate-topology count gates share held-fixed feature/state assembly, batch probability, online inference, threshold decision, and report shape.
- DoA interface: both gate architectures share only topology-neutral probability, online inference, and threshold decision; their forward and optimizer mathematics remain separate.
- parity: four parameter digests and all probability, decision, refusal, report payload, repr, and dict surfaces match checkpoint b8e98dd.
- validation: focused gate battery 91/91; full STARSS-focused suite 493/493 in 153.21s under nice -n 10.
- production and owned Python net reduction: 58 LOC.
- cumulative verified owned Python reduction: 4,308 LOC.
- rollback_tag: mop-collapse-starss23-lifecycle-17.
- next_exact_edit: remeasure remaining variant producer shells against the declaration target.

### STARSS23 lifecycle cluster 18 (current checkpoint)

- lifecycle authority: spatial DoA, SuperFlux spectral, and interchannel coherence now declare only corpus preparation, frontend identity, provenance payloads, and variant anchors; one shared executor owns prereg reads, seeded runs, FLOP budgets, statistics, controls, receipts, artifact envelopes, and finalization.
- preservation: all three independent verifiers remain untouched, and frontend-specific cache/raw adapter preparation, FLOP charges, hypotheses, refusal classes, and artifact vocabulary stay local.
- parity: all three complete sealed artifact dictionaries exactly match checkpoint b7fcb0b under fixed clocks, including receipts, spread evidence, and final seals.
- validation: focused independent-verifier battery 44/44; full STARSS-focused suite 493/493 in 155.83s under nice -n 10.
- production and owned Python net reduction: 260 LOC.
- cumulative verified owned Python reduction: 4,568 LOC.
- rollback_tag: mop-collapse-starss23-lifecycle-18.
- next_exact_edit: remeasure adjacent count and DoA producer shells against the declaration target.

### STARSS23 lifecycle cluster 19 (current checkpoint)

- lifecycle authority: the base count bed plus swapped-fold, re-authored-provider, alternate-gate, and clip-macro reproductions now share one corpus-preparation and sealed-artifact executor.
- preservation: provider-specific noisy-TV namespaces, seed runners, FLOP models, splits, preregistration builders, gate/estimator identities, clip-cluster readout, survival conjunctions, refusal classes, and all five independent verifier implementations remain local.
- parity: five complete real-data artifact dictionaries and seals exactly match checkpoint a7916c2 under fixed clocks, including the swapped split and clip-macro evidence extensions.
- validation: focused producer and independent-verifier battery 73/73 in 72.92s; full STARSS-focused suite 493/493 in 145.36s under nice -n 10.
- production and owned Python net reduction: 407 LOC.
...
### Mechanism runner and construction-search engine collapse (current checkpoint)

- joint-axis runner: calibrated uncertainty, reducible novelty, stability/plasticity v1, and stability/plasticity r2 now declare their schemas, axes, controls, null, margins, and mechanism provider against one typed runner lifecycle.
- exact joint-axis parity: all four mechanisms preserve result and receipt payloads under null and favorable regimes at seeds 0 and 7: c88fd435a128aed19cdd7600dcd3461e11b2f5e7069ce302b68e05fa6b0a6a67.
- construction-search selection: the ordinary and vector runner shells now share one lifecycle; the bit-identical faster vector engine is the only maintained evaluator.
- exact construction-search parity: ordinary/vector result and receipt payloads remain exact at seeds 0, 7, and 103: 66253d4b0f4acb8ca4b0c6ca5c102f8fe870fe9f0cf6230194f452395c14b48b.
- physical deletion: the 198-line scalar candidate and 657 lines of completed scalar-vs-vector equivalence harnesses were removed; independent chunked-oracle correctness, RSS, wall-time, and ordinary-runner validation remain.
- owned Python change: source 301 added/932 deleted; tests 13/683; batch net -1,301 LOC; cumulative verified reduction: 56,569 LOC.
