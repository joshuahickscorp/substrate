...
ARTIFACT_ROOT = ROOT / "evidence" / "artifacts" / "substrate" / "execution-forge"
THREAD_VARIABLES = (
    "OMP_NUM_THREADS",
...
def _prepare(root: Path) -> None:
    ...
    (root / "evidence" / "artifacts").mkdir(parents=True, exist_ok=True)
...
def _configure(root: Path, thread_budget: int) -> None:
    ...
    evidence.ARTIFACTS = root / "evidence" / "artifacts"
