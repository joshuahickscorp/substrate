...
REQUIREMENTS = [
    ("R01", "Freeze the inherited interpretation",
     ["MOP_FAST_STATE_FORGE_START_AUTHORITY.json", "MOP_FAST_STATE_FORGE_START_AUTHORITY.md"], [],
     "run fastforge.runs.authority"),
    ("R02", "Bind the inherited nulls", ["MOP_FAST_STATE_BINDING_NULLS.json"], ["R01"],
     "run fastforge.runs.authority"),
    ("R03", "Repair cross domain arm aliasing",
     ["MOP_CROSS_DOMAIN_ARM_AUTHORITY.json", "MOP_CROSS_DOMAIN_ARM_AUDIT.json",
      "MOP_CROSS_DOMAIN_ARM_MUTATIONS.json"], ["R01"], "run fastforge.runs.armaudit"),
    ("R04", "Implement and seal Architecture G", ["MOP_ARCHITECTURE_G.json"], ["R03"],
     "run fastforge.runs.architectures"),
    ("R05", "Implement and seal Architecture H", ["MOP_ARCHITECTURE_H.json"], ["R03"],
     "run fastforge.runs.architectures"),
    ("R06", "Baseline convergence curves", ["MOP_BASELINE_CONVERGENCE_REPORT.json"], [],
     "run fastforge.runs.bench"),
    ("R07", "Resource profile", ["MOP_FAST_STATE_RESOURCE_REPORT.json"], [], "run fastforge.runs.bench"),
    ("R08", "Within domain battery, HAR", ["MOP_HAR_WITHIN_DOMAIN_REPORT.json"], ["R04", "R05", "R06"],
     "run fastforge.runs.withinrun shards then aggregate"),
    ("R09", "Within domain battery, Speech", ["MOP_SPEECH_WITHIN_DOMAIN_REPORT.json"], ["R04", "R05", "R06"],
     "run fastforge.runs.withinrun shards then aggregate"),
    ("R10", "Cross domain direction 1, HAR to Speech", ["MOP_HAR_TO_SPEECH_REPORT.json"],
     ["R04", "R05", "R06"], "run fastforge.runs.crossdomain shards then aggregate"),
    ("R11", "Cross domain direction 2, Speech to HAR", ["MOP_SPEECH_TO_HAR_REPORT.json"],
     ["R04", "R05", "R06"], "run fastforge.runs.crossdomain shards then aggregate"),
    ("R12", "Bidirectional synthesis", ["MOP_FAST_STATE_BIDIRECTIONAL_SYNTHESIS.json"], ["R10", "R11"],
     "run fastforge.runs.crossdomain aggregate"),
    ("R13", "Interference map", ["MOP_SUBSTRATE_INTERFERENCE_MAP.json", "MOP_SUBSTRATE_INTERFERENCE_REPORT.md"],
     ["R04", "R05"], "run fastforge.runs.interference shards then aggregate"),
    ("R14", "Plasticity action headroom gate",
     ["MOP_SUBSTRATE_PLASTICITY_ACTION_HEADROOM.json", "MOP_SUBSTRATE_PLASTICITY_POLICY_REPORT.json"],
     ["R13", "R12"], "run fastforge.runs.plasticity"),
    ("R15", "Architecture improvement rounds", ["MOP_FAST_STATE_ARCHITECTURE_COMPARISON.json"],
     ["R12", "R13"], "run fastforge.runs.rounds"),
    ("R16", "Functional reorganization", ["MOP_FUNCTIONAL_REORGANIZATION_REPORT.json"], ["R13"],
     "run fastforge.runs.reorg"),
    ("R17", "Task free context inference", ["MOP_TASK_FREE_CONTEXT_REPORT.json"], ["R13"],
     "run fastforge.runs.reorg"),
    ("R18", "Fast core representation analysis",
     ["MOP_FAST_CORE_REPRESENTATION_REPORT.json", "MOP_FAST_CORE_REPRESENTATION_REPORT.md"], ["R12"],
     "run fastforge.runs.represent"),
    ("R19", "Third temporal domain preflight", ["MOP_THIRD_TEMPORAL_DOMAIN_PREFLIGHT.json"], ["R06"],
     "run fastforge.runs.domaingate"),
    ("R20", "Code accounting", ["MOP_FAST_STATE_CODE_REPORT.json"], [], "run fastforge.runs.codereport"),
    ("R21", "Test report", ["MOP_FAST_STATE_TEST_REPORT.json"], ["R04", "R05"], "run fastforge.runs.testreport"),
    ("R22", "Independent verification", ["MOP_FAST_STATE_INDEPENDENT_VERIFICATION.json"],
     ["R12", "R13", "R14"], "run fastforge.verify"),
    ("R23", "Mutation suites", ["MOP_FAST_STATE_MUTATION_REPORT.json"], ["R12", "R13", "R14"],
     "run fastforge.runs.mutations"),
    ("R24", "Clean clone validation", ["MOP_FAST_STATE_CLEAN_CLONE.json"], ["R22", "R23"],
     "run fastforge.runs.cleanclone"),
    ("R25", "Evidence fabric indexing", ["MOP_FAST_STATE_EVIDENCE_FABRIC.json"], ["R24"],
     "run fastforge.runs.fabric"),
    ("R26", "Synthesis and next frontier",
     ["MOP_FAST_STATE_FORGE_SYNTHESIS.json", "MOP_FAST_STATE_FORGE_SYNTHESIS.md",
      "MOP_FAST_STATE_NEXT_FRONTIER.json"], ["R25"], "run fastforge.runs.synthesis"),
]

IMPLEMENTATION_TARGETS = {
    "evidence_and_falsification": 98, "orchestration": 98, "failure_understanding": 98,
    "owned_substrate": 98, "multi_timescale_and_partitioned_plasticity": 95,
    "functional_reorganization": 85, "cross_domain_entity": 90,
}
EVIDENCE_TARGETS = {
    "evidence_and_falsification": 95, "orchestration": 95, "failure_understanding": 95,
    "useful_plasticity": 60, "owned_substrate": 80, "partitioned_moldability": 70,
    "functional_reorganization": 55, "cross_domain_entity": 65,
}
...
def status():
    ...
    for rid, title, arts, deps, nxt in REQUIREMENTS:
        ...
        st[rid] = {
            "id": rid, "title": title, "artifacts": arts, "dependencies": deps,
            "status": "terminal" if done else ("ready" if all(
                all(io.exists(a) for a in dict((r[0], r[2]) for r in REQUIREMENTS)[d]) for d in deps)
                else "blocked"),
...
def evidence_scores(st):
    ...
    third = (io.load("MOP_THIRD_TEMPORAL_DOMAIN_PREFLIGHT.json").get("gate", {}).get("verdict")
             if io.exists("MOP_THIRD_TEMPORAL_DOMAIN_PREFLIGHT.json") else None)
    ...
    return {
        ...
        "owned_substrate": 45 if st["R08"]["status"] == "terminal" and st["R09"]["status"] == "terminal" else 10,
        ...
        "_verdicts": {"cross_domain": cross, "plasticity": plast, "reorganization": reorg,
...
def main():
    st = status()
    impl = {k: round(100 * sum(1 for r in v if st[r]["status"] == "terminal") / len(v))
            for k, v in DIMENSION_REQS.items()}
    ev = evidence_scores(st)
    io.seal("MOP_FAST_STATE_FORGE_STATE.json", {
    ...
    io.seal("MOP_FAST_STATE_PROGRESS_SCORECARD.json", {
        "schema": "mop-fast-state-progress-scorecard/v1",
        "implementation": impl, "implementation_targets": IMPLEMENTATION_TARGETS,
    ...
    rows = ["# Fast State Plasticity Forge ledger", "",
    ...
    for k in IMPLEMENTATION_TARGETS:
        rows.append(f"| {k} | {impl.get(k, 0)} | {IMPLEMENTATION_TARGETS[k]} | "
    ...
    io.seal_md("MOP_FAST_STATE_PROGRESS_SCORECARD.md",
