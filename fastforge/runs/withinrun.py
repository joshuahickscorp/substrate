...
SEEDS = [0, 1, 2, 3, 4]
DOMAINS = ["har", "speech"]
STRESS_ARMS = ["G0_always_trainable", "G1_frozen_after_first", "G4_adapters_only", "H_cosine_gate",
               "H_never_update", "AT_no_slow", "lstm_gdumb", "separate_per_context"]
BASELINES = ["gru", "lstm", "lstm_gdumb", "separate_per_context", "shared_adapters", "ewc", "reservoir",
             "no_memory", "joint_upper_bound", "bag_order_free"]
...
def shard(dname, seed):
    ...
    for arm in W.WITHIN_ARMS:
        ...
        print(f"  {dname} seed{seed} {arm:24s} plain avg={out[f'{arm}|plain']['metrics']['avg_final']:.3f} "
...
def summarize(dname, rows):
    ...
    keys = ["avg_final", "retention", "new_context", "return_to_prior", "future_adaptation",
            "avg_final_missing_observations", "missing_observation_drop", "params", "updates"]
    means = {a: {k: round(float(np.mean([rows[s][f"{a}|plain"]["metrics"][k] for s in seeds])), 4)
    ...
    for a in STRESS_ARMS:
        if f"{a}|limited" not in rows[seeds[0]]:
            continue
        stress[a] = {
            st: {"utility": round(float(np.mean([util(a, s, st) for s in seeds])), 4),
                 "vs_plain": S.E.effect([util(a, s, st) for s in seeds], [util(a, s) for s in seeds])}
            for st in ("limited", "small_mem")
        }
    passing = [a for a in arms if effects[a]["lower_95_cb"] >= io.SESOI]
    return {
        "domain": dname, "seeds": seeds, "steps_per_context": steps_for(dname),
        "learning_rate": S.lr_for(dname), "arms": arms,
        "means": means, "utility": um, "strongest_matched_baseline": strongest,
        ...
        "simplification_check": {
            "G1_frozen_vs_G0_always": S.E.effect([util("G1_frozen_after_first", s) for s in seeds],
                                                 [util("G0_always_trainable", s) for s in seeds]),
            "G4_adapters_only_vs_G0": S.E.effect([util("G4_adapters_only", s) for s in seeds],
                                                 [util("G0_always_trainable", s) for s in seeds]),
        },
    }
...
def main():
    ...
    for dname in DOMAINS:
        ...
        io.seal(f"MOP_{dname.upper()}_WITHIN_DOMAIN_REPORT.json",
