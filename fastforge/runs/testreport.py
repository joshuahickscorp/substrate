...
CRITICAL = ["fastforge/arch.py", "fastforge/engine.py", "fastforge/sequence.py", "fastforge/arms.py",
            "fastforge/within.py", "fastforge/data.py"]
STATE_TRANSITIONS = [
    "acquire first domain", "domain boundary carry", "domain boundary reinitialize",
    "second domain acquisition under a partition", "shared group frozen", "shared group gated",
    "return to the first domain", "future unit adaptation", "anchor restoration", "memory admission",
    "memory eviction", "context routing",
]


def run(cmd, env=None):
    return subprocess.run(cmd, cwd=io.ROOT, capture_output=True, text=True,
...
def main():
    ...
    cov = run(["python", "-m", "pytest", "tests/unit/test_fast_state_forge.py", "-q",
               "--cov=" + ",".join(CRITICAL), "--cov-branch", "--cov-report=json:cov.json"], env)
    cov_data = {}
    p = io.ROOT / "cov.json"
    if p.is_file():
        ...
        cov_data = {
            ...
            "per_file": {k: round(v["summary"]["percent_covered"], 2) for k, v in raw.get("files", {}).items()},
        }
        p.unlink()
    tail = (full.stdout or full.stderr).strip().splitlines()
    io.seal("MOP_FAST_STATE_TEST_REPORT.json", {
        ...
        "coverage": cov_data or {"note": "coverage plugin unavailable in this environment, recorded as a "
                                         "skip rather than claimed"},
        "coverage_targets": {"critical_statement_ge_92": cov_data.get("statement_coverage_percent", 0) >= 92,
                             "branch_ge_82": cov_data.get("branch_coverage_percent", 0) >= 82},
        "property_and_metamorphic_transitions_covered": STATE_TRANSITIONS,
        "mandatory_tests": [
            "cross domain arm non aliasing", "trainable group identity", "frozen group identity",
            "fast core persistence", "domain adapter isolation", "anchor restoration", "fast delta bounds",
            "domain order reversal", "bidirectional transfer", "domain core non reinitialization",
            "domain local checkpoint recovery", "task free context inference", "interference map invariants",
            "oracle headroom leakage", "plasticity action bounds",
        ],
