...
def pair(d1, d2, fn, default=None):
    """One value per domain direction. Keeps the terminal answers readable and one line each."""
    out = {}
    for name, rep in (("har->speech", d1), ("speech->har", d2)):
        if rep:
            try:
                out[name] = fn(rep)
            except Exception:
...
def answers():
    ...
    imap = (
        io.load("MOP_SUBSTRATE_INTERFERENCE_MAP.json")
        if io.exists("MOP_SUBSTRATE_INTERFERENCE_MAP.json")
        else {}
    )
    cross = (
        io.load("MOP_FAST_STATE_BIDIRECTIONAL_SYNTHESIS.json")
        if io.exists("MOP_FAST_STATE_BIDIRECTIONAL_SYNTHESIS.json")
        else {}
    )
    plast = (
        io.load("MOP_SUBSTRATE_PLASTICITY_POLICY_REPORT.json")
        if io.exists("MOP_SUBSTRATE_PLASTICITY_POLICY_REPORT.json")
        else {}
    )
    rounds = (
        io.load("MOP_FAST_STATE_ARCHITECTURE_COMPARISON.json")
        if io.exists("MOP_FAST_STATE_ARCHITECTURE_COMPARISON.json")
        else {}
    )
    reorg = (
        io.load("MOP_FUNCTIONAL_REORGANIZATION_REPORT.json")
        if io.exists("MOP_FUNCTIONAL_REORGANIZATION_REPORT.json")
        else {}
    )
    tfree = (
        io.load("MOP_TASK_FREE_CONTEXT_REPORT.json") if io.exists("MOP_TASK_FREE_CONTEXT_REPORT.json") else {}
    )
    third = (
        io.load("MOP_THIRD_TEMPORAL_DOMAIN_PREFLIGHT.json")
    ...
    ver = (
        io.load("MOP_FAST_STATE_INDEPENDENT_VERIFICATION.json")
        if io.exists("MOP_FAST_STATE_INDEPENDENT_VERIFICATION.json")
        else {}
    )
    mut = (
        io.load("MOP_FAST_STATE_MUTATION_REPORT.json")
        if io.exists("MOP_FAST_STATE_MUTATION_REPORT.json")
        else {}
    )
    clone = io.load("MOP_FAST_STATE_CLEAN_CLONE.json") if io.exists("MOP_FAST_STATE_CLEAN_CLONE.json") else {}
    fabric = (
        io.load("MOP_FAST_STATE_EVIDENCE_FABRIC.json")
        if io.exists("MOP_FAST_STATE_EVIDENCE_FABRIC.json")
        else {}
    )
    score = (
        io.load("MOP_FAST_STATE_PROGRESS_SCORECARD.json")
        if io.exists("MOP_FAST_STATE_PROGRESS_SCORECARD.json")
        else {}
    )
    within = {
        d: (
            io.load(f"MOP_{d.upper()}_WITHIN_DOMAIN_REPORT.json")
            if io.exists(f"MOP_{d.upper()}_WITHIN_DOMAIN_REPORT.json")
            else {}
        )
        for d in ("har", "speech")
    }
    ...
    q["7 did freezing the fast core preserve acquisition"] = (
        f"G1 frozen after the first domain: {pair(d1, d2, lambda r: r['utility']['G1_frozen_after_first'])} "
        f"against G0 always trainable: {pair(d1, d2, lambda r: r['utility']['G0_always_trainable'])}"
        if d1
        else MISSING
    )
    q["8 did reopening the fast core improve return recovery"] = (
        f"G2 return recovery effect versus the strongest baseline: "
        f"{pair(d1, d2, lambda r: r['effects']['G2_reopened_at_return']['return_recovery_vs_baseline'])}"
        if d1
        else MISSING
    )
    q["9 did adapter isolation reduce interference"] = (
        f"G4 adapters only, negative transfer: "
        f"{pair(d1, d2, lambda r: r['means']['G4_adapters_only']['negative_transfer'])}"
        if d1
        else MISSING
    )
    q["10 did anchored fast dynamics help"] = (
        f"H arms against the strongest matched baseline: "
        f"{pair(d1, d2, lambda r: r['effects']['H_always_update']['vs_strongest_baseline']['lower_95_cb'])}"
        if d1
        else MISSING
    )
    q["11 did gradient conflict gating help"] = (
        f"H cosine gate versus H always update: "
        f"{pair(d1, d2, lambda r: (r['utility']['H_cosine_gate'], r['utility']['H_always_update']))}"
        if d1
        else MISSING
    )
    q["12 did probe loss gating help"] = (
        f"H probe gate versus H always update: "
        f"{pair(d1, d2, lambda r: (r['utility']['H_probe_gate'], r['utility']['H_always_update']))}"
        if d1
        else MISSING
    )
    ...
    q["14 did oracle update partitioning show headroom"] = (
        str(
            {
                d: v["policy_headroom"]["oracle_advantage_over_best_fixed"]
                for d, v in plast.get("per_direction", {}).items()
            }
        )
        if plast
        else MISSING
    ...
    q["16 did a learned gate beat simple policies"] = (
        ...
        if plast.get("learned_gate_opened") is False
        else MISSING
    )
    ...
    q["19 did either beat separate per domain models"] = (
        ...
        f"{pair(d1, d2, lambda r: r['strongest_matched_baseline'])}, "
        f"and no substrate arm cleared SESOI against it"
        if d1 and not cross.get("arms_passing_in_both_directions")
        else MISSING
    )
    q["20 did either direction of domain transfer pass"] = str(cross.get("per_direction_verdicts", MISSING))
    ...
    q["22 did return recovery improve without acquisition harm"] = (
        ...
        if cross.get("bidirectional_verdict") == "cross_domain_null"
        else MISSING
    )
    q["23 did held out adaptation improve"] = (
        f"{pair(d1, d2, lambda r: best_lcb(r, ('G', 'H'), 'future_adaptation_vs_baseline'))}"
        if d1
        else MISSING
    )
    ...
    q["29 did the third temporal domain validate the premise"] = (
        f"{third.get('verdict', MISSING)}: {[(a['domain'], a['verdict']) for a in third.get('attempts', [])]}"
    )
    ...
    q["33 what strong baseline remained best"] = str(cross.get("per_direction_strongest_baseline", MISSING))
    ...
    q["35 what code was deleted"] = (
        str(code.get("deletions", MISSING)) + ". " + str(code.get("deletion_policy", ""))
    )
    q["36 did clean clone pass"] = (
        str(clone.get("all_pass", MISSING))
        + " with skips "
        + str(clone.get("environmental_skips", {}).get("note", ""))
    )
...
def main():
    ...
    io.seal(
        "MOP_FAST_STATE_NEXT_FRONTIER.json",
        {
            ...
            "open": [
                ...
                "whether the fast state advantage measured within a domain is a property of the core or of "
                "the "
            ...
            "not_reopened": [
                ...
                "adding seeds to the inherited A-T no slow near miss",
            ],
            "activation": False,
        },
    )
    ...
    print(
        json.dumps({k: v for k, v in a.items() if k.startswith(("20", "21", "30", "42"))}, indent=1)[:600],
        flush=True,
    )
