...
A mutation that changes nothing is accepted, and an accepted mutation invalidates the result it was applied to.
...
def decide(rows, seeds, arms, target, strongest, sesoi=io.SESOI, floor_margin=0.02, cost=0.05,
    ...
    return {"effect_lcb": eff["lower_95_cb"], "floors_ok": floors_ok,
...
def constructed_positive(rows, seeds, arms):
    ...
    strongest = max([b for b in BASELINES if b in arms], key=lambda a: float(np.mean([u(a, s) for s in seeds])))
...
def record_mutations(rows, seeds, arms, target, strongest):
    ...
    def flip(name, mutated, **kw):
        ...
        flips[name] = d
        res[name + "_rejected"] = d["passes"] != base["passes"] or abs(d["effect_lcb"] - base["effect_lcb"]) > 1e-9
    ...
    for s in seeds:  # domain order substitution: the other direction's numbers pasted in
        m[s][target]["metrics"], m[s][strongest]["metrics"] = (
            m[s][strongest]["metrics"], m[s][target]["metrics"])
    flip("domain_order_substitution", m)

    for name, key in (("adapter_reassignment", "adapter"), ("head_reassignment", "head"),
        ...
        res[name + "_rejected"] = any(
            ...
            and not any(g.startswith(f"{key}_wrong.") for g in rows[s][target]["receipts"][1]["trainable_groups"])
            for s in seeds)
    ...
    weakest = min([b for b in BASELINES if b in arms],
...
def live_mutations():
    ...
    res["core_reinitialization_rejected"] = (honest["shared_groups_changed_at_boundary"] == []
                                             and cheat["shared_groups_changed_at_boundary"] != [])
    ...
    more = S.run("G0_always_trainable", (a, b), 0,
                 steps={k: v * 3 for k, v in TINY.items()})
    res["update_inflation_rejected"] = (more["metrics"]["train_updates"]
                                        != small["metrics"]["train_updates"])
...
def main():
    ...
    live = live_mutations()
    io.seal("MOP_FAST_STATE_MUTATION_REPORT.json", {
        ...
        "all_rejected": all(v["all_rejected"] for v in out.values()) and all(live.values()) if out
    ...
    for d, v in out.items():
        print(d, "all_rejected", v["all_rejected"],
              [k for k, x in v["mutations"].items() if not x], flush=True)
