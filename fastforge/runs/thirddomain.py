...
def attempt(name, title, rights, design, differs, note):
    ...
    return {
        ...
        "windows_train": g["n_train"],
        "windows_test": g["n_test"],
        "units_train": g["n_units_train"],
        "units_test": g["n_units_test"],
...
def main():
    ...
    pam = attempt(
        "pamap2_transition",
        "PAMAP2 continuous transition prediction",
        ...
        [
            "continuous transitions retained",
            "label lies in the future so sequence state is required",
            "returning contexts available",
            "future subject adaptation available",
        ],
    ...
    har = attempt(
        "harth_transition",
        "HARTH free living transition prediction",
        ...
        [
            ...
            "label lies in the future",
            "22 subject units",
            "transition balanced sampling",
        ],
    ...
    stream = attempt(
        "speech_stream",
        "Speaker disjoint continuous audio, streaming keyword identity",
        "Speech Commands v0.02, public (CC BY 4.0), Google",
        "three keyword clips from one speaker are concatenated into a stream and the label is the keyword "
        "that ends the stream.",
        [
            "a different modality from the two accelerometry attempts",
            "the label depends on position in the stream, not on which keywords are present",
            "144 training speakers and 63 test speakers, speaker disjoint",
            "returning contexts, because keywords recur across streams",
        ],
        "attempted because both accelerometry style attempts failed the order gate in the same way: the set "
        "of values in a window carried the answer and their order did not. This attempt is designed so that "
        "an order free reader sees an identical frame set whichever keyword came last.",
    )
    attempts = [a for a in (pam, har, stream) if a]
    ...
    io.seal(
        "MOP_THIRD_TEMPORAL_DOMAIN_PREFLIGHT.json",
        {
            ...
            "priority_order_followed": [
                "HARTH continuous free living data",
            ...
            "status": "secondary. A third domain can support or fail to support the premise, it cannot "
            "carry a "
    ...
    for a in attempts:
        print(
            f"  {a['domain']}: {a['verdict']} gru {a['gate']['gru']} bag {a['gate']['bag_order_free']} "
            f"shuffled {a['gate']['gru_shuffled']}",
            flush=True,
        )
