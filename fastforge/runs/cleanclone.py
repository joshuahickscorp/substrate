...
def main():
    ...
    steps["substrate_tests"] = run([PY, "-m", "pytest", "tests/unit/test_fast_state_forge.py", "-q"], dest, env)
    steps["integrated_tests"] = run([PY, "-m", "pytest", "tests/unit", "-q"], dest, env)
    steps["evidence_index"] = run(
        [PY, "-c",
         "import json,hashlib,pathlib;"
         "f=json.load(open('proof/substrate/%s/MOP_FAST_STATE_EVIDENCE_FABRIC.json'));"
         "bad=[a['logical_id'] for a in f['artifacts'] "
         "if hashlib.sha256(pathlib.Path(a['original_path']).read_bytes()).hexdigest()!=a['content_hash']];"
         "print('bad',len(bad));assert not bad" % io.PROGRAM], dest, env)
    steps["checkpoint_recovery"] = run(
        [PY, "-c",
         "import json,hashlib,pathlib;"
         "f=json.load(open('proof/substrate/%s/MOP_FAST_STATE_EVIDENCE_FABRIC.json'));"
         "miss=[a['logical_id'] for a in f['artifacts'] "
         "if not pathlib.Path(a['canonical_path']).is_file()];print('missing',len(miss));assert not miss"
         % io.PROGRAM], dest, env)
    dirt = subprocess.run(["git", "status", "--porcelain"], cwd=dest, capture_output=True, text=True)
    steps["worktree_clean"] = {"cmd": "git status --porcelain",
    ...
    io.seal("MOP_FAST_STATE_CLEAN_CLONE.json", {
