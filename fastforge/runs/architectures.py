...
def inventory(kind):
    ...
    groups = {g: {"parameters": sorted(ns), "count": int(sum(named[n].numel() for n in ns))}
...
def main():
    ...
    io.seal("MOP_ARCHITECTURE_G.json", {
        "schema": "mop-architecture-g/v1",
        "name": "Fast Shared Core with Domain Local Plasticity",
        "premise": "a persistent fast temporal representation transfers across domains while domain local "
                   "adapters absorb domain specific slow change",
        "owned_state": {
            "domain specific trainable projection": "Projection, conv over channels plus a shape shared linear",
        ...
        "explicitly_absent": ["shared medium state", "shared trainable slow workspace",
                              "predictive auxiliary objective", "learned replay or retrieval"],
        ...
        "implementation": {"module": "fastforge/arch.py", "class": "ArchG",
                           "loc": len(inspect.getsource(A.ArchG).splitlines())},
    })
    io.seal("MOP_ARCHITECTURE_H.json", {
        "schema": "mop-architecture-h/v1",
        "name": "Anchored Fast Dynamics with Interference Gated Adaptation",
        "premise": "shared plasticity is safe when it is explicitly bounded and reversible, and is opened "
                   "only when the measured interference signal permits it",
        "materially_different_from_G": [
            "the shared core is a frozen anchor plus a bounded trainable delta, not a directly trainable core",
        ...
        "owned_state": {
            "shared fast dynamics core": "GRU parameters evaluated functionally as anchor plus bounded delta",
        ...
        "implementation": {"module": "fastforge/arch.py", "class": "ArchH",
                           "loc": len(inspect.getsource(A.ArchH).splitlines())},
    })
    for k in ("G", "H"):
        inv = inventory(k)
        print(k, "params", inv["total_parameters"], "partition_exact", inv["partition_is_exact"],
              "groups", len(inv["parameter_groups"]), flush=True)
