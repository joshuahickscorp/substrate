...
CANDIDATES = {
    ...
    "G-R3": dict(
        name="G_R3_split_core",
        policy=dict(
            arch="G_R3_split_core", carry=True, shared_p2=False, shared_p3=True, phase1_extra=["fast_core_ih"]
        ),
...
def causal_evidence():
    ...
    if rows:
        ...
        for k in ("second_acquisition", "first_retention", "return_recovery", "future_adaptation"):
            best_sub = max(
                [a for a in d0[seeds[0]] if a.startswith(("G", "H"))],
                key=lambda a: float(np.mean([d0[s][a]["metrics"][k] for s in seeds])),
            )
            comp[k] = {
                "best_substrate_arm": best_sub,
                "gap_to_baseline": round(
                    float(np.mean([d0[s][best_sub]["metrics"][k] for s in seeds]))
                    - float(np.mean([d0[s][base]["metrics"][k] for s in seeds])),
                    4,
                ),
            }
...
def main():
    if len(sys.argv) > 2 and sys.argv[1] == "shard":
        ...
        for family in ("G", "H"):
            for k in chosen[family]:
                ...
                print(
                    f"  round {k} {d} seed{s} acq2={out[c['name']]['metrics']['second_acquisition']:.3f}",
                    flush=True,
                )
        p = io.RUNS / "rounds"
    ...
    for dname in rounds:
        ...
        strongest = (
            io.load(f"MOP_{dname.split('->')[0].upper()}_TO_{dname.split('->')[1].upper()}_REPORT.json")[
                "strongest_matched_baseline"
            ]
            if io.exists(f"MOP_{dname.split('->')[0].upper()}_TO_{dname.split('->')[1].upper()}_REPORT.json")
            else "lstm_gdumb"
        )
        base_params = float(np.mean([base[dname][s]["lstm_gdumb"]["metrics"]["params"] for s in seeds]))

        def util(m, base_params=base_params):
            return float(
    ...
    both = sorted(
        a
        for a in passing
        if all(comparison[d][a]["vs_strongest_baseline"]["lower_95_cb"] >= io.SESOI for d in comparison)
    )
    io.seal(
