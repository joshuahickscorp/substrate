# EXECUTION_MANIFEST.md: the Mixture-of-Thinking operational plan

This is the single operational document for (A) the laptop scaffolding build, (B) the Studio append, and
(C) the laptop MAXIMIZATION RUN. Registry ids refer to docs/mixture_of_thinking/11_experiment_registry.md.
...
- PR1 mode-error disjointness: `scripts/pr1_mode_error_disjointness.py` writing
  ...
  touch its script or outputs. PR1 gates MT1/MT2/MT3 interpretation and the MT8/DR12 preconditions.
...
- NEW files (shared infrastructure, owned here so no other WP collides):
  - `src/devsys/substrate/adapter.py` (SubstrateAdapter ABC + RealEncoderAdapter, RandomInitViTAdapter,
    RandomPixelAdapter, honest resolution metadata; per 13_code_scaffolding.md, the one gated-NEW module)
  - `src/devsys/diagnostics/alignment.py` (thin aggregator over geometry + seed_consistency, permutation
    p-value; does NOT reimplement CKA)
  - `src/devsys/diagnostics/cross_substrate.py` (dict[substrate_tag -> latents] + labels in, per-substrate
    probe acc + CKA matrix + shuffled-label null + random-map-of-equal-rank floor out)
  - `src/devsys/diagnostics/riskcov.py` (AUROC, equal-mass ECE, risk-coverage curve and area, Pareto
    frontier area, seed-CI and sign-flip reporting; H-RISKCOV)
  - `src/devsys/diagnostics/continual_metrics.py` (factored BWT core lifted from
  ...
  - `src/devsys/shell/workspace.py` (pure composition of predictor/heads/ensemble/modulation behind
    cfg.shell flags; no new science)
  - `src/devsys/shell/capmatch.py` (matched-capacity constructor: solve hidden width for a target param
    count within 2%, fixed-total-params bandwidth-sweep variant; H-CAPMATCH)
  - `scripts/mot_aggregate_report.py` (reads every `runs/mot/*.json`, emits the verdict table with seed
    sign-flips, writes `runs/mot/aggregate_report.json`)
  - `tests/unit/test_mot_shared_modules.py` (tiny-tensor unit tests for the six new src modules, no
...
- EXISTING shared files this WP (and only this WP) extends:
  ...
  - `configs/encoder/*.yaml` + `src/devsys/substrate/encoder_registry.py`: add family and
    training_objective keys and surface them in list_encoders()
  - `src/devsys/shell/heads.py`: add kWTA and MoE gated heads (the e7 head family; blocks DR2/PR3, WS5,
    CM4 and the C1-C3 routing metrics until added)
  - `src/devsys/shell/__init__.py`: export the new heads and workspace
  - `src/devsys/diagnostics/compute.py`: attention-op and kNN-op FLOP counters (for MT7 scorer, WS slot
    attention, DR10/PR8 retrieval)
  - `src/devsys/diagnostics/__init__.py`: re-export alignment, cross_substrate, riskcov,
    continual_metrics
...
- Experiments: MT5 (adaptive halting), MT6 (confidence stopping), DR9 (verify-revise), DR8 (fixed point,
  V-JEPA arm now, typing arm reruns later on the random-init cache).
- NEW files: `scripts/mot_mt5_adaptive_halting.py`, `scripts/mot_mt6_confidence_stop.py`,
  `scripts/mot_dr9_verify_revise.py`, `scripts/mot_dr8_fixed_point.py`,
  `tests/integration/test_mot_halting.py`.
...
- Nulls wired: MT5 adaptive ties fixed depth at equal average FLOPs or halt collapses to constant N;
  MT6 trained halt ties the free update-norm rule; DR9 verify-revise ties single-shot and trained ties
  shuffled (the ex18 result); DR8 no geometric decay and past-horizon loss rises (the n9/y1 result).

### WP-04: search and debate (MT7, MT8)

- Experiments: MT7 (beam/tree search at matched TOTAL FLOPs, pruned work counted), MT8 (latent debate,
  gated on the PR1 verdict for decorrelated seeded modules).
- NEW files: `scripts/mot_mt7_beam_search.py`, `scripts/mot_mt8_latent_debate.py`,
  `tests/integration/test_mot_search_debate.py`.
- Controls: greedy chain at matched total FLOPs, random branch scorer, oracle beam upper bound; plain
  ensemble arm and self-debate arm for MT8, seed-permuted modules.
- Depends on: WP-02, WP-03 (Verifier harness); MT8 additionally reads the PR1 verdict json (do not
  rerun PR1). Queue class: heavy. Difficulty: 3.
- Nulls: MT7 search ties deeper greedy at matched total FLOPs; MT8 debate ties max(single, ensemble)
...
- NEW files: `scripts/mot_dr6_rollout_planning.py`, `scripts/mot_dr11_mc_rollouts.py`,
  `scripts/mot_dr13_horizon_limit.py`, `tests/integration/test_mot_rollout.py`.
...
- NEW files: `scripts/mot_dr10_retrieve_reason.py`, `scripts/mot_pr8_retrieval_head.py`,
  `scripts/mot_pr7_fast_slow.py`, `tests/integration/test_mot_memory.py`.
...
- Experiments: PR4 (epistemic gate vs noisy-TV), PR5 (content-gated critical period), PR6 (offline sleep
  consolidation).
- NEW files: `scripts/mot_pr4_epistemic_gate.py`, `scripts/mot_pr5_content_gated_cp.py`,
  `scripts/mot_pr6_sleep_consolidation.py`, `tests/integration/test_mot_plasticity.py`.
...
- Experiments: DR12 (disagreement-as-uncertainty), AL1 (uncertainty router with noisy-TV guard),
  MT1/MT2/MT3 (router vs best mode, vs uniform ensemble, hetero vs homogeneous; synthetic pilots today,
  real answer blocked on DR1).
- NEW files: `scripts/mot_dr12_disagreement.py`, `scripts/mot_al1_uncertainty_router.py`,
  `scripts/mot_mt123_router_pilots.py`, `tests/integration/test_mot_routing.py`.
...
- Depends on: WP-02; PR1 verdict on disk for gating; WP-02 heads for the e7 mode. Queue class: heavy.
  Difficulty: 3 (DR12, AL1), 4 (MT1-3 pilots, matched compute across three modes).
- Nulls: DR12 disagreement AUROC <= confidence AUROC or noisy-TV fail; AL1 router ties random episode
  selection or noisy-TV fail; MT1 routed density <= best single mode at matched compute; MT2 router <=
  uniform blend at matched total FLOPs; MT3 hetero <= homo k-copy MoE at matched params AND FLOPs. If
  PR1 reports NULL, MT1-3 still run but are REPORTED against the PR1 context (expected null,
  documented).
...
- NEW files: `scripts/mot_at4_programmatic_ceiling.py`, `scripts/mot_at5_probe_class_sweep.py`,
  `tests/integration/test_mot_atlas_guards.py`.
...
- NEW files: `scripts/mot_dr2_sparse_real_pilot.py`, `scripts/mot_ws5_slot_ablation_pilot.py`,
  `scripts/mot_cm4_workspace_pilot.py`, `tests/integration/test_mot_real_pilots.py`.
...
- NEW files: `scripts/mot_pr2_plasticity_substrates.py` (consumes the random-init-ViT feature cache;
  ...
  single-frame pass writing `data/cache/vjepa2_vitl_singleframe`), `scripts/mot_at3_time_axis.py`,
  `tests/integration/test_mot_substrate_passes.py`.
...
- NEW files: `scripts/mot_ws1_agreement_vs_confidence.py`, `scripts/mot_ws2_fusion_tournament.py`,
  `scripts/mot_ws3_arbitration.py`, `scripts/mot_ws4_bandwidth_sweep.py`,
  `tests/integration/test_mot_workspace.py`.
...
- Experiments: DR14 (VQ, 4-bit, additive-noise arms; the dropped-channel arm needs dense latents and is
  Studio).
- NEW files: `scripts/mot_dr14_corruption.py`, `tests/integration/test_mot_corruption.py`.
...
- NEW files: `scripts/mot_at1_grid_pilot.py`, `scripts/mot_al2_alignment_pilot.py`,
  `tests/integration/test_mot_atlas_grid.py`.
...
homogeneous + seed SD) licenses the MT routing rows as live tests; PR1 NULL demotes MT1/MT2/MT3/MT8/DR12
...
- Q1.1 AT5 probe-class sweep. cmd `python scripts/mot_at5_probe_class_sweep.py --seeds 0-4`
  5 seeds, class light, ~30 min, out `runs/mot/at5_probe_class_sweep.json`
- Q1.2 AT4 programmatic ceiling. cmd `python scripts/mot_at4_programmatic_ceiling.py --seeds 0-4`
  5 seeds, class light, ~45 min, out `runs/mot/at4_programmatic_ceiling.json`
- Q1.3 MT5 adaptive halting. cmd `python scripts/mot_mt5_adaptive_halting.py --seeds 0-4`
  5 seeds, class heavy, ~60 min, out `runs/mot/mt5_adaptive_halting.json`
- Q1.4 MT6 confidence stopping. cmd `python scripts/mot_mt6_confidence_stop.py --seeds 0-4`
  5 seeds, class heavy, ~30 min, out `runs/mot/mt6_confidence_stop.json`
- Q1.5 DR9 verify-revise. cmd `python scripts/mot_dr9_verify_revise.py --seeds 0-4`
  5 seeds, class heavy, ~45 min, out `runs/mot/dr9_verify_revise.json`
- Q1.6 DR8 fixed point, V-JEPA arm. cmd `python scripts/mot_dr8_fixed_point.py --cache vjepa --seeds 0-4`
  5 seeds, class heavy, ~30 min, out `runs/mot/dr8_fixed_point_vjepa.json`
- Q1.7 MT7 beam search. cmd `python scripts/mot_mt7_beam_search.py --seeds 0-4`
  5 seeds, class heavy, ~60 min, out `runs/mot/mt7_beam_search.json`
- Q1.8 DR6 rollout planning. cmd `python scripts/mot_dr6_rollout_planning.py --seeds 0-4`
  5 seeds, class heavy, ~45 min, out `runs/mot/dr6_rollout_planning.json`
- Q1.9 DR13 horizon limit. cmd `python scripts/mot_dr13_horizon_limit.py --seeds 0-4`
  5 seeds, class heavy, ~45 min, out `runs/mot/dr13_horizon_limit.json`
- Q1.10 DR11 MC rollouts (noisy-TV guarded). cmd `python scripts/mot_dr11_mc_rollouts.py --seeds 0-4`
  5 seeds, class heavy, ~45 min, out `runs/mot/dr11_mc_rollouts.json`
- Q1.11 DR10 retrieve-then-reason. cmd `python scripts/mot_dr10_retrieve_reason.py --seeds 0-4`
  5 seeds, class heavy, ~30 min, out `runs/mot/dr10_retrieve_reason.json`
- Q1.12 PR8 retrieval head. cmd `python scripts/mot_pr8_retrieval_head.py --seeds 0-4`
  5 seeds, class heavy, ~30 min, out `runs/mot/pr8_retrieval_head.json`
- Q1.13 PR7 fast/slow weights. cmd `python scripts/mot_pr7_fast_slow.py --seeds 0-4`
  5 seeds, class heavy, ~45 min, out `runs/mot/pr7_fast_slow.json`
- Q1.14 PR4 epistemic gate vs noisy-TV. cmd `python scripts/mot_pr4_epistemic_gate.py --seeds 0-4`
  5 seeds, class heavy, ~45 min, out `runs/mot/pr4_epistemic_gate.json`
- Q1.15 PR5 content-gated critical period. cmd `python scripts/mot_pr5_content_gated_cp.py --seeds 0-4`
  5 seeds, class heavy, ~45 min, out `runs/mot/pr5_content_gated_cp.json`
- Q1.16 PR6 sleep consolidation. cmd `python scripts/mot_pr6_sleep_consolidation.py --seeds 0-4`
  5 seeds, class heavy, ~45 min, out `runs/mot/pr6_sleep_consolidation.json`
- Q1.17 AL1 uncertainty router. cmd `python scripts/mot_al1_uncertainty_router.py --seeds 0-4`
  5 seeds, class heavy, ~45 min, out `runs/mot/al1_uncertainty_router.json`
- Q1.18 DR12 disagreement (PR1 context). cmd `python scripts/mot_dr12_disagreement.py --seeds 0-4`
  5 seeds, class heavy, ~45 min, out `runs/mot/dr12_disagreement.json`
- Q1.19 MT8 latent debate (PR1 context). cmd `python scripts/mot_mt8_latent_debate.py --seeds 0-4`
  5 seeds, class heavy, ~60 min, out `runs/mot/mt8_latent_debate.json`
- Q1.20 MT1/MT2/MT3 router pilots (PR1 context, synthetic; real answer stays blocked on DR1).
  cmd `python scripts/mot_mt123_router_pilots.py --seeds 0-4`
...
- Q1.21 DR14 corruption arms (VQ/4-bit/noise; runs after Q1.3-Q1.8 so it can test the survivors).
  cmd `python scripts/mot_dr14_corruption.py --seeds 0-2`
  3 seeds, class heavy, ~60 min, out `runs/mot/dr14_corruption.json`
- Q1.22 DR2/PR3 sparse-real pilot (528K cache). cmd `python scripts/mot_dr2_sparse_real_pilot.py --seeds 0-4`
  5 seeds, class heavy, ~60 min, out `runs/mot/dr2_sparse_real_pilot.json`
- Q1.23 WS5 slot-ablation pilot. cmd `python scripts/mot_ws5_slot_ablation_pilot.py --seeds 0-4`
  5 seeds, class heavy, ~45 min, out `runs/mot/ws5_slot_ablation_pilot.json`
- Q1.24 CM4 workspace pilot. cmd `python scripts/mot_cm4_workspace_pilot.py --seeds 0-2`
...
- Q3.1 PR2 plasticity real vs random-init-ViT (the decisive fork row, 14.6 #1).
  cmd `python scripts/mot_pr2_plasticity_substrates.py --seeds 0-4`
  5 seeds, class heavy, ~90 min, out `runs/mot/pr2_plasticity_substrates.json`
- Q3.2 AT3 time-axis ablation. cmd `python scripts/mot_at3_time_axis.py --seeds 0-4`
  5 seeds, class heavy, ~30 min, out `runs/mot/at3_time_axis.json`
- Q3.3 DR8 typing arm on random-init cache.
  cmd `python scripts/mot_dr8_fixed_point.py --cache randominit_vitl --seeds 0-4`
...
- Q3.4 WS1 agreement vs confidence (central WS gate; all five guards preregistered).
  cmd `python scripts/mot_ws1_agreement_vs_confidence.py --seeds 0-4`
  5 seeds, class heavy, ~60 min, out `runs/mot/ws1_agreement_vs_confidence.json`
- Q3.5 WS2 fusion tournament. cmd `python scripts/mot_ws2_fusion_tournament.py --seeds 0-4`
  5 seeds, class heavy, ~90 min, out `runs/mot/ws2_fusion_tournament.json`
- Q3.6 WS4 bandwidth sweep. cmd `python scripts/mot_ws4_bandwidth_sweep.py --seeds 0-4`
...
- Q3.7 WS3 arbitration, ONLY if WS1 positive (else logged SKIPPED with the WS1 verdict cited).
  cmd `python scripts/mot_ws3_arbitration.py --seeds 0-4`
...
- Q4.3 MT1-3 rerun against the FINAL PR1 verdict with the tuned single-mode baselines re-tuned (the
  tuned-baseline doctrine). cmd `python scripts/mot_mt123_router_pilots.py --seeds 0-9 --tuned`
...
- Q4.5 aggregate verdict report (sign-flips, seed CIs, gate table).
  cmd `python scripts/mot_aggregate_report.py`
...
set. If PR1 nulls, MT1-3/MT8/DR12 are still run and reported against the PR1 context.
...
```markdown
## MoT lane (mixture of thinking): what it is and how to resume

A second execution lane exists: docs/mixture_of_thinking/EXECUTION_MANIFEST.md. It is the single
operational plan for the MoT experiment bank (MT/DR/PR/WS/AT/AL/CM registry in
docs/mixture_of_thinking/11_experiment_registry.md). The laptop ran a maximization queue into
...
1. Read docs/mixture_of_thinking/EXECUTION_MANIFEST.md sections 1 and 4. Respect the live-state rules:
...
2. Check runs/mot/ against the Stage 0-4 queue in the manifest. Any missing json is an unrun row; rerun
   ...
   routing rows.
3. On the Studio, execute the appended "Mixture-of-Thinking lane" section of STUDIO_HANDOFF.md in table
...
- Routing rows (MT1-3, MT8, DR12): a win requires delta > 5-seed spread at matched compute AND no sign
  flip; PR1-null context demotes any win to PLAUSIBLE-BUT-UNVERIFIED pending the Studio regime.
- Halting rows (MT5, MT6): matched on MEAN FLOPs within tol 0.10 (matched_within); halting entropy ~0
  is an automatic null (constant halt head).
- Search rows (MT7, DR6, DR11, DR13): matched on TOTAL FLOPs including pruned/discarded work.
