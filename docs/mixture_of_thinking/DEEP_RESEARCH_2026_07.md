...
- data/cache/dinov2s_nuisance_real does NOT exist. WS1 (scripts/mot_ws1_agreement_vs_confidence.py)
...
1. LEARNED ROUTER, TUNED MODES (cpu-now). Does a tiny learned router realize a nonzero fraction
   of the oracle gain once each mode is individually TUNED (removing MT2's untuned-blend
   ...
   gain is capacity not routing and it re-derives MT1-NULL. Null: learned-minus-tuned-best CI
...
## 5. The name: "Mixture of Thinking" collides with prior art
...
Honest position: we are NOT first to "mixture of thinking." Rename the program and frame the
