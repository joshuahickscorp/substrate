...
### MixtureArbitrator
- Status: PARTIAL. The mixture-of-experts / router mechanism exists inside e7_sparse (tiny MoE with softmax router, k-WTA head) and ex9_slot_attention; there is no standalone `MixtureArbitrator` that arbitrates among heads/experts/substrates as a reusable object. Given the section title (mixture of thinking) this is the conceptual center, but the code reality is that the only SURVIVING mixture positive (e7_sparse) is a head-architecture fact, reframed as NOT substrate-carrying-special-structure.
