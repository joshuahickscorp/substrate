# Mixture of Thinking: Master Index

This is the document to open first. It is the decision-first synthesis of the whole Mixture-of-Thinking (MoT)
...
| Adaptive halting (ACT-lite) | accuracy-at-matched-mean-FLOPs beats fixed-step | halt allocates by noise not difficulty | open (MT5 the honest framing) |
...
|-------------------|--------------|-----------------|----------------------|
| MT1 adaptive compute | beats fixed depth at matched AVERAGE FLOPs (allocation) | ties fixed depth; halt collapses to constant | weak (per-sample hardness) |
| MT2 confidence stopping | trained halt beats free update-norm rule at matched FLOPs | confidence == update-norm | no |
| MT3 beam/tree search | K scored trajectories + pruning beat greedy at matched total FLOPs | search == unrolled depth | no |
| MT4 latent debate | two seeded modules + referee beat single AND ensemble at matched FLOPs | debate == unrolled ensemble | no |
| MT5 mixture router | learned router beats single best strategy at matched mean FLOPs | reduces to bought compute (p9/ex17) | indirect (shell-lever) |
...
|--------|---------|-------|------------------------------|
| MT | Mixture of Thinking routing and adaptive compute | 8 | MT1 router-vs-best-mode (core thesis), MT2 routing-vs-ensemble, MT3 hetero-vs-homo |
...
- MT1 / MT2 / MT3 pilots on the existing synthetic harness: router-vs-best-mode, routing-vs-ensemble,
  heterogeneous-vs-homogeneous. Synthetic answers only, but they exercise the router and expose obvious nulls.
- MT5 adaptive halting versus fixed depth at matched AVERAGE FLOPs (the one honest iteration-beats-depth
  framing), MT6 confidence stopping, DR6 rollout planning (ex2 extension), DR9 memory-first retrieval, DR12
...
Full module-by-module audit against the actual src/devsys tree in [13 code scaffolding](13_code_scaffolding.md).
...
2. MT5 adaptive halting vs fixed depth at matched AVERAGE FLOPs (the one honest iteration-beats-depth framing).
3. MT1/MT2/MT3 router pilots on the synthetic harness (expose obvious nulls before studio).
...
3. MT1 + H4: a heterogeneous router beats the best single mode AND a homogeneous k-copy MoE at matched compute.
...
## 16. Falsifiable claims that would MAKE Mixture of Thinking real
...
6. MT5: per-sample adaptive halting beats fixed depth at equal AVERAGE FLOPs by allocating depth to hard inputs (iteration beats depth via allocation, the one honest framing).
...
## 17. Falsifiable claims that would KILL Mixture of Thinking
...
5. **11_experiment_registry.md, MT1 (broken intra-registry cross-reference).** Changed both "blocked on DR9"
   occurrences in the MT1 Data/task and Dependency-risk cells to "blocked on DR1" (DR1 is the real
