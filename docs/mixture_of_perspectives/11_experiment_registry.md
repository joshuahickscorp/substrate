...
- **MT** (Mixture of Perspectives): routing/arbitration among heterogeneous thinking modes, adaptive
...
## MT: Mixture-of-Perspectives routing and adaptive compute
...
| MP8 | Latent debate beats single module | Two seeded modules critiquing via a referee beat one module AND a plain ensemble at matched total FLOPs | cpu-now | med | med | No |

### MT full schema

| Field | MP1 Router-vs-best | MP2 Routing-vs-ensemble | MP3 Hetero-vs-homo | MP4 Reasoning-router (synthesis) | MP5 Adaptive-halt-vs-depth | MP6 Confidence-stop | MP7 Beam/tree search | MP8 Latent debate |
|-------|---|---|---|---|---|---|---|---|
...
| **Substrates** | Cached V-JEPA 2 ViT-L pooled latents | Same as MP1 | Same as MP1 | Same shell primitives on cached latents | Cached latents, difficulty-graded regime | Cached latents | Cached latents, ambiguous-target task | Cached latents, multiple seeded shells, one frozen encoder |
...
| **Data/task** | Synthetic continual-stream pilot; real bound-attribute cache for the real answer (blocked on DR1) | Same set as MP1 | Same set | Difficulty-graded reasoning regime (D3-certified) | Difficulty-graded regime | Same | Multi-modal/ambiguous-target regime | Multi-answer regime with correctable errors |
...
| ID | Name | Thesis tested (one line) | Tier | Diff | Sci value | Custom-model decision |
|----|------|--------------------------|------|------|-----------|-----------------------|
| PR1 | Mode-error disjointness (router precondition) | Per-sample errors across modes are decorrelated enough that an oracle router beats the single best mode | cpu-now | low | high (cheap gate for MP1/MP4) | No (but gates the cheap-vs-studio decision) |
...
| CM10 | Action-conditioned forward model | A trainable action-conditioned model enables planning beating reactive, action-shuffle, unrolled depth, and an exact-referent external control | local mechanics; external data/control evidence | high | med | Weak: P7 rendering and eight-arm mechanics pass locally, but the fixture loses to reactive and still lacks external trajectories and replication |
...
| **Controls** | random-init same-arch, D3, matched capacity/data/resolution | random-init same-arch, best frozen substrate, matched compute | dense-tokens arm, off-the-shelf slot model, swap test | action-shuffle, matched compute, unrolled depth, exact-referent external action control | flat-schedule baseline, matched compute, noisy-TV-guarded ordering, seed sweep | best single expert+workspace, single pilot, strongest open model, matched total FLOPs |
| **Matched baseline** | Random-init same-arch encoder | Best frozen atlas substrate | Dense-without-slots | Reactive + unrolled depth + exact-referent external action control | Flat-schedule substrate | Best single expert + workspace |
...
|-------------|-------------------------|-----------|
| MP1 | MT1_routed_vs_best_mode (sec01) | Sole source; the routed-vs-best triad head |
| MP4 | MP5 router (sec04) + DR-ROUTER (sec03) + "Mixture-of-thinking router" (sec04) | All three are the same learned-router-over-reasoning-primitives synthesis test; merged into one central row |
| MP5 | "Adaptive latent compute beats fixed depth" (sec04, sec04-local MP1) | Adaptive-halt-vs-depth |
| MP6 | "Confidence stopping vs fixed early-exit" (sec04-local MP2) | |
| MP7 | "Beam/tree search over latent states" (sec04-local MP3) | |
| MP8 | "Latent debate between modules" (sec04-local MP4) | |
