# Section 3: The Mixture-of-Perspectives Taxonomy of Thinking Modes
...
`src/mop/shell/` and `src/mop/diagnostics/`, or to a small module a competent afternoon could
add. The point of a Mixture-of-Perspectives (MoT) framing is not novelty for its own sake: it is that the
...
These are wired at the harness level (`src/mop/diagnostics/`) and apply to every claim in this
...
- **Seed-stability** (`diagnostics/seed_consistency.py`); sign-flips are published as instability. Sweep
  seeds via `mop.harness.sweep.run_sweep` (cfg.seed override), NOT `experiment.seeds` (silent no-op).
