...
reasoning" id or an MT "mixture of perspectives" id) with a null, a baseline it must beat, metrics,
...
`src/mop/shell/refine.py` is, by its own docstring, "exactly a residual network unrolled in
place", and the compute-matched control in `src/mop/diagnostics/compute.py`
...
1. **Input-dependent compute allocation** (adaptive halting, confidence stopping): a variable-
   ...
   it. MP1 and MP2 below make it the central test.
...
2. **Search / non-monotone exploration** (tree, beam, MC rollout, debate): a feedforward net
   ...
   falsifiable. MP3, MP4, DR6, DR7 test them.
...
The MT ("mixture of perspectives") ids test whether ALLOCATING different amounts or kinds of
...
### MP1 — Adaptive latent compute beats fixed depth at matched AVERAGE FLOPs
...
### MP2 — Confidence stopping vs a fixed early-exit schedule (is the confidence signal real)
...
### MP3 — Beam/tree search over latent states beats greedy single-trajectory refinement
...
### MP4 — Latent debate between modules reduces error vs a single module at matched compute
...
### MP5 — Mixture-of-thinking router: choose a strategy per input beats the single best strategy
...
- **Mechanism.** A tiny router reads the raw latent, picks one strategy (or a soft mixture), pays
  that strategy's compute. This is the literal "mixture of perspectives" thesis.
...
- **Required substrate.** cached latents; depends on MP1-MP3 having produced distinct strategies.
...
- **Baseline it must beat.** single-shot at matched compute; AND a SHUFFLED verifier (scores
  permuted across batch); AND update-norm confidence (free signal, from MP2).
...
- **Core question.** Any reasoning gain found (MP1-5, DR1-4) must be tested for whether it is a
...
- **Core question.** Does the disagreement among an ensemble of shell modules (or debate rounds in
  MP4) predict per-sample error better than a single module's own confidence, and does using it to
...
| Adaptive latent compute | MP1 |
| Latent scratchpad | DR3 |
| Latent chain-of-thought without text | DR2 |
| Latent self-verification | DR7 |
| Latent debate between modules | MP4 |
| Tree/beam search over latent states | MP3 |
...
(DR10), planning RETURN on true dynamics (DR4/DR11), and matched-compute Pareto frontiers (MP1),
