...
src/mop/studio/profiles.py (the shipped m3pro-local-max and studio-1tb profiles).
...
- The two surviving positives at the scale they were found: e7_sparse (sparse/gated heads halve
  ...
  mop.harness.sweep.run_sweep) and ex2_latent_planning's synthetic-control precursor (the
...
Tier-0 job): the ONLY shipped laptop profile, M3PRO_LOCAL_MAX in src/mop/studio/profiles.py,
...
src/mop/devices (resolve/autocast/safe_to), configs/device/mps.yaml and cpu.yaml,
...
with per-worker BLAS caps to avoid oversubscription), mop.harness.sweep.run_sweep (real per-run
