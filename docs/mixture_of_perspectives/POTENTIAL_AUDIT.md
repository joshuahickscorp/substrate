...
(`mop/diagnostics/compute.py` with unit tests). The one experiment that computes the true thesis ratio,
...
homogeneous k-copy bank on the density metric. The only positive, MP2 (+0.062), is accuracy-vs-uniform-blend at
...
level, and that test came back NULL while its sibling MP3 came back a clean LOSS.
