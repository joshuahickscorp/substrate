...
wired to an EXISTING diagnostic in `src/mop/diagnostics/` rather than
...
- **False-positive prevention.** Hold the replay budget IDENTICAL across arms
  ...
  in the ground truth). Sweep seeds via `mop.harness.sweep.run_sweep` with
...
5. **Seed-stability.** Sweep via `mop.harness.sweep.run_sweep` with per-run
