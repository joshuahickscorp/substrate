...
| id | name | null hypothesis | exp_tier | status | tax |
|---|---|---|---|---|---|
| mot_al1_uncertainty_router | uncertainty_router_with_noisy_tv_guard | the uncertainty router matches random episode selection on adaptation-per-update, OR it chases irreducible aleatoric noise (fails the noisy-TV guard), replicating the e4 negative in router clothing | cpu-now | registry-only | 8 |
| mot_al2_shared_latent_alignment | shared_latent_alignment_vs_random_map_floor | a random map of equal rank predicts the target as well as the learned map (alignment-artifact): any apparent cross-substrate agreement is the map's capacity, not shared structure | gpu-later | registry-only | 2 |
| mot_al3_audio_video_alignment | audio_video_temporal_alignment | audio-SSL and V-JEPA event boundaries co-locate no better than under a shuffled-time permutation: temporal-predictive structure is modality-specific, not universal | gpu-later | registry-only | 3 |

## AT

| id | name | null hypothesis | exp_tier | status | tax |
|---|---|---|---|---|---|
| mot_at1_nuisance_grid | cross_substrate_nuisance_grid | every substrate's decodability delta over its matched-resolution random-init control is within seed spread (random-control-artifact): pretraining bought no nuisance invariance beyond architecture and resolution in any substrate | gpu-later | registry-only | 3 |
| mot_at2_mode_substrate_dep | mode_substrate_dependence | every mode's advantage is reproduced by random-init-ViT (or random-encoder), so no mode depends on the pretrained substrate rather than architecture/resolution or generic separability | gpu-later | registry-only | 3 |
| mot_at3_time_axis | time_axis_ablation | with token/frame count equalized, full-clip and single-frame decodability are within seed spread for every factor: no factor genuinely requires the temporal axis | cpu-now | registry-only | 3 |
| mot_at4_programmatic_ceiling | programmatic_ceiling_reference | the programmatic reference does not exceed the perceptual substrates on any atlas factor (trivially separable everywhere), so no perceptual tie can be attributed to a substrate bound | cpu-now | registry-only | 5 |
| mot_at5_probe_class_sweep | probe_class_sweep | every cell's verdict is invariant to probe class, so probe choice never changes decodability and probe-specific is an empty verdict | cpu-now | registry-only | 4 |

## CM

| id | name | null hypothesis | exp_tier | status | tax |
|---|---|---|---|---|---|
| mot_cm10_action_forward_model | action_conditioned_forward_model | action-shuffle ties the true-action forward model on planning success: the model ignores actions and is a plain video predictor | env-later | registry-only | 7 |
| mot_cm11_developmental_plasticity | developmental_scheduled_plasticity | a non-developmental substrate trained on identical data with a flat schedule ties the developmental one on every plasticity signature (window, path-dependence, U-shape): the development is cosmetic | gpu-later | registry-only | 1 |
| mot_cm12_mot_substrate_capstone | mixture_of_thinking_substrate_capstone | a single best-expert-plus-workspace ties the full mixture at matched total FLOPs: the mixture is pure overhead and should be descoped to the single expert | gpu-later | registry-only | 2 |
| mot_cm1_compositional_gate | compositional_bound_on_real_video_c1_gate | held-out-combination accuracy for real V-JEPA ties random-init same-arch ViT (delta < +0.05) on a D3-certified separable regime: the substrate memorizes conjunctions rather than factoring them | gpu-later | registry-only | 3 |
| mot_cm2_atlas_gate | multi_substrate_atlas_gate | all frozen substrates tie their own random-init controls on the failing gate (no substrate exceeds +0.15 off-ceiling where V-JEPA-L did not): no substrate swap helps | gpu-later | registry-only | 3 |
| mot_cm3_dense_vs_pooled | dense_vs_pooled_compositional | dense and coarse-grid tie full mean-pool on held-out combos (delta < +0.1 off-ceiling on a D3 regime): the pooling interface is not the bound (single-factor pooling already refuted, orientation decodes at 1.0 pooled) | gpu-later | registry-only | 3 |
| mot_cm4_workspace_shell | workspace_shell_routing | the workspace shell ties param-matched dense OR ties matched-compute unrolled depth on BWT and planning, or sign-flips across seeds: the win is capacity/depth, not routing | gpu-later | registry-only | 2 |
| mot_cm5_studio_rejuvenation | studio_scale_rejuvenation_c3 | plasticity loss never appears at reachable scale, OR rejuvenation ties doing-nothing, OR frozen-substrate restoration ties the adapting-substrate arm (no C3 signal) | gpu-later | registry-only | 1 |
| mot_cm6_distilled_density | compressed_capability_density | a same-size random-init substrate OR a same-size student trained without the invariance-matching loss ties the distilled student on the nuisance-invariance delta: compression bought no capability density | gpu-later | registry-only | 2 |
| mot_cm7_min_objective_probe | minimum_objective_encoder_probe | at matched tiny capacity, matched data, matched 256px, both custom objectives tie random-init same-arch AND tie each other; objective is not a lever at this scale and the +0.31 was scale/data/architecture/resolution. A tie is a strong negative closing the custom-encoder line | gpu-later | registry-only | 3 |
| mot_cm8_custom_jepa_pilot | custom_jepa_factoring_pilot | the custom-JEPA pilot ties both a random-init same-arch ViT and the best frozen atlas substrate on held-out combinations at matched compute: the custom objective bought nothing | gpu-later | registry-only | 3 |
| mot_cm9_slot_jepa_binding | object_centric_slot_jepa_binding | dense tokens without slots tie the slotted model on multi-object held-out combinations, OR the binding-specificity swap shows non-target slots change too: slots add nothing over dense | gpu-later | registry-only | 2 |

## DR

| id | name | null hypothesis | exp_tier | status | tax |
|---|---|---|---|---|---|
| mot_dr10_retrieve_reason | memory_first_retrieve_then_reason | retrieval ties from-scratch OR ties random retrieval: the frozen neighbor metric is not task-aligned | cpu-now | registry-only | 3 |
| mot_dr11_mc_rollouts | monte_carlo_latent_rollouts | MC ties the matched single longer rollout (samples collapse to the mean) OR wins only by exploiting irreducible aleatoric noise (fails noisy-TV) | cpu-now | registry-only | 8 |
| mot_dr12_disagreement | disagreement_as_uncertainty | disagreement ties single-head confidence AND gating on it ties uniform compute, OR it chases irreducible noise (fails noisy-TV, the e4 pattern) | cpu-now | registry-only | 8 |
| mot_dr13_horizon_limit | planning_horizon_limit | planning never beats reactive at any horizon (contradicts ex2) OR beats it only at horizon 1 (not really planning) | cpu-now | registry-only | 9 |
| mot_dr14_corruption | reasoning_under_corruption_compression | reasoning and single-pass degrade at the same rate under every corruption: iteration only processes what survives | gpu-later | registry-only | 3 |
| mot_dr15_modality_general | modality_general_reasoning | the gain is identical across all three modalities (a shell/task property) OR present only on the most separable modality (an artifact) | gpu-later | registry-only | 2 |
| mot_dr1_video_cache | real_bound_attribute_video_cache | even on real video the pooled and dense latents ceiling at 1.0 on held-out-combination decode (D3 says still trivially separable), so no compositional mode scores above the random-encoder floor | gpu-later | registry-only | 5 |
| mot_dr2_sparse_real | sparse_head_forgetting_on_real_latents | on real latents with a D3 certificate and a paired significance test, sparse ties param-matched dense (and ties a dense head with matched activation-sparsity penalty); the synthetic win was cluster separability | gpu-later | registry-only | 2 |
| mot_dr3_latent_scratchpad | latent_scratchpad_vs_residual_stream | scratchpad ties residual-only at all memory loads: the pooled frozen latent already discarded the detail external memory would store (a substrate bound) | gpu-later | registry-only | 3 |
| mot_dr4_causal_intervention | counterfactual_causal_latent_intervention | intervened rollouts do not beat the correlational baseline and single-factor interventions leak into other factors: the frozen latent entangles factors | gpu-later | registry-only | 10 |
| mot_dr5_cross_substrate_consistency | cross_substrate_reasoning_consistency | the reasoning gain is identical across all substrates (a shell/task property, not substrate-carried) OR vanishes on a second real encoder (fragile) | gpu-later | registry-only | 2 |
| mot_dr6_rollout_planning | internal_simulation_rollout_planning | planning ties reactive at matched compute OR ties action-shuffle: rollouts carry no usable future info beyond one step | cpu-now | registry-only | 9 |
| mot_dr7_latent_cot | latent_chain_of_thought | the latent chain ties one-shot at matched compute and shuffling does not hurt: the chain is a bag of extra layers (ex17 generalized) | gpu-later | registry-only | 9 |
| mot_dr8_fixed_point | recurrent_refinement_fixed_point_vs_drift | update norms do not decay geometrically and unrolling past N worsens loss: no fixed point, it is unrolled depth (the n9/y1 result) | cpu-now | registry-only | 9 |
| mot_dr9_verify_revise | verify_revise_under_corrected_controls | verify-revise ties single-shot at matched compute and the trained verifier ties the shuffled one: no correction-relevant signal (the ex18 result) | cpu-now | registry-only | 4 |

## MT

| id | name | null hypothesis | exp_tier | status | tax |
|---|---|---|---|---|---|
| mot_mt1_router_vs_best | router_beats_single_best_mode | routed density equals or trails its best constituent mode at matched compute, or the gap sits inside the seed spread | cpu-now | registry-only | 2 |
| mot_mt2_routing_vs_ensemble | routing_beats_uniform_ensemble | equal-weight blend matches the routed system's density at matched total compute; the router adds nothing over averaging | cpu-now | registry-only | 2 |
| mot_mt3_hetero_vs_homo | heterogeneous_beats_homogeneous_moe | homogeneous k-copy MoE matches the heterogeneous mixture at equal param/FLOP; diversity adds nothing beyond generic mixture capacity | cpu-now | registry-only | 2 |
| mot_mt4_reasoning_router | router_over_reasoning_primitives | at matched mean FLOPs the router ties or loses to the single best fixed mode; any win is bought compute or vanishes under seed sweep | gpu-later | registry-only | 2 |
| mot_mt5_adaptive_halting | adaptive_halting_vs_fixed_depth | at equal average FLOPs, adaptive halting ties fixed-depth: no exploitable hardness heterogeneity, or the halt head collapses to a constant | cpu-now | registry-only | 9 |
| mot_mt6_confidence_stop | confidence_stopping_vs_free_early_exit | the trained halt head ties the free update-norm rule; confidence is just the latent ceasing to move | cpu-now | registry-only | 9 |
| mot_mt7_beam_search | beam_tree_search_vs_greedy_refinement | at matched total FLOPs, search ties a deeper greedy chain because the scorer cannot outrank the refiner's own step | cpu-now | registry-only | 9 |
| mot_mt8_latent_debate | latent_debate_vs_single_module | debate ties the single matched module AND ties a plain ensemble; structured exchange is an unrolled ensemble/MoE | cpu-now | registry-only | 2 |

## PR

| id | name | null hypothesis | exp_tier | status | tax |
|---|---|---|---|---|---|
| mot_pr1_error_disjointness | mode_error_disjointness_router_precondition | per-sample errors are highly correlated (oracle-router gain over single-best is within seed spread), so no router can help and MT1/MT4 are not worth building | cpu-now | registry-only | 2 |
| mot_pr2_plasticity_substrates | plasticity_advantage_real_vs_random_encoder | shell adaptation speed and BWT on real V-JEPA are within the seed spread of the same shell on random-init-ViT (matched resolution): the substrate's structure does not ease trained-shell learning dynamics | cpu-now | registry-only | 3 |
| mot_pr3_modular_real | modular_plasticity_on_real_latents | on real latents with a D3 certificate and a paired significance test, sparse ties param-matched dense (and a matched activation-sparsity dense head): synthetic advantage was cluster separability | cpu-now | registry-only | 2 |
| mot_pr4_epistemic_gate | epistemic_gate_vs_noisy_tv | on the noisy-TV split the disagreement gate allocates no more LR-integral (or query budget) to the reducible partition than an ungated arm: replicates the e4 epistemic/aleatoric conflation | cpu-now | registry-only | 8 |
| mot_pr5_content_gated_cp | content_gated_critical_period | at matched LR-integral and shuffled task order, content-gated shows no retention or reopening advantage over cosine decay / tuned constant LR: pure LR annealing (the e3/d6 negative) | cpu-now | registry-only | 2 |
| mot_pr6_sleep_consolidation | offline_sleep_consolidation | offline replay-only ties online replay at matched total gradient steps: any sleep benefit is added compute, not phase separation | cpu-now | registry-only | 2 |
| mot_pr7_fast_slow | fast_slow_two_timescale_weights | fast weights tie a slow-only head (and a matched-size replay buffer) on within-task adaptation: the fast store is redundant capacity or a small cache | cpu-now | registry-only | 2 |
| mot_pr8_retrieval_head | memory_augmented_retrieval_head | retrieval-conditioned head ties plain kNN (parametric part adds nothing) OR ties the no-memory parametric head (memory adds nothing), at matched capacity | cpu-now | registry-only | 2 |

## WS

| id | name | null hypothesis | exp_tier | status | tax |
|---|---|---|---|---|---|
| mot_ws1_agreement_vs_confidence | agreement_vs_confidence_central_gate | agreement AUROC minus single-substrate-confidence AUROC <= 0 within seed CI; OR the win fails the invertible-remap vacuity guard (B is a linear remap of A), the shuffled-cross-source control, or noisy-TV | cpu-now | registry-only | 2 |
| mot_ws2_fusion_tournament | matched_capacity_fusion_tournament | at matched capacity every structured fusion ties the concat-MLP on held-out accuracy and NLL: structure buys nothing beyond param count | cpu-now | registry-only | 2 |
| mot_ws3_arbitration | uncertainty_vs_disagreement_arbitration | precision weights are uninformative so inverse-variance ties equal-weight averaging, and disagreement is uncorrelated with correctness so disagreement routing ties random routing at matched rate; OR disagreement fires on aleatoric noise (fails noisy-TV) | cpu-now | registry-only | 2 |
| mot_ws4_bandwidth_sweep | broadcast_bottleneck_bandwidth_sweep | the bottleneck's benefit is indistinguishable from generic regularization at matched capacity, and broadcast-back adds nothing over write-only: narrow-vs-wide is just a capacity effect | cpu-now | registry-only | 2 |
| mot_ws5_router_slot | modular_router_plus_shared_slot | the shared slot adds nothing over sparse routing alone: slot-ablation ties the full model on BWT and accuracy at matched capacity; sparse routing does all the work | cpu-now | registry-only | 2 |
