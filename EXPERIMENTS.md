...
227 catalogued: implemented=139, registry-only=87, deferred=1.
...
| e5_curiosity | Curiosity as self-curriculum | prediction-error/RND curiosity equals random on a learnable-vs-noisy env; only learning-progress survives | cpu-now | implemented | 5 |
| e6_relational | Relational map over latents | the learned token-aware readout ties the strongest parameter-matched learned-flat, token-shuffle, or exact-architecture random-init control on held-out combinations | env-later | implemented | 3 |
...
| id | name | null hypothesis | exp_tier | status | tax |
|---|---|---|---|---|---|
| f10_intrinsic_form_curriculum | intrinsic_form_curriculum | learning-progress selection fails to improve untouched held-out-form transfer over every control or spends as much time on noisy forms as uniform, so the curriculum is not form-aware | cpu-now | implemented | 8 |
| f11_form_dream_replay | form_dream_replay | generated Form replay falls outside the preregistered retention band of stored replay at the same memory ceiling or fails the held-out Form-manifold validity floor | cpu-now | implemented | 4 |
| f12_private_form_language_stability | private_form_language_stability | cross-seed code agreement sits at or below the random-codebook floor and cross-seed probe transfer is at chance, so the form codes are private idiolects rather than a shared language | cpu-now | implemented | 2 |
| f13_form_energy_budget | form_energy_budget | every form interface lies on the same full-system accuracy-versus-cost frontier as raw or matched random features, so form structure buys no capability per retained byte, parameter, FLOP, or analytically estimated joule | cpu-now | implemented | 2 |
| f14_lifelong_form_expansion | lifelong_form_expansion | new-form insertion changes any old-memory key, value, or referent id, changes old-memory retrieval, forgets old forms beyond the retention band, or fails to beat the strongest matched existing-head and raw or shuffled retrieval controls | cpu-now | implemented | 5 |
| f15_embodied_affordance_form | embodied_affordance_form | consequence-conditioned Form tokens fail to beat both passive observation and action-shuffled controls on held-out affordance decoding and action selection | cpu-now | implemented | 7 |
| f16_perfect_slate_null | perfect_slate_null | the blank substrate fails to beat inherited frozen features, the larger shell, or its identical frozen-random initialization on licensed held-out transfer at a matched estimated end-to-end FLOP budget | env-later | registry-only | 10 |
| f17_missing_form_recovery | missing_form_recovery | recovery fails to beat the strongest tuned single-form, impute-by-mean, or zero-filled-concat control, or confidence does not predict correctness under a missing form, so recovery or monitoring is uninformative | cpu-now | implemented | 2 |
...
| f19_cross_scale_referent_binding | cross_scale_referent_binding | hierarchical referent memory ties the strongest flat, single-scale, or random-hierarchy control at matched bytes, so scale structure buys no retrieval and memory stays clip-shaped | cpu-now | implemented | 3 |
| f1_form_alignment_gate | form_alignment_gate | paired referent alignment fails to beat the strongest raw, moment-matched, or shuffled-anchor control by the preregistered margin, or remains near chance | cpu-now | implemented | 2 |
| f20_substrate_crisis_test | substrate_crisis_test | the crisis detector fails to beat the strongest raw-error, fixed-confidence, or random baseline, or triggers on aleatoric noise, so prospective insufficiency is not established | cpu-now | implemented | 4 |
| f21_asynchronous_temporal_binding | asynchronous_temporal_binding | the learned binding window ties the fixed window on binding F1 across delay and jitter, or it binds shuffled-time pairs, so context-dependent binding adds nothing over a constant | cpu-now | registry-only | 2 |
| f22_active_form_acquisition | active_form_acquisition | no matched-cost gain: the information-gain chooser fails to beat random, uncertainty, saliency, or full-observation acquisition on held-out worlds, or the gain vanishes when downstream training is untouched | cpu-now | registry-only | 7 |
| f26_cross_form_contradiction_triangulation | cross_form_contradiction_triangulation | source localization AUROC ties the raw per-source residual and the majority-vote and mean-fusion baselines, so contradiction handling reduces to averaging | cpu-now | registry-only | 2 |
| f27_causal_crossmodal_binding | causal_crossmodal_binding | the intervention-trained model ties the temporal-correlation model on unseen-intervention prediction, or it also binds the synchronous-but-unrelated pair, so intervention adds nothing beyond synchrony | cpu-now | registry-only | 10 |
| f28_sensor_value_forecast | sensor_value_forecast | the forecast does not rank realized gains: entropy, historical average, or hindsight scoring matches or beats the committed forecast, so sensor value is not predictable before payment | cpu-now | registry-only | 4 |
| f2_heldout_form_transfer | heldout_form_transfer | matched-exposure multi-form training fails to beat the strongest single-reference or matched-noise control by the preregistered margin, the held-out form remains near chance, or referent-shuffled alignment ties the treatment | cpu-now | implemented | 2 |
| f31_body_hardware_self_model | body_and_hardware_self_model | own-telemetry predictions do not beat the boundary-shuffled or constant-report control within the declared tolerances, so the declared body and hardware model carries nothing beyond channel base rates | cpu-now | registry-only | 4 |
| f32_tool_incorporation | tool_incorporation_boundary_shift | post-tool prediction accuracy on tool-coupled fields does not separate from the tool-detached or wrong-tool control, so no boundary shift beyond generic drift is established | cpu-now | registry-only | 4 |
| f33_internal_telemetry_prediction | internal_telemetry_prediction | forecasts do not beat persistence or rolling-mean baselines at any declared horizon, so the monitor restates the last value and adds no forward information | cpu-now | registry-only | 4 |
| f34_homeostatic_resource_control | homeostatic_resource_control | band occupancy under the declared policy does not beat matched random actuation at the same actuation rate, so the setpoint coupling adds no control beyond acting at all | cpu-now | registry-only | 4 |
| f35_self_report_grounding | self_report_grounding | grounded_fraction under the true trace does not separate from the shuffled-report or wrong-trace control, or reports do not change under state intervention, so reports are not grounded in the trace | cpu-now | registry-only | 4 |
| f36_limited_broadcast_necessity | limited_broadcast_necessity | lesioning or delaying the limited broadcast link does not hurt shared-task performance beyond the unrestricted-bus and message-shuffled controls, so the link is not necessary | cpu-now | registry-only | 2 |
| f37_broadcast_sufficiency | limited_broadcast_sufficiency | at matched compute at least one comparator matches or beats the limited-broadcast arm on shared-task performance, so limited broadcast is not selectively sufficient | cpu-now | registry-only | 2 |
| f38_metacognitive_efficiency | metacognitive_efficiency | no declared OA component yields benefit per monitor FLOP above its matched-cost null and random-monitor control, so monitoring does not pay for its own compute | cpu-now | registry-only | 4 |
| f3_form_bottleneck_capacity | form_bottleneck_capacity | the nested wide bottleneck fails to beat the zero-padded small bottleneck by the preregistered margin, or wide performance remains near the chance or shuffled-label floor | cpu-now | implemented | 4 |
| f4_raw_payload_vs_form_tokens | raw_payload_vs_canonical_form_tokens | ordered canonical form tokens fail to beat the strongest raw, handcrafted, shuffled-referent, or token-order control by the preregistered margin | cpu-now | implemented | 2 |
| f50_curriculum_goldilocks_test | curriculum_goldilocks_test | no transfer gain, or the selector is captured by irreducible noise: banded selection matches random or ordered schedules on downstream transfer per sample | cpu-now | registry-only | 8 |
| f51_safe_play_goal_babbling | safe_play_goal_babbling | discovered skills are not reused: external-task acceleration is absent or matched by random exploration or ungated curiosity, so guarded babbling adds nothing over noise | cpu-now | registry-only | 8 |
| f52_quality_diverse_mode_ecology | quality_diverse_mode_ecology | diversity is decorative or costly: the archive fails to beat the monolithic optimum at matched compute, or a stop rule fires before the declared horizon | cpu-now | registry-only | 5 |
| f53_joint_referent_establishment | joint_referent_establishment | no partner-general alignment: agreement on held-out partner policies falls to the cue-blind or pattern-matching control level, so apparent alignment is regularity matching over the training policy family | cpu-now | registry-only | 2 |
| f54_communicative_repair | communicative_repair | no efficiency or accuracy gain: repetition or matched extra bandwidth recovers failed reference as well as the dedicated repair channel | cpu-now | registry-only | 2 |
| f55_selective_imitation | selective_imitation | same overimitation or lower success: fidelity or task success does not beat indiscriminate cloning, so step selection adds nothing | cpu-now | registry-only | 2 |
| f56_teaching_value | teaching_value | no held-out learner advantage: equal information or generic selection matches taught progress, so teaching is relabeled data volume | cpu-now | registry-only | 2 |
| f57_emergent_symbol_grounding | emergent_symbol_grounding | private idiolect or noncausal code: the code fails cross-seed, cross-partner, cross-generation, or cross-referent transfer, or a direct-state channel matches it at equal bandwidth | cpu-now | registry-only | 2 |
| f58_cultural_accumulation | cultural_accumulation | no ratchet: the generation trend is flat or negative, or gains are convention-only and vanish on external tasks | cpu-now | registry-only | 5 |
| f59_memory_poisoning_resistance | memory_poisoning_resistance | an injected adversarial revision stays readable after the quarantine mark, rollback fails to restore the exact pre-injection content, a consolidated summary carries a declared private token forward, or deletion after consolidation leaves the memory readable, so resistance is not established | cpu-now | registry-only | 4 |
| f5_cross_form_memory_binding | cross_form_memory_binding | cross-form retrieval fails to beat raw or shuffled controls, or remains materially below same-form independent-view retrieval, so memory is form-local rather than referent-bound | cpu-now | implemented | 2 |
| f60_transactional_self_rewrite | transactional_self_rewrite | the gate allows a promotion when the executor requests it, when any stage receipt is missing or truncated, when fewer than two distinct evaluators sign, or when evaluators disagree, so transactional discipline is not established | cpu-now | registry-only | 9 |
| f61_physical_reservoir_digital_twin | physical_reservoir_digital_twin | no shared interface survives all three priors without a per-prior special case, or the priors are nondeterministic under seed, so a common physical-reservoir twin is not established | cpu-now | registry-only | 4 |
| f62_material_native_dynamics_value | material_native_dynamics_value | the material prior fails to beat every tuned matched-cost control on a retained win, or the win does not replicate, so native-dynamics value is not established | cpu-now | registry-only | 4 |
| f63_drift_and_aging_adaptation | drift_and_aging_adaptation | adaptation does not retain function above the no-adapt floor at lower cost than oracle-reset or full-retraining, so drift-aware adaptation is not established | cpu-now | registry-only | 4 |
| f64_damage_and_reattachment_recovery | damage_and_reattachment_recovery | no repair policy restores selectively lesioned function under novel inputs above the fixed-final and restart controls at lower cost than full-retraining, so restoration-without-retraining is not established | cpu-now | registry-only | 4 |
| f65_specimen_to_specimen_transfer | specimen_to_specimen_transfer | readout transfer between specimens does not exceed within-specimen and random-specimen controls, or the contract permits local physical execution, so specimen-to-specimen transfer is not established and not safely gated | env-later | registry-only | 4 |
| f66_cross_substrate_form_portability | cross_substrate_form_portability | a readout carried across simulated forms does not beat the identity and random-remap controls at lower refit cost than full-retrain, so cross-form portability is not established | cpu-now | registry-only | 4 |
| f6_sensorimotor_form_closure | sensorimotor_form_closure | true action-conditioned Form closure fails to beat both action-blind and action-shuffled controls on held-out rollout prediction and deterministic goal reachability | cpu-now | implemented | 7 |
| f7_developmental_form_growth | developmental_form_growth | learning-progress growth fails to beat the stronger fixed-final or random-growth control on the adaptation-retention frontier at matched final parameters | cpu-now | implemented | 1 |
| f8_plastic_substrate_rewrite | plastic_substrate_rewrite | a plastic substrate fails to beat the frozen inherited, larger frozen-shell, or SSL-trained random-init controls on licensed held-out factors at a matched estimated end-to-end FLOP budget | env-later | registry-only | 9 |
| f9_cross_form_compositional_binding | cross_form_compositional_binding | held-out cross-form combinations collapse toward the strongest single-form, shuffled-label, or shuffled-referent floor while seen pairs stay high, so the system did not bind factors across forms | cpu-now | implemented | 3 |
...
| mop_al2_shared_latent_alignment | shared_latent_alignment_vs_random_map_floor | a random map of equal rank predicts the target as well as the learned map (alignment-artifact): any apparent cross-substrate agreement is the map's capacity, not shared structure | env-later | registry-only | 2 |
| mop_al3_audio_video_alignment | audio_video_temporal_alignment | audio-SSL and V-JEPA event boundaries co-locate no better than under a shuffled-time permutation: temporal-predictive structure is modality-specific, not universal | env-later | registry-only | 3 |

## AT

| id | name | null hypothesis | exp_tier | status | tax |
|---|---|---|---|---|---|
| mop_at1_nuisance_grid | cross_substrate_nuisance_grid | every substrate's decodability delta over its matched-resolution random-init control is within seed spread (random-control-artifact): pretraining bought no nuisance invariance beyond architecture and resolution in any substrate | env-later | registry-only | 3 |
| mop_at2_mode_substrate_dep | mode_substrate_dependence | every mode's advantage is reproduced by random-init-ViT (or random-encoder), so no mode depends on the pretrained substrate rather than architecture/resolution or generic separability | cpu-now | registry-only | 3 |
...
| mop_cm11_developmental_plasticity | developmental_scheduled_plasticity | a non-developmental substrate trained on identical data with a flat schedule ties the developmental one on every plasticity signature (window, path-dependence, U-shape): the development is cosmetic | cpu-now | registry-only | 1 |
| mop_cm12_mop_substrate_capstone | mixture_of_perspectives_substrate_capstone | a single best-expert-plus-workspace ties the full mixture at matched total FLOPs: the mixture is pure overhead and should be descoped to the single expert | cpu-now | registry-only | 2 |
| mop_cm1_compositional_gate | compositional_bound_on_real_video_c1_gate | held-out-combination accuracy for real V-JEPA ties random-init same-arch ViT (delta < +0.05) on a D3-certified separable regime: the substrate memorizes conjunctions rather than factoring them | env-later | registry-only | 3 |
| mop_cm2_atlas_gate | multi_substrate_atlas_gate | all frozen substrates tie their own random-init controls on the failing gate (no substrate exceeds +0.15 off-ceiling where V-JEPA-L did not): no substrate swap helps | env-later | registry-only | 3 |
| mop_cm3_dense_vs_pooled | dense_vs_pooled_compositional | dense and coarse-grid tie full mean-pool on held-out combos (delta < +0.1 off-ceiling on a D3 regime): the pooling interface is not the bound (single-factor pooling already refuted, orientation decodes at 1.0 pooled) | env-later | registry-only | 3 |
| mop_cm4_workspace_shell | workspace_shell_routing | the workspace shell ties param-matched dense OR ties matched-compute unrolled depth on BWT and planning, or sign-flips across seeds: the win is capacity/depth, not routing | env-later | registry-only | 2 |
| mop_cm5_studio_rejuvenation | local_max_rejuvenation_c3 | plasticity loss never appears at reachable scale, OR rejuvenation ties doing-nothing, OR frozen-substrate restoration ties the adapting-substrate arm (no C3 signal) | cpu-now | registry-only | 1 |
| mop_cm6_distilled_density | compressed_capability_density | a same-size random-init substrate OR a same-size student trained without the invariance-matching loss ties the distilled student on the nuisance-invariance delta: compression bought no capability density | env-later | registry-only | 2 |
...
| mop_p5_context_capability | context_capability_pilot | at matched parameters and matched active FLOPs, windowed, recurrent, and hierarchical factorized context mechanisms match exact global attention on held-out factor combinations within the 0.10 SESOI at 32 and 64 frames while peaking at lower measured bytes; exact global interaction is not a live capability lever in this context range | cpu-now | implemented | 9 |
...
| mop_dr14_corruption | reasoning_under_corruption_compression | reasoning and single-pass degrade at the same rate under every corruption: iteration only processes what survives | env-later | registry-only | 3 |
| mop_dr15_modality_general | modality_general_reasoning | the gain is identical across all three modalities (a shell/task property) OR present only on the most separable modality (an artifact) | env-later | registry-only | 2 |
| mop_dr1_video_cache | real_bound_attribute_video_cache | even on real video the pooled and dense latents ceiling at 1.0 on held-out-combination decode (D3 says still trivially separable), so no compositional mode scores above the random-encoder floor | env-later | registry-only | 5 |
| mop_dr2_sparse_real | sparse_head_forgetting_on_real_latents | on real latents with a D3 certificate and a paired significance test, sparse ties param-matched dense (and ties a dense head with matched activation-sparsity penalty); the synthetic win was cluster separability | env-later | registry-only | 2 |
| mop_dr3_latent_scratchpad | latent_scratchpad_vs_residual_stream | scratchpad ties residual-only at all memory loads: the pooled frozen latent already discarded the detail external memory would store (a substrate bound) | env-later | registry-only | 3 |
| mop_dr4_causal_intervention | counterfactual_causal_latent_intervention | intervened rollouts do not beat the correlational baseline and single-factor interventions leak into other factors: the frozen latent entangles factors | env-later | registry-only | 10 |
| mop_dr5_cross_substrate_consistency | cross_substrate_reasoning_consistency | the reasoning gain is identical across all substrates (a shell/task property, not substrate-carried) OR vanishes on a second real encoder (fragile) | env-later | registry-only | 2 |
...
| mop_dr7_latent_cot | latent_chain_of_thought | the latent chain ties one-shot at matched compute and shuffling does not hurt: the chain is a bag of extra layers (ex17 generalized) | env-later | registry-only | 9 |
...
| mop_mt4_reasoning_router | router_over_reasoning_primitives | at matched mean FLOPs the router ties or loses to the single best fixed mode; any win is bought compute or vanishes under seed sweep | cpu-now | registry-only | 2 |
