#!/usr/bin/env python
"""MT1/MT2/MT3 router pilots (WP-08; registry docs/mixture_of_thinking/11_experiment_registry.md,
...
Three preregistered questions over ONE heterogeneous mode bank routed per episode:
  MT1: a per-episode router over {reactive readout, planner (refiner), sparse (kWTA) head} beats the
       single best mode on capability density (accuracy per FLOP) at matched compute
  MT2: ROUTING beats a uniform equal-weight blend of the same modes at matched TOTAL FLOPs (the
       ...
       density alongside)
  MT3: mode DIVERSITY is load-bearing: the heterogeneous bank beats k seed-copies of the best mode
...
Preregistered verdict logic per pilot (fixed in code before any run; manifest appendix):
  ...
  (MT3: matched params AND FLOPs; MT2's matched blend is matched by construction; MT1's density
  ...
  MT1 metric: density delta vs the best single mode (selected on router-train, the tuned baseline).
  MT2 metric: accuracy delta vs the budget-matched blend when attainable (matched_within tol 0.10),
  else density delta vs the full blend (fallback logged).
  MT3 metric: density delta vs the param-matched homogeneous k-copy bank.
  A PR1 NULL context demotes every WIN string to PLAUSIBLE-BUT-UNVERIFIED (rows still run).

Usage: PYTHONPATH=. python scripts/mot_mt123_router_pilots.py --seeds 0-4 [--tuned]
...
from scripts.mot_dr12_disagreement import parse_seeds, read_pr1_context
from scripts.pr1_mode_error_disjointness import SUBPOPS, RefinerHead, make_dataset
from torch import nn

from devsys.diagnostics.compute import matched_within, mlp_flops, param_count, refiner_flops
...
CONTRACT = {
    ...
    "null_hypothesis": (
        "MT1: routed density <= best single mode at matched compute; MT2: router <= uniform blend "
        "at matched total FLOPs; MT3: heterogeneous <= homogeneous k-copy at matched params and FLOPs"
    ),
    "tier": "cpu-now",
}
...
def density(acc: float, flops_per_sample: float) -> float:
    """Capability density: accuracy per MFLOP per sample (the MT1/MT2/MT3 pilot metric)."""
...
def verdict_block(
    ...
    matched_ok is True (for MT3 that means params AND FLOPs matched on every seed, the manifest
...
def run(
    ...
    for s in seeds:
        ...
        # MT1: best single mode, SELECTED ON ROUTER-TRAIN (the tuned baseline), scored on eval episodes
        ...
        # MT2: uniform blend, full cost, plus the budget-matched blend at the routed arm's cost
        ...
        # MT3: homogeneous k-copy bank of the best mode at matched TOTAL params (capmatch)
        ...
        # tuned flag, same grid, selection on the router-train split), so the MT3 baseline is never
    ...
    # MT3's appendix wording is "matched params AND FLOPs": both must hold on every seed or the
    # verdict is UNMATCHED. MT1 embeds compute in the density metric; MT2's matched blend is matched
    ...
    return {
        "experiment": "mot_mt123_router_pilots",
        "contract": CONTRACT,
        "config": {
            "seeds": seeds,
            "dim": dim,
        ...
        "verdict_rule": (
            ...
            "recorded compute match holds on every seed (MT3: params and FLOPs; a failed match is "
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="MT1/MT2/MT3 router pilots (PR1-gated, synthetic)")
