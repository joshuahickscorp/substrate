#!/usr/bin/env python3
"""Build the sealed 74-capsule Generation 1 successor-horizon v2 program."""
...
DEFAULT_OUTPUT = REPO_ROOT / "configs/campaign/generation1_successor_horizon_v2.json"
POLICY_PATH = "configs/local_execution_throttle_v5_opportunistic.yaml"
CLI_PATH = "scripts/generation1_successor_horizon_v2/mop_generation1_successor_horizon.py"
BUILDER_PATH = "scripts/build_generation1_successor_horizon_v2_program.py"
RUNTIME_PATH = "src/mop/studies/generation1_successor_horizon_v2.py"
VERIFIER_PATH = "src/mop/studies/generation1_successor_horizon_v2_verify.py"
VERIFICATION_SCHEMA = verifier.VERIFICATION_SCHEMA

ADMISSION_PATH = f"{PROGRAM_ROOT}/admission.json"
RESULT_PATH = "proof/GENERATION1_SUCCESSOR_HORIZON_V2.json"
VERIFICATION_PATH = "proof/GENERATION1_SUCCESSOR_HORIZON_V2.verification.json"
REPORT_PATH = "runs/generation1/GENERATION1_SUCCESSOR_EVIDENCE_REPORT_V2.md"
REPORT_RECEIPT_PATH = f"{PROGRAM_ROOT}/report_receipt.json"

PARENT_RESULT_PATH = "proof/GENERATION1_SUCCESSOR_HORIZON.json"
PARENT_VERIFICATION_PATH = "proof/GENERATION1_SUCCESSOR_HORIZON.verification.json"
PARENT_REPORT_RECEIPT_PATH = "runs/generation1/generation1-successor-horizon-v1/report_receipt.json"
...
def _authority(path: str) -> dict[str, str]:
    ...
    if not source.is_file() or source.is_symlink():
        raise ValueError(f"successor-horizon v2 authority must be a regular file: {source}")
...
def _admission_capsule() -> dict[str, Any]:
    return _capsule(
        ...
        compute=False,
        resource_basis=(
            "single-thread validation of the exact sealed v1 program manifest, clean "
            "zero-injection terminal supervisor status, replayed v1 verifier, result, "
            "report receipt, and final H05 classification under the kernel-enforced "
            "4096-MiB taskpolicy cap"
        ),
...
def _shard_capsule(
    *,
    ...
    scheduler_epoch = _scheduler_epoch(epoch_index)
    artifact_epoch = horizon.EPOCH_IDS[epoch_index]
    capsule_id = f"g1_{scheduler_epoch}_{lane}_shard_{shard_index:02d}"
    ...
    return _capsule(
        ...
        compute=True,
        resource_basis=(
            f"checkpointed {lane} robustness shard for artifact epoch "
            f"{artifact_epoch}, planned at {planned_minutes:.1f} serial minutes; "
            "the scheduler-local H01-H05 task identity preserves the unchanged "
            "reviewed 8-to-1 coexistence envelope"
        ),
...
def _classification_capsule(
    ...
    artifact_epoch = horizon.EPOCH_IDS[epoch_index]
    return _capsule(
        capsule_id=f"g1_{scheduler_epoch}_classify",
        kind="aggregate",
        ...
        compute=False,
        resource_basis=(
            f"single-thread reconstruction of artifact epoch {artifact_epoch}, "
            "including its v1-parent continuation and next-epoch routing barrier, "
            "under the coexistence taskpolicy cap"
        ),
...
def _tail_capsules(last_classification: str) -> list[dict[str, Any]]:
    ...
    aggregate = _capsule(
        ...
        compute=False,
        resource_basis=(
            "single-thread binding of the 65 H06-H10 shard receipts and five "
            "classifications into one sealed cross-epoch v2 result"
        ),
    ...
    verify = _capsule(
        ...
        artifact=_artifact(
            ...
            "verification_sha256",
            {
                ...
                "checks.predecessor_authority_chain_valid": True,
                "checks.dependency_closed_admission_valid": True,
                ...
                "checks.predecessor_and_v2_seed_spaces_disjoint": True,
        ...
        compute=False,
        resource_basis=(
            "separately authored one-thread streaming reconstruction of all v2 "
            "receipts, parent bindings, classifications, aggregation, and semantic "
            "mutations"
        ),
        forecast_write_gb=0.2,
        atomic_write_gb=0.05,
    ...
    report = _capsule(
        ...
        compute=False,
        resource_basis=(
            "single-thread rendering of a non-authoritative v2 Markdown view plus "
            "a sealed receipt binding the result, verification, and report bytes"
        ),
        forecast_write_gb=0.02,
        atomic_write_gb=0.01,
    )
...
def validate_runtime(path: Path, expected_sha256: str) -> None:
    ...
    if program.program_sha256 != expected_sha256:
        raise ValueError("loaded successor-horizon v2 digest differs from generated digest")
    ...
    if problems:
        raise ValueError("successor-horizon v2 program is not runtime-admissible:\n" + "\n".join(problems))
...
def main() -> int:
    ...
    if not output.is_relative_to(REPO_ROOT.resolve()):
        raise SystemExit("successor-horizon v2 manifest must remain inside the repository")
    program = build_program()
    if arguments.check:
        if json.loads(output.read_text(encoding="utf-8")) != program:
            raise SystemExit("successor-horizon v2 manifest is stale")
    else:
