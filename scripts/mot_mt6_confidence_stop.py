#!/usr/bin/env python
"""MT6: trained confidence stopping vs the FREE update-norm early-exit rule (H-HALT, WP-03).

Thesis: a trained halt head carries a real per-sample correctness signal beyond "the latent stopped
moving". Mechanism: the SAME trained refiner + halt head as MT5 (imported harness), evaluated against
...
Preregistered null (verbatim, registry MT6): the trained halt head ties the free update-norm rule;
...
Usage: .venv/bin/python scripts/mot_mt6_confidence_stop.py --seeds 0-4
"""
...
from mot_mt5_adaptive_halting import (  # the WP-03 shared halting harness
    ENTROPY_FLOOR_BITS,
    FLOP_TOL,
    default_cfg,
    make_graded_split,
...
from devsys.diagnostics.compute import matched_within, refiner_flops
from devsys.diagnostics.riskcov import auroc, seed_ci, sign_flip_report
