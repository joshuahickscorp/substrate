#!/usr/bin/env python3
"""Materialize one bounded artifact of the full-generations mechanics wave scaffold."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    gate = commands.add_parser(
        "materialize-gate",
        help="materialize one serial full-generations admission gate",
    )
    ...
    integration = commands.add_parser(
        "run-integration",
        help="evaluate the conditionally eligible post-W21 substituted-dependency I1 integration",
    )
    ...
    release_audit = commands.add_parser(
        "release-audit",
        help="capture the advisory read-only release audit without gating activation or promotion",
    )
    release_audit.add_argument("--output", type=Path, default=wave.DEFAULT_RELEASE_AUDIT)
    return parser
...
def dispatch(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    if arguments.command == "verify":
        verifier = importlib.import_module("mop.studies.generation1_full_generations_wave_verify")
    ...
    if arguments.command == "release-audit":
        return wave.write_release_audit(output=arguments.output)
    raise ValueError(f"unsupported full-generations-wave command: {arguments.command}")
