#!/usr/bin/env python
"""Run or resume the P4 capability-density response-surface screen.

Profiles are wall envelopes, not claim levels. ``p4smoke`` proves the plumbing end to end with one
seed and twelve steps; ``p4screen`` is the full five-seed screen. A stopped run is resumable from
content-addressed per-arm checkpoints and per-seed receipts by rerunning the exact same command.
The proof artifact is written atomically and only when the screen completes; the promotion block
refuses confirmatory claims by construction.
"""
...
from mop.substrate.p4_screen import run_p4_screen

PROFILES = ("p4smoke", "p4screen")
...
def _config(profile: str) -> dict:
    cfg = compose(["experiment=mop_p4_capability_density_screen"])
    value = OmegaConf.to_container(cfg.experiment, resolve=True)
    if not isinstance(value, dict):
        raise TypeError("resolved P4 screen config is not a mapping")
    profiles = value.pop("profiles", None)
    if not isinstance(profiles, dict) or profile not in profiles:
        raise SystemExit(f"profile {profile!r} is missing from the P4 screen config")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--profile", choices=PROFILES, default="p4screen")
    ...
    run_dir = args.run_dir or REPO_ROOT / "runs/p4_screen" / args.profile
    receipt = run_p4_screen(config, run_dir, args.device)
    out = args.out or REPO_ROOT / "proof/P4_CAPABILITY_DENSITY_SCREEN.json"
    if receipt["complete"]:
        ...
        json.dumps(
            {
                ...
                "cells_complete": sum(1 for row in receipt["cells"].values() if row["complete"]),
                "response_surface_estimable": receipt["response_surface"]["estimable"],
