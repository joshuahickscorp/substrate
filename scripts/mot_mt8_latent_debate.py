#!/usr/bin/env python
"""MT8: latent debate between seeded modules vs single module and plain ensemble (WP-04, MANIFEST).
...
verifier, mot_dr9_verify_revise.train_backbone) are trained on the same regime, then frozen. The
...
runs/pre_studio/pr1_mode_error_disjointness.json (never rebuilt, via mot_dr12_disagreement's reader).
...
Preregistered null (verbatim, manifest WP-04): MT8 debate ties max(single, ensemble) (the ex17
...
Usage: .venv/bin/python scripts/mot_mt8_latent_debate.py --seeds 0-4
"""
...
from mot_dr9_verify_revise import train_backbone
from mot_dr12_disagreement import PR1_PATH, read_pr1_context
from mot_mt5_adaptive_halting import make_graded_split, parse_seeds, pearson, resolve_out
...
from devsys.diagnostics.difficulty_calibration import reference_separation
