...
def _run_arm(replay_ewc, train, test, dim, n_classes, dev, seed=0):
    seed_everything(seed)
    model = ClassHead(dim, n_classes, hidden=64, depth=2)
    buf = ReplayBuffer(capacity=2048, dim=dim, index="brute", seed=seed) if replay_ewc else None
    con = (
        Consolidation(
            OmegaConf.create(
...
def main(argv: list[str] | None = None) -> int:
    ...
    naive = _run_arm(False, train, test, x.shape[1], n_classes, dev)
    prot = _run_arm(True, train, test, x.shape[1], n_classes, dev)
