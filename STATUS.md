...
- [!] P5 smoke admission failed closed on three consecutive samples. Normalized one-minute CPU load
  was 3.862 to 3.987 per logical CPU against the 0.85 ceiling, and available unified memory was
  6.830 to 8.909 GB against the unchanged 10.0 GB requirement. AC power, disk, thermal, telemetry,
  lane, swap, and unmanaged-heavy gates passed. The first blocker is category 1 local admission
  state, not measured hardware necessity. The execute attempt launched no child.
...
- [x] Full P4 completed under the corrected CPU governor and was closed by the governor after C12.
  ...
  CPU-load and memory-headroom admission gates.
...
- [!] P5 execution is next on lane 0 (staging: p5smoke, then the cold-process training-grid memory
  ...
  currently held by the unchanged CPU-load and memory-headroom gates.
