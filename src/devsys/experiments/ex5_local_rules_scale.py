...
devsys.learning.alternatives.rules (which other experiments depend on exactly as-is).
...
Persistent rule implementations (exactly what update rule each one is, stated plainly):
  ...
  * feedback_alignment: the SAME 2-layer architecture, but the hidden-layer credit signal
    ...
    devsys.learning.alternatives.rules.train_feedback_alignment's per-step update, but
  ...
  * predictive_coding: Whittington-Bogacz-style local energy descent, the same shape as
    devsys.learning.alternatives.rules.train_predictive_coding: clamp input and output to
...
and its retention measured with devsys.metrics.ContinualResult.
