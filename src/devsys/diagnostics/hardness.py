...
def make_graded_slot_task(
    ...
    codebook = {
        name: torch.randn(SLOT_CARD[name], slot_dim, generator=g) * 1.5 for name in SLOT_ORDER
    }

    slots = torch.stack(
        [torch.randint(0, SLOT_CARD[name], (n,), generator=g) for name in SLOT_ORDER], dim=1
    )
    ...
    from devsys.shell.verifier_exec import target_program
