"""E2-E10 scaffolds plus the runnable bleeding-edge EX experiments (EX8, EX12, EX17). Each is a
self-contained Experiment subclass with its null baked in, runnable at toy scale. Imported here so the
registry can pick them up (see __init__). EX rows that are not yet runnable stay registry-only in
registry/experiments.yaml (catalogued, honest tier) until their module lands."""

from __future__ import annotations

# Cross-disciplinary roots-expansion series (cpu-now), generated by the roots workflow. Each series file
# holds self-contained Experiment subclasses; the registry/experiments.yaml rows preregister their nulls.
...
from .f_form_substrate import F1, F2, F3, F4, F5, F9, F10, F12, F17
