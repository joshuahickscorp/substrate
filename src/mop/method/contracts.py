...
class ControlContract(Contract):
    ...
    def violations(self):
        ...
        return [
            f"control:{self.name}: fails {k}" for k, ok in s.items() if k != "all_pass" and ok is False
        ]
...
class PowerContract(Contract):
    ...
    def violations(self):
        ...
        elif p.get("mde") is not None and self.declared.get("sesoi") is not None:
            if p["mde"] > self.declared["sesoi"]:
                v.append(
