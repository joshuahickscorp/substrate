...
def validate(graph: dict, evidence: dict | None = None) -> list[str]:
    ...
    for i, n in nodes.items():
        if n["type"] == "mechanism":
            if not any(nodes.get(e["dst"], {}).get("type") == "intervention" for e in out[i]):
    ...
    for i, n in nodes.items():
        if n["type"] == "primary_outcome":
            if not any(e["type"] == "measured_relation" for e in inn[i]):
    ...
    for e in graph.get("edges", []):
        ...
        if s.get("time") == "future" and d.get("type") in ("mechanism", "intervention", "treatment"):
            if e.get("type") != "forbidden_information_path":
