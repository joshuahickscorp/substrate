"""MechanismRunner for the stability vs plasticity bed: measure both axes, mint a demonstration.

This module wires the bed and the learners into the ladder run-receipt contract. It runs the
mechanism and every declared control on a chosen regime, measures retention and future-learnability
for each, and mints a mechanics-demonstration RunReceipt whose verdict is fail-closed on the P6
split:

- On the NULL regime the verdict is always ``null``: the stream is built so a stable core and rapid
  adaptation cannot coexist, and the measurement confirms the mechanism cannot beat every control on
  both axes there.
- On the FAVORABLE regime the verdict is ``mechanics-ok`` ONLY when the mechanism strictly improves
  BOTH retention AND future-learnability over EVERY control. Improving only one axis is exactly the
  P6 split, so it mints ``null``, never a joint win.
...
from .stability_plasticity_bed import REGIME_FAVORABLE, REGIME_NULL, REGIMES
from .stability_plasticity_impl import MECHANISM_ARM, run_all
from .stability_plasticity_scaffold import DUAL_AXES, PRIOR_NULL, REQUIRED_CONTROLS, DualMetricReading
...
class RunResult:
    ...
    ``retention_margin`` and ``plasticity_margin`` are the mechanism's WORST margin over the control
    ...
    def both_axes_win(self) -> bool:
        """Strictly beats every control on retention AND on future-learnability. The joint bar."""
...
@dataclass(frozen=True, slots=True)
class StabilityPlasticityRunner:
    """Runs the stability vs plasticity mechanism and controls, and mints a demonstration receipt.
    ...
    def run(self, bed: Bed, seed: int, regime: str = REGIME_FAVORABLE) -> RunResult:
        """Measure retention and future-learnability for the mechanism and each control on a regime."""
