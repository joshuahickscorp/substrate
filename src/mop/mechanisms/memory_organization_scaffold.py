"""Scaffold spine for the memory-organization epoch (G1-R1) of the Form Substrate Program.

This module raises the SCAFFOLDING axis only. It encodes, as machine-checkable contracts, the bar
a memory organization must clear before it may be retained: it must improve FUTURE decisions, not
only prediction, and it must support revision, provenance, deletion, and action. It supplies
fail-closed contracts, deterministic seeded exercises, declared decision controls, an episodic-harm
guard, and an activation gate that is off until two named prior nulls are cleared. It builds the
harness, not the result.

Claim scope for the whole module: deterministic programmatic mechanics only; no capability or
natural-data claim. Nothing here establishes that any memory organization carries value. The
exercises are toy op logs. The controls are declarations. The activation gate refuses by default.

The bar is forced by two named prior nulls. The replay-parity null says a plain replay baseline at
matched cost can match an organized memory, so a win must survive a replay-only control at equal
retrieval budget. The episodic-harm null says episodic memory has been observed to harm future
decisions, so an organization is refused the moment measured episodic harm exceeds tolerance.
...
# ---------------------------------------------------------------------------
# Named prior nulls that force this epoch's bar.
# ---------------------------------------------------------------------------
...
# The four capabilities a memory organization must declare and exercise. Ordering is load-bearing
...
# The controls a future-decision value comparison must run against. Order is load-bearing.
...
# Value must be measured on future decisions. Prediction-only improvement is explicitly not enough.
FUTURE_DECISION_TARGET = "future-decisions"
VALUE_TARGETS: tuple[str, ...] = (FUTURE_DECISION_TARGET, "prediction")

# The treatment arm plus its four declared controls. Used by the deterministic mechanics probes.
...
# ---------------------------------------------------------------------------
# Section A. Capability exercises. A capability must be declared AND exercised.
# ---------------------------------------------------------------------------


@dataclass(frozen=True, slots=True)
class CapabilityExercise:
    """One of the four required capabilities, carried with a hash-checked exercise witness.

    A declaration alone is refused. The op log records the ops that exercised the capability, and
    the witness digest binds the log so tampering is detected. Claim scope: deterministic
    ...
    def build(cls, capability: str, op_log: Sequence[str]) -> CapabilityExercise:
        """Deterministically build a valid exercise with a computed witness digest."""
...
class MemoryOrganizationContract:
    """Declares one memory organization; refuses unless all four capabilities are exercised.

    Prediction-only improvement is not a sufficient reason to keep an organization, so a contract
    that declares prediction-only sufficiency is refused. Claim scope: deterministic programmatic
...
# ---------------------------------------------------------------------------
# Section B. Matched retrieval budget and future-decision value contract.
# The replay-parity null lives here: a replay-only control at matched cost forces the bar.
# ---------------------------------------------------------------------------


@dataclass(frozen=True, slots=True)
class MatchedRetrievalBudget:
    """The matched retrieval budget every decision arm must be held to before comparison."""
...
class FutureDecisionValueContract:
    """Declares that value is measured on future decisions against the four declared controls.

    Refuses unless the value target is future decisions, the controls match the canonical set and
    order, matched retrieval cost is required, at least two replications are demanded, and
    prediction-only sufficiency is not asserted. Claim scope: deterministic programmatic mechanics
...
# ---------------------------------------------------------------------------
# Section C. Episodic-harm guard. The episodic-harm null lives here: measured harm fails closed.
# ---------------------------------------------------------------------------


@dataclass(frozen=True, slots=True)
class EpisodicHarmGuard:
    """Fails closed the moment episodic memory measurably harms future decisions.

    Encodes the episodic-harm finding as a fail-closed condition. A no-memory arm and an episodic
    arm are compared on the same future decisions; when the episodic arm is worse by more than the
    tolerance the guard refuses. Claim scope: deterministic programmatic mechanics only; no
    capability claim.
    """
    ...
    def assert_no_episodic_harm(self, *, no_memory_value: float, episodic_value: float) -> float:
        """Refuse if the episodic arm is worse than the no-memory arm beyond the tolerance.

        Returns the measured harm (no_memory_value minus episodic_value) when within tolerance.
        """
...
# ---------------------------------------------------------------------------
# Section D. Activation gate. Off by default; earns activation only with a valid receipt that
# clears both named prior nulls. This encodes "activation not earned yet".
# ---------------------------------------------------------------------------


@dataclass(frozen=True, slots=True)
class ActivationReceipt:
    """A confirmation receipt that both named prior nulls were cleared under matched cost.

    A valid receipt requires that the replay-parity and episodic-harm nulls were both cleared, that
    the future-decision win replicated at least twice, that matched cost was confirmed, and that no
    episodic harm remained. The receipt digest binds the body so a forged receipt is detected.
    ...
    def build(cls, *, issuer: str, replications: int) -> ActivationReceipt:
        """Deterministically build a valid receipt with a computed digest."""
...
class MemoryActivationGate:
    """A fail-closed gate. Activation is off by default and earns only with a valid receipt.

    Claim scope: deterministic programmatic mechanics only; no capability claim. The gate exists so
    that no memory organization can be marked active until both the replay-parity and episodic-harm
    nulls have been cleared under matched cost and replicated.
    """
    ...
    def authorize(self, receipt: ActivationReceipt | None = None) -> None:
        """Fail closed unless a valid receipt clearing both named prior nulls is supplied."""
...
# ---------------------------------------------------------------------------
# Section E. Deterministic seeded mechanics probes and builders.
# All helpers are seeded and reproducible; no wall-clock, no unseeded randomness.
# ---------------------------------------------------------------------------


def simulate_capability_ops(seed: int, capability: str) -> tuple[str, ...]:
    """Deterministically synthesize the op log that exercised one capability for a seed.

    Claim scope: deterministic programmatic mechanics only; no capability claim. The ops are toy
    labels, not evidence that any capability carried value.
    """
...
def build_memory_organization(seed: int) -> MemoryOrganizationContract:
    """Build a fully covering, exercised memory organization contract for one seed."""
...
def toy_decision_values(seed: int) -> dict[str, float]:
    """Deterministic toy future-decision values per arm; not evidence, a byte-exact test vector.
...
def default_future_decision_contract() -> FutureDecisionValueContract:
    """Return a valid, non-vacuous future-decision value contract."""
...
def coverage() -> dict[str, Sequence[str]]:
    """Map this epoch's sub-questions to the scaffold guarantees that address them (readiness only).
