"""MechanismRunner for the redesigned stability vs plasticity bed: measure both axes, mint a demo.

This module wires the r2 bed and the learners into the ladder run-receipt contract. It runs the
...
- On the NULL regime the verdict is always ``null``: the stream is built from conflicting
  same-subspace overwrites with a silent recurrence signal, and the measurement confirms the
  mechanism cannot beat every control on both axes there.
...
class StabilityPlasticityR2Runner:
    """Runs the redesigned mechanism and controls, and mints a demonstration receipt.
