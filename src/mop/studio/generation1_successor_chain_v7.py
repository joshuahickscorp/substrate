"""Append-only v7 adopter for the Generation 1 successor evidence chain.

Version 7 supersedes the immutable v6 integrity hold. It preserves the
observe-only legacy adoption and unchanged v1 horizon launch, keeps the bounded
re-snapshot around the exact multiprocessing spawn-title transition, and keeps
the second, equally fail-closed re-snapshot class for a child-label worker that
has not yet settled into the clean all-three (repo-cwd, parent-PGID, parent-PPID)
residual.  Unlike v6, that second class no longer requires the worker to be
*attached* to the exact owning parent by ppid or pgid: every child-label worker
running inside the repository working tree that is not yet the clean residual is
tracked by (pid, create_time) against the pinned parent, including a
consolidated-final worker mid-exit that has already been reparented to launchd
(ppid 1) with its own process group.  That exact case (a detached mop-final
worker mid-exit carrying the repo cwd) is what held v6.  A tracked worker is
tolerated only while it exits exactly or resolves into that clean residual; a
process that shares only a foreign cwd, carries a foreign label, or stays
inexact across the entire bounded window still fails closed to integrity_hold.
"""
...
CHAIN_ID = "generation1-successor-evidence-chain-v7"
PARENT_LABEL = "mop-successor-evidence-chain-v7"
STATE_SCHEMA = "mop-generation1-successor-evidence-chain-state/v7"
...
class SuccessorEvidenceChain(v3.SuccessorEvidenceChain):
    ...
    def _load_state(self) -> dict[str, Any]:
        ...
        state = _read_json(self.state_path, "successor chain v7 state")
        _validate_seal(state, "state_sha256", "successor chain v7 state")
        if state.get("schema") != STATE_SCHEMA or state.get("program_id") != CHAIN_ID:
            raise SuccessorChainRefused("successor chain v7 state identity drifted")
        ...
        if not isinstance(capsules, dict) or set(capsules) != expected_capsules:
            raise SuccessorChainRefused("successor chain v7 capsule inventory drifted")
    ...
    def _write_adoption_receipt(
        ...
        if path.exists():
            receipt = _read_json(path, f"{spec.stage_id} v7 adoption receipt")
            _validate_seal(receipt, "receipt_sha256", f"{spec.stage_id} v7 adoption receipt")
            if (
                ...
                raise SuccessorChainRefused(f"{spec.stage_id} v7 adoption receipt identity drifted")
        else:
    ...
    def _control_requested(self) -> bool:
        ...
        if not path.exists():
            return False
        value = _read_json(path, "successor chain v7 control")
        _validate_seal(value, "control_sha256", "successor chain v7 control")
        if (
            value.get("schema") != CONTROL_SCHEMA
            or value.get("program_id") != CHAIN_ID
            or value.get("action") != "drain"
        ):
            raise SuccessorChainRefused("successor chain v7 control identity drifted")
        return True
...
def validate_chain_status(
    ...
    *,
    ...
    if set(status) != STATUS_FIELDS:
        raise SuccessorChainRefused("successor chain v7 status fields drifted")
    _validate_seal(status, "status_sha256", "successor chain v7 status")
    state = status.get("state")
    problems = status.get("problems")
    if (
        ...
        or status.get("supersedes") != "generation1-successor-evidence-chain-v6"
        ...
        raise SuccessorChainRefused("successor chain v7 status identity or authority drifted")
    if (state == "complete" or state not in TERMINAL_STATES) and (
        status.get("execution_enabled") is not True or problems != []
    ):
        raise SuccessorChainRefused("successor chain v7 safe acknowledgement is not clean")
    ...
    if created_at is None or updated_at is None or created_at > updated_at:
        raise SuccessorChainRefused("successor chain v7 status timestamps drifted")
    if state in TERMINAL_STATES:
        if finished_at is None or finished_at > updated_at:
            raise SuccessorChainRefused("successor chain v7 terminal timestamp drifted")
    elif status.get("finished_at") is not None:
        raise SuccessorChainRefused("successor chain v7 nonterminal status is marked finished")
    supervisor = status.get("supervisor")
    ...
    if (
        ...
        raise SuccessorChainRefused("successor chain v7 supervisor authority drifted")
    capsules = status.get("capsules")
    ...
    if not isinstance(capsules, Mapping) or set(capsules) != expected_ids:
        raise SuccessorChainRefused("successor chain v7 status capsule inventory drifted")
    complete = 0
    for capsule_id, raw_row in capsules.items():
        if not isinstance(raw_row, Mapping) or set(raw_row) != CAPSULE_FIELDS:
            raise SuccessorChainRefused(f"successor chain v7 capsule {capsule_id} fields drifted")
        ...
        if (
            ...
            raise SuccessorChainRefused(f"successor chain v7 capsule {capsule_id} receipt shape drifted")
        if isinstance(process, Mapping):
            ...
            if (
                ...
                raise SuccessorChainRefused(
                    f"successor chain v7 capsule {capsule_id} process identity drifted"
                )
        if row_state == "complete":
            complete += 1
            if returncode != 0 or finished_at is None or not artifacts or last_problem is not None:
                raise SuccessorChainRefused(f"successor chain v7 capsule {capsule_id} completion drifted")
        elif (
            ...
            raise SuccessorChainRefused(f"successor chain v7 capsule {capsule_id} noncompletion drifted")
    expected_counts = {
        "complete": complete,
    ...
    if status.get("counts") != expected_counts:
        raise SuccessorChainRefused("successor chain v7 status counts drifted")
    ...
    if not coherent or (state not in TERMINAL_STATES and all_complete):
        raise SuccessorChainRefused("successor chain v7 state and capsule progression drifted")
    if state not in TERMINAL_STATES and any(row.get("status") == "failure_hold" for row in capsules.values()):
        raise SuccessorChainRefused("successor chain v7 safe status contains a failed capsule")
    return state
...
def _validate_complete_state_shape(
    ...
    if set(state) != STATE_FIELDS:
        raise SuccessorChainRefused("successor chain v7 state fields drifted")
    _validate_seal(state, "state_sha256", "successor chain v7 state")
    if (
        ...
        raise SuccessorChainRefused("successor chain v7 complete state boundary drifted")
    ...
    if (
        ...
        raise SuccessorChainRefused("successor chain v7 complete state timestamps drifted")
    ...
    if not isinstance(capsules, Mapping) or set(capsules) != expected_ids:
        raise SuccessorChainRefused("successor chain v7 complete capsule inventory drifted")
    for capsule_id, raw_row in capsules.items():
        if not isinstance(raw_row, Mapping) or set(raw_row) != CAPSULE_FIELDS:
            raise SuccessorChainRefused(f"successor chain v7 capsule {capsule_id} fields drifted")
        attempts = raw_row.get("attempts")
        if (
            ...
            raise SuccessorChainRefused(f"successor chain v7 capsule {capsule_id} completion receipt drifted")
        if capsule_id == "successor_horizon" and raw_row.get("adoption_receipts") != []:
            raise SuccessorChainRefused("successor chain v7 horizon capsule has unexpected adoption receipts")
    for spec in owner.specs:
        ...
        if len(receipt_paths) != len(set(receipt_paths)):
            raise SuccessorChainRefused(
                f"successor chain v7 {spec.stage_id} adoption receipt inventory contains duplicates"
            )
        ...
        for raw_path in receipt_paths:
            ...
            if relative.is_absolute() or ".." in relative.parts:
                raise SuccessorChainRefused(
                    f"successor chain v7 {spec.stage_id} adoption receipt path is unsafe"
                )
            ...
            if (
                ...
                raise SuccessorChainRefused(
                    f"successor chain v7 {spec.stage_id} adoption receipt is missing "
                    "or is not a canonical regular non-symlink file"
                )
            path = candidate.resolve()
            if not path.is_relative_to(receipt_root):
                raise SuccessorChainRefused(
                    f"successor chain v7 {spec.stage_id} adoption receipt path drifted"
                )
            file_stat = candidate.stat()
            file_identity = (file_stat.st_dev, file_stat.st_ino)
            if path in resolved_receipts or file_identity in receipt_files:
                raise SuccessorChainRefused(
                    f"successor chain v7 {spec.stage_id} adoption receipt resolves to a duplicate file"
                )
            ...
            if set(receipt) != ADOPTION_FIELDS:
                raise SuccessorChainRefused(
                    f"successor chain v7 {spec.stage_id} adoption receipt fields drifted"
                )
            _validate_seal(
                receipt,
                "receipt_sha256",
                f"{spec.stage_id} v7 adoption receipt",
            )
            ...
            if (
                ...
                raise SuccessorChainRefused(
                    f"successor chain v7 {spec.stage_id} adoption receipt authority drifted"
                )
            pid = process.get("pid")
            ...
            if (
                ...
                raise SuccessorChainRefused(
                    f"successor chain v7 {spec.stage_id} adopted process identity drifted"
                )
            if observed_status is not None and (
                ...
                raise SuccessorChainRefused(
                    f"successor chain v7 {spec.stage_id} observed status authority drifted"
                )
            identity_sha = canonical_sha256(process)[:16]
            if candidate.name != f"{process.get('pid')}-{identity_sha}.json":
                raise SuccessorChainRefused(
                    f"successor chain v7 {spec.stage_id} adoption receipt filename drifted"
                )
            validated_processes.append(process)
        if not validated_processes and row.get("process") is not None:
            raise SuccessorChainRefused(
                f"successor chain v7 {spec.stage_id} has process identity without an adoption receipt"
            )
        if validated_processes and row.get("process") != validated_processes[-1]:
            raise SuccessorChainRefused(f"successor chain v7 {spec.stage_id} adopted process history drifted")
...
def _read_stable_state_status(
    ...
    for attempt in range(SNAPSHOT_ATTEMPTS):
        before = _read_json(state_path, "successor chain v7 state")
        first_status = _read_json(status_path, "successor chain v7 status")
        after = _read_json(state_path, "successor chain v7 state")
        second_status = _read_json(status_path, "successor chain v7 status")
        try:
    ...
    raise SuccessorChainRefused("successor chain v7 state/status snapshot did not become coherent")
...
def _validate_bound_artifact_hashes(
    ...
    for capsule_id, row in capsules.items():
        if not isinstance(row, Mapping):
            raise SuccessorChainRefused(f"successor chain v7 capsule {capsule_id} is not an object")
        artifacts = row.get("artifacts")
        if not isinstance(artifacts, list):
            raise SuccessorChainRefused(f"successor chain v7 capsule {capsule_id} artifacts drifted")
        for report in artifacts:
            if not isinstance(report, Mapping):
                raise SuccessorChainRefused(
                    f"successor chain v7 capsule {capsule_id} artifact report drifted"
                )
            raw_path = report.get("path")
            if not isinstance(raw_path, str) or not raw_path:
                raise SuccessorChainRefused(f"successor chain v7 capsule {capsule_id} artifact path drifted")
            relative = Path(raw_path)
            ...
            if (
                ...
                raise SuccessorChainRefused(
                    f"successor chain v7 capsule {capsule_id} artifact path is unsafe"
                )
            path = candidate.resolve()
            if not path.is_relative_to(repo_root) or report.get("sha256") != sha256_file(path):
                raise SuccessorChainRefused(
                    f"successor chain v7 capsule {capsule_id} final artifact hash drifted"
                )
...
def read_validated_complete_chain_status(
    *,
    ...
    if not root.is_dir() or not root.is_relative_to(repo_root):
        raise SuccessorChainRefused("successor chain v7 root is absent or outside the repository")
    if acquire_lock:
        for attempt in range(LOCK_ATTEMPTS):
            try:
                ...
                if attempt + 1 < LOCK_ATTEMPTS:
                    ...
                    continue
                raise SuccessorChainRefused(
                    "successor chain v7 lifetime lock remained held while validating a complete snapshot"
                ) from exc
        raise SuccessorChainRefused("successor chain v7 lifetime lock acquisition exhausted")
    if horizon_program_path is None:
        relative = DEFAULT_HORIZON_PROGRAM.resolve().relative_to(REPO_ROOT.resolve())
        horizon_program_path = repo_root / relative
    horizon_program_path = horizon_program_path.resolve()
    ...
    _validate_seal(stable_status, "status_sha256", "successor chain v7 status")
    configured_specs = tuple(specs) if specs is not None else legacy_specs(repo_root)
    owner = SuccessorEvidenceChain(
        root=root,
        repo_root=repo_root,
    ...
    if validated_state != "complete":
        raise SuccessorChainRefused("successor chain v7 durable status is not complete")
    ...
    if owner.state["capsules"] != before_capsules:
        raise SuccessorChainRefused(
            "successor chain v7 evidence required reconciliation during terminal replay"
        )
    ...
    if current_state != stable_state or current_status != stable_status:
        raise SuccessorChainRefused("successor chain v7 state/status changed during terminal replay")
    owner._validate_completed_chain_artifacts(())
    if owner.state["capsules"] != before_capsules:
        raise SuccessorChainRefused("successor chain v7 evidence required reconciliation during final replay")
    ...
    if final_state != stable_state or final_status != stable_status:
        raise SuccessorChainRefused("successor chain v7 state/status changed during final evidence replay")
...
def read_chain_status(root: Path = DEFAULT_ROOT) -> dict[str, Any]:
    ...
    status = _read_json(path, "successor chain v7 status")
    _validate_seal(status, "status_sha256", "successor chain v7 status")
    if status.get("schema") != STATUS_SCHEMA or status.get("program_id") != CHAIN_ID:
        raise SuccessorChainRefused("successor chain v7 status identity drifted")
    return status
...
def _visible_parent_process(entrypoint: Path) -> ProcessSnapshot | None:
    ...
    for process in candidates:
        if (
            ...
            raise SuccessorChainRefused("visible successor chain v7 parent has inexact metadata")
    ...
    if len(labelled) > 1 or len(groups) > 1:
        raise SuccessorChainRefused("multiple successor chain v7 parent processes are visible")
    if labelled:
        return labelled[0]
...
def _revalidate_terminal_chain(root: Path) -> dict[str, Any]:
    with FileLock(root / LOCK_FILE):
        ...
        if owner.state.get("status") not in TERMINAL_STATES:
            raise SuccessorChainRefused(
                "sealed terminal status disagrees with the durable successor chain v7 state"
            )
...
def start_chain_detached(
    *,
    ...
    if not execute:
        raise SuccessorChainRefused("detached successor chain v7 start requires explicit --execute")
    root = root.resolve()
    root.mkdir(parents=True, exist_ok=True)
    with FileLock(root / START_LOCK_FILE):
        entrypoint = (REPO_ROOT / "scripts/mop_generation1_successor_chain_v7.py").resolve()
        status_path = root / STATUS_FILE
        status = None
        ...
        if _process_identity_alive(status_identity):
            if visible is None:
                raise SuccessorChainRefused("sealed successor chain v7 identity has no exact visible parent")
            if (
                ...
                raise SuccessorChainRefused(
                    "sealed successor chain v7 identity conflicts with visible parent"
                )
            if status is None:
                raise SuccessorChainRefused("live successor chain v7 identity lacks a sealed status")
            try:
        ...
        if acknowledged is None:
            raise SuccessorChainRefused(
                f"detached successor chain v7 did not acknowledge startup; inspect {stderr_path}"
            )
        return {
    ...
    try:
        if arguments.command == "run":
            if not arguments.execute:
                raise SuccessorChainRefused("successor chain v7 execution requires explicit --execute")
