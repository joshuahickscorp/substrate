"""Append-only waiter for the categorized Generation 1 successor batch wave.

This parent never signals or restarts the predecessor.  It observes an exact,
clean, complete ``generation1-successor-horizon-v2`` generic-supervisor
snapshot, checks current host admission, and only then starts or resumes the
sealed categorized batch program through the generic Generation 1 supervisor.
"""
...
PROGRAM_ID = "generation1-categorized-batch-extension-chain-v1"
PARENT_LABEL = "mop-categorized-batch-extension-chain"
STATE_SCHEMA = "mop-generation1-categorized-batch-extension-chain-state/v1"
STATUS_SCHEMA = "mop-generation1-categorized-batch-extension-chain-status/v1"

PREDECESSOR_PROGRAM_ID = "generation1-successor-horizon-v2"
TARGET_PROGRAM_ID = "generation1-successor-categorized-batch-wave-v1"

DEFAULT_ROOT = REPO_ROOT / "runs/generation1" / PROGRAM_ID
DEFAULT_PREDECESSOR_PROGRAM = REPO_ROOT / "configs/campaign/generation1_successor_horizon_v2.json"
DEFAULT_PREDECESSOR_STATUS = (
    REPO_ROOT / "runs/generation1/generation1-successor-horizon-v2/current_status.json"
)
DEFAULT_TARGET_PROGRAM = REPO_ROOT / "configs/campaign/generation1_successor_categorized_batch_wave_v1.json"
...
def _read_json(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise CategorizedBatchExtensionRefused(f"{label} is missing: {path}") from exc
    if path.is_symlink() or not path.is_file() or not 0 < stat.st_size <= MAX_JSON_BYTES:
        raise CategorizedBatchExtensionRefused(f"{label} is not a safe bounded regular file: {path}")
    raw = path.read_bytes()
    if len(raw) != stat.st_size:
        raise CategorizedBatchExtensionRefused(f"{label} changed while being read: {path}")
    try:
        payload = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise CategorizedBatchExtensionRefused(f"{label} is not valid JSON: {path}") from exc
    if not isinstance(payload, dict):
        raise CategorizedBatchExtensionRefused(f"{label} must be a JSON object: {path}")
    return payload
...
def _validate_seal(payload: Mapping[str, Any], field: str, label: str) -> None:
    ...
    if not isinstance(declared, str) or declared != canonical_sha256(core):
        raise CategorizedBatchExtensionRefused(f"{label} self-seal is invalid")


def _require_fields(value: object, fields: frozenset[str], label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise CategorizedBatchExtensionRefused(f"{label} must be an object")
    if set(value) != fields:
...
def _safe_current_file(repo_root: Path, raw_path: object, label: str) -> Path:
    if not isinstance(raw_path, str) or not raw_path:
        raise CategorizedBatchExtensionRefused(f"{label} path must be repository-relative")
    relative = Path(raw_path)
    if relative.is_absolute() or ".." in relative.parts:
        raise CategorizedBatchExtensionRefused(f"{label} path is unsafe")
    candidate = repo_root / relative
    cursor = repo_root
    for component in relative.parts:
        cursor = cursor / component
        if cursor.is_symlink():
            raise CategorizedBatchExtensionRefused(f"{label} path traverses a symlink")
    resolved = candidate.resolve()
    if not resolved.is_relative_to(repo_root) or not resolved.is_file():
        raise CategorizedBatchExtensionRefused(f"{label} is not a safe current file")
    return resolved
...
def _program_authority(
    ...
    *,
    ...
    if program.program_id != expected_program_id:
        raise CategorizedBatchExtensionRefused(f"{expected_program_id} manifest identity drifted")
...
def validate_generic_status(
    ...
    *,
    ...
    if execution_enabled is not True and not (state == "execution_disabled" and execution_enabled is False):
        raise CategorizedBatchExtensionRefused(f"{program.program_id} execution authority drifted")
    ...
    if (
        ...
        raise CategorizedBatchExtensionRefused(f"{program.program_id} status metadata drifted")
    ...
    if not isinstance(problems, list) or any(
        ...
        raise CategorizedBatchExtensionRefused(f"{program.program_id} problem inventory drifted")
    ...
    if (
        ...
        raise CategorizedBatchExtensionRefused(f"{program.program_id} queue or injection authority drifted")
    ...
    if not isinstance(capsules, Mapping) or set(capsules) != set(expected_capsules):
        raise CategorizedBatchExtensionRefused(f"{program.program_id} capsule inventory drifted")
    current_capsule = status.get("current_capsule")
    if current_capsule is not None and current_capsule not in expected_capsules:
        raise CategorizedBatchExtensionRefused(f"{program.program_id} active capsule drifted")
    ...
    if state == "complete" and (
        ...
        raise CategorizedBatchExtensionRefused(f"completed {program.program_id} status is not clean")
    return state
...
def _stable_complete_generic_status(
    ...
    *,
        ...
        for attempt in range(LOCK_ATTEMPTS):
            try:
                ...
                if not isinstance(exc.__cause__, BlockingIOError):
                    raise CategorizedBatchExtensionRefused(str(exc)) from exc
    ...
    raise CategorizedBatchExtensionRefused(f"{program.program_id} complete status did not become stable")
...
def _default_host_admission(
    ...
    try:
        ...
        raise CategorizedBatchExtensionRefused(
            f"categorized host admission failed closed: {type(exc).__name__}: {exc}"
        ) from exc
...
def _admission_summary(
    ...
    *,
    ...
    if allowed is not True and allowed is not False:
        raise CategorizedBatchExtensionRefused("categorized host admission returned no exact decision")
    return {
        "allowed": allowed,
...
class CategorizedBatchExtensionChain:
    ...
    def __init__(
        ...
        *,
        ...
        for label, path in (
            ("categorized extension root", self.root),
            ...
            if not path.is_relative_to(self.repo_root):
                raise CategorizedBatchExtensionRefused(f"{label} must remain inside the repository")
    ...
    def _load_program(self, path: Path, expected_id: str) -> Program:
        ...
        if program.program_id != expected_id:
            raise CategorizedBatchExtensionRefused(f"loaded {expected_id} program identity drifted")
        return program
    ...
    def _validate_authorities(self, state: Mapping[str, Any]) -> None:
        if state.get("parent_implementation") != _implementation_authority(self.repo_root):
            raise CategorizedBatchExtensionRefused("categorized extension implementation authority drifted")
        if state.get("predecessor_program") != self._predecessor_authority():
            raise CategorizedBatchExtensionRefused("categorized extension predecessor authority drifted")
        if state.get("target_program") != self._target_authority():
            raise CategorizedBatchExtensionRefused("categorized extension target authority drifted")
    ...
    def _load_state(self) -> dict[str, Any]:
        ...
        state = _read_json(self.state_path, "categorized extension state")
        _validate_seal(state, "state_sha256", "categorized extension state")
        if set(state) != STATE_FIELDS:
            raise CategorizedBatchExtensionRefused("categorized extension state fields drifted")
        if state.get("schema") != STATE_SCHEMA or state.get("program_id") != PROGRAM_ID:
            raise CategorizedBatchExtensionRefused("categorized extension state identity drifted")
        ...
        if not isinstance(capsules, Mapping) or set(capsules) != {
            ...
            raise CategorizedBatchExtensionRefused("categorized extension capsule inventory drifted")
    ...
    def _reconcile_predecessor(self, row: dict[str, Any]) -> bool:
        ...
        if not program.status_path.exists():
            if row.get("status") == "complete":
                raise CategorizedBatchExtensionRefused("completed V2 predecessor evidence disappeared")
            row["status"] = "pending"
            row["last_problem"] = "waiting for exact clean successor-horizon-v2 completion"
            return False
        raw = _read_json(program.status_path, "successor horizon v2 status")
        state = validate_generic_status(program, raw)
        if state != "complete":
            if row.get("status") == "complete":
                raise CategorizedBatchExtensionRefused("completed V2 predecessor evidence regressed")
            ...
            row["last_problem"] = f"observed successor horizon v2 state {state}; signals remain forbidden"
            return False
        ...
        if row.get("status") == "complete":
            if (
                ...
                raise CategorizedBatchExtensionRefused("completed V2 predecessor binding drifted")
            return True
        row.update(
            {
                "status": "complete",
    ...
    def _target_status(self, program: Program) -> dict[str, Any] | None:
        ...
        try:
            ...
            raise CategorizedBatchExtensionRefused(
                f"categorized target status is invalid: {type(exc).__name__}: {exc}"
            ) from exc
    ...
    def _visible_target_supervisors(self, program: Program) -> tuple[ProcessSnapshot, ...]:
        ...
        if any(
            ...
            raise CategorizedBatchExtensionRefused("visible categorized supervisor has inexact metadata")
        if len(visible) > 1:
            raise CategorizedBatchExtensionRefused("multiple categorized generic supervisors are visible")
        return visible
    ...
    def _record_admission(
        ...
        if not isinstance(receipt, Mapping):
            raise CategorizedBatchExtensionRefused("categorized host admission returned no receipt")
    ...
    def _reconcile_target(self, row: dict[str, Any], program: Program) -> dict[str, Any]:
        ...
        if row.get("status") == "complete":
            if status is None:
                raise CategorizedBatchExtensionRefused("completed categorized target status disappeared")
            complete = _stable_complete_generic_status(program)
            ...
            if visible and (
                ...
                raise CategorizedBatchExtensionRefused(
                    "completed categorized target conflicts with visible supervisor"
                )
            expected = [_artifact(self.repo_root, program.status_path, complete)]
            if (
                row.get("returncode") != 0
                ...
                raise CategorizedBatchExtensionRefused("completed categorized target binding drifted")
            return row
        if status is not None:
            ...
            if state in UNSAFE_TERMINAL_STATES:
                row["status"] = "failure_hold"
                row["finished_at"] = _iso(self.now_fn())
                row["last_problem"] = f"categorized target entered {state}"
                return row
            supervisor = status.get("supervisor")
            if isinstance(supervisor, Mapping):
                identity_state = self.identity_probe_fn(supervisor)
                if identity_state == "unknown":
                    raise CategorizedBatchExtensionRefused("categorized supervisor identity is inexact")
                if identity_state == "alive":
                    if len(visible) != 1 or not _snapshot_matches(visible[0], supervisor):
                        raise CategorizedBatchExtensionRefused(
                            "sealed categorized supervisor conflicts with visible process"
                        )
                    row["status"] = "running"
                    row["process"] = dict(supervisor)
                    row["started_at"] = row.get("started_at") or _iso(self.now_fn())
                    row["last_problem"] = f"categorized generic supervisor is in state {state}"
                    return row
        if visible:
            row["status"] = "adoption_wait"
            row["process"] = visible[0].identity()
            row["last_problem"] = (
                "exact categorized supervisor is visible while sealed acknowledgement is pending"
            )
            return row
        if not self.execute:
            row["status"] = "pending"
            row["last_problem"] = "categorized target start requires explicit --execute"
            return row
        ...
        with FileLock(predecessor.program_root / GENERIC_LOCK_FILE):
            ...
            row["last_problem"] = "sealed categorized launch intent; replaying all gates"
            self.state["status"] = "waiting_target"
            self.state["finished_at"] = None
            self._publish()
            row = self.state["capsules"]["categorized_batch_wave_v1"]
            after = _stable_complete_generic_status(predecessor, acquire_lock=False)
            if before != after:
                raise CategorizedBatchExtensionRefused(
                    "V2 predecessor changed across categorized launch intent"
                )
            if not self._record_admission(program, status):
                row["status"] = "pending"
                row["last_problem"] = (
            ...
            try:
                ...
                row["last_problem"] = (
                    f"categorized startup acknowledgement pending: {type(exc).__name__}: {exc}"
                )
                return row
            if not isinstance(launched, Mapping):
                raise CategorizedBatchExtensionRefused("categorized generic starter returned no receipt")
            returned = launched.get("status")
            if not isinstance(returned, Mapping):
                row["status"] = "retry_wait"
                row["last_problem"] = "categorized starter returned no sealed status acknowledgement"
                return row
            returned_state = validate_generic_status(program, returned)
            if returned_state in UNSAFE_TERMINAL_STATES:
                row["status"] = "failure_hold"
                row["finished_at"] = _iso(self.now_fn())
                row["last_problem"] = f"categorized target acknowledged unsafe state {returned_state}"
                return row
            ...
            row["status"] = "running"
            row["started_at"] = row.get("started_at") or _iso(self.now_fn())
            row["last_problem"] = "categorized generic supervisor start accepted"
    ...
    def _validate_terminal_artifacts(self, target_program: Program) -> None:
        predecessor = self.state["capsules"]["successor_horizon_v2"]
        if predecessor.get("status") == "complete":
            self._reconcile_predecessor(predecessor)
        target = self.state["capsules"]["categorized_batch_wave_v1"]
    ...
    def tick(self) -> dict[str, Any]:
        try:
            ...
            predecessor_row = self.state["capsules"]["successor_horizon_v2"]
            ...
            if not predecessor_complete:
                ...
                return self._publish()
            target_row = self.state["capsules"]["categorized_batch_wave_v1"]
...
def _validate_status_authorities(
    ...
    *,
    ...
    if status.get("parent_implementation") != _implementation_authority(repo_root):
        raise CategorizedBatchExtensionRefused(
            "categorized extension sealed implementation authority drifted"
        )
    if status.get("predecessor_program") != _program_authority(
        ...
        raise CategorizedBatchExtensionRefused("categorized extension sealed predecessor authority drifted")
    if status.get("target_program") != _program_authority(
        target_program_path,
        repo_root=repo_root,
        expected_program_id=TARGET_PROGRAM_ID,
    ):
        raise CategorizedBatchExtensionRefused("categorized extension sealed target authority drifted")


def validate_categorized_extension_status(
    status: Mapping[str, Any],
    *,
    repo_root: Path = REPO_ROOT,
    predecessor_program_path: Path = DEFAULT_PREDECESSOR_PROGRAM,
    ...
    _require_fields(status, STATUS_FIELDS, "categorized extension status")
    _validate_seal(status, "status_sha256", "categorized extension status")
    state = status.get("state")
    problems = status.get("problems")
    if (
        ...
        raise CategorizedBatchExtensionRefused("categorized extension status identity or safety drifted")
    if (state == "complete" or state not in TERMINAL_STATES) and (
        status.get("execution_enabled") is not True or problems != []
    ):
        raise CategorizedBatchExtensionRefused("categorized extension safe acknowledgement is not clean")
    ...
    if created is None or updated is None or created > updated:
        raise CategorizedBatchExtensionRefused("categorized extension timestamps drifted")
    if state in TERMINAL_STATES:
        if finished is None or finished > updated:
            raise CategorizedBatchExtensionRefused("categorized extension terminal timestamp drifted")
    elif status.get("finished_at") is not None:
        raise CategorizedBatchExtensionRefused("categorized extension nonterminal status is marked finished")
    supervisor = status.get("supervisor")
    ...
    if (
        ...
        raise CategorizedBatchExtensionRefused("categorized extension supervisor authority drifted")
    ...
    if last_admission is not None and (
        ...
        raise CategorizedBatchExtensionRefused("categorized extension admission summary drifted")
    capsules = status.get("capsules")
    expected_ids = {"successor_horizon_v2", "categorized_batch_wave_v1"}
    if not isinstance(capsules, Mapping) or set(capsules) != expected_ids:
        raise CategorizedBatchExtensionRefused("categorized extension capsule inventory drifted")
    completed = 0
    for capsule_id, row in capsules.items():
        _require_fields(row, ROW_FIELDS, f"categorized extension capsule {capsule_id}")
        row_state = row.get("status")
        attempts = row.get("attempts")
        ...
        if (
            ...
            raise CategorizedBatchExtensionRefused(
                f"categorized extension capsule {capsule_id} receipt drifted"
            )
        if isinstance(process, Mapping):
            ...
            if (
                ...
                raise CategorizedBatchExtensionRefused(
                    f"categorized extension capsule {capsule_id} process identity drifted"
                )
        if row_state == "complete":
            completed += 1
            if returncode != 0 or finished_at is None or not artifacts or last_problem is not None:
                raise CategorizedBatchExtensionRefused(
                    f"categorized extension capsule {capsule_id} completion drifted"
                )
        elif (
            ...
            raise CategorizedBatchExtensionRefused(
                f"categorized extension capsule {capsule_id} noncompletion drifted"
            )
    counts = {
        "complete": completed,
    ...
    if status.get("counts") != counts:
        raise CategorizedBatchExtensionRefused("categorized extension counts drifted")
    predecessor_state = capsules["successor_horizon_v2"].get("status")
    target_state = capsules["categorized_batch_wave_v1"].get("status")
    pristine_target = capsules["categorized_batch_wave_v1"] == _empty_row()
    ...
    if not coherent or (state not in TERMINAL_STATES and all_complete):
        raise CategorizedBatchExtensionRefused("categorized extension state and capsule progression drifted")
    completed_bindings = (
        (
            "successor_horizon_v2",
            predecessor_program_path,
    ...
    for capsule_id, program_path, expected_program_id in completed_bindings:
        ...
        if capsules[capsule_id].get("artifacts") != expected_artifacts:
            raise CategorizedBatchExtensionRefused(
                f"categorized extension capsule {capsule_id} completion binding drifted"
            )
    return state
...
def _read_stable_state_status(root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    for attempt in range(SNAPSHOT_ATTEMPTS):
        before = _read_json(root / STATE_FILE, "categorized extension state")
        first = _read_json(root / STATUS_FILE, "categorized extension status")
        after = _read_json(root / STATE_FILE, "categorized extension state")
        second = _read_json(root / STATUS_FILE, "categorized extension status")
        try:
        ...
        if attempt + 1 < SNAPSHOT_ATTEMPTS:
            time.sleep(SNAPSHOT_INTERVAL_SECONDS)
    raise CategorizedBatchExtensionRefused("categorized extension state/status did not become coherent")


def read_validated_complete_categorized_extension_status(
    *,
    root: Path = DEFAULT_ROOT,
    repo_root: Path = REPO_ROOT,
    predecessor_program_path: Path = DEFAULT_PREDECESSOR_PROGRAM,
    ...
    if not root.is_dir() or not root.is_relative_to(repo_root):
        raise CategorizedBatchExtensionRefused(
            "categorized extension root is absent or outside the repository"
        )
    if acquire_lock:
        for attempt in range(LOCK_ATTEMPTS):
            try:
                with FileLock(root / LOCK_FILE):
                    return read_validated_complete_categorized_extension_status(
                        root=root,
                        repo_root=repo_root,
                        predecessor_program_path=predecessor_program_path,
                        target_program_path=target_program_path,
                ...
                if attempt + 1 < LOCK_ATTEMPTS:
                    ...
                    continue
                raise CategorizedBatchExtensionRefused(
                    "categorized extension lifetime lock remained held"
                ) from exc
    state, status = _read_stable_state_status(root)
    if set(state) != STATE_FIELDS:
        raise CategorizedBatchExtensionRefused("categorized extension complete state fields drifted")
    _validate_seal(state, "state_sha256", "categorized extension state")
    if (
        ...
        raise CategorizedBatchExtensionRefused("categorized extension complete state boundary drifted")
    if (
        ...
        raise CategorizedBatchExtensionRefused("categorized extension durable status is not complete")
    ...
    expected = {
        "successor_horizon_v2": [_artifact(repo_root, predecessor.status_path, predecessor_status)],
        "categorized_batch_wave_v1": [_artifact(repo_root, target.status_path, target_status)],
    }
    for capsule_id, artifacts in expected.items():
        if state["capsules"][capsule_id].get("artifacts") != artifacts:
            raise CategorizedBatchExtensionRefused(
                f"categorized extension capsule {capsule_id} terminal binding drifted"
            )
    final_state, final_status = _read_stable_state_status(root)
    if final_state != state or final_status != status:
        raise CategorizedBatchExtensionRefused("categorized extension changed during terminal replay")
    return status


def read_categorized_extension_status(root: Path = DEFAULT_ROOT) -> dict[str, Any]:
    status = _read_json(root.resolve() / STATUS_FILE, "categorized extension status")
    _validate_seal(status, "status_sha256", "categorized extension status")
    return status
...
def _visible_parent(entrypoint: Path) -> ProcessSnapshot | None:
    ...
    if any(
        ...
        raise CategorizedBatchExtensionRefused("visible categorized extension parent has inexact metadata")
    ...
    if len(labelled) > 1 or len(groups) > 1:
        raise CategorizedBatchExtensionRefused("multiple categorized extension parents are visible")
    if labelled:
        return labelled[0]
    return next(
...
def start_categorized_extension_detached(
    *,
    root: Path = DEFAULT_ROOT,
    execute: bool,
    ...
    if not execute:
        raise CategorizedBatchExtensionRefused(
            "detached categorized extension start requires explicit --execute"
        )
    root = root.resolve()
    root.mkdir(parents=True, exist_ok=True)
    with FileLock(root / START_LOCK_FILE):
        entrypoint = (REPO_ROOT / "scripts/mop_generation1_categorized_batch_extension_chain.py").resolve()
        status = None
        if (root / STATUS_FILE).is_file():
            status = read_categorized_extension_status(root)
            validate_categorized_extension_status(status)
        ...
        if identity_state == "unknown":
            raise CategorizedBatchExtensionRefused("sealed categorized extension parent identity is inexact")
        if identity_state == "alive":
            if (
                visible is None
                ...
                raise CategorizedBatchExtensionRefused(
                    "sealed categorized extension identity conflicts with visible parent"
                )
        ...
        if status is not None and status.get("state") in TERMINAL_STATES:
            if status.get("state") == "complete":
                status = read_validated_complete_categorized_extension_status(root=root)
        ...
        while time.monotonic() < deadline:
            if status_path.is_file() and status_path.stat().st_mtime_ns > prior_mtime:
                try:
                    acknowledged = read_categorized_extension_status(root)
                    validate_categorized_extension_status(acknowledged)
                    break
                except (CategorizedBatchExtensionRefused, OSError, ValueError):
                    acknowledged = None
        ...
        if acknowledged is None:
            raise CategorizedBatchExtensionRefused(
                f"detached categorized extension did not acknowledge startup; inspect {stderr_path}"
            )
        visible = _visible_parent(entrypoint)
        ...
        if (
            ...
            raise CategorizedBatchExtensionRefused(
                "detached categorized extension acknowledgement conflicts with visible parent"
            )
        if _default_identity_probe(visible.identity()) != "alive":
            raise CategorizedBatchExtensionRefused(
                "detached categorized extension visible parent is not provably alive"
            )
        return {
    ...
    try:
        if arguments.command == "run":
            if not arguments.execute:
                raise CategorizedBatchExtensionRefused(
                    "categorized extension execution requires explicit --execute"
                )
            set_process_label(PARENT_LABEL)
            payload = CategorizedBatchExtensionChain(
                root=arguments.root,
                execute=True,
            ).run(max_cycles=1 if arguments.once else None)
        elif arguments.command == "start":
            payload = start_categorized_extension_detached(
                root=arguments.root,
                execute=arguments.execute,
                use_caffeinate=not arguments.no_caffeinate,
            )
        else:
            payload = read_categorized_extension_status(arguments.root)
...
__all__ = [
    "CategorizedBatchExtensionChain",
    "CategorizedBatchExtensionRefused",
    ...
    "build_parser",
    "main",
    "read_categorized_extension_status",
