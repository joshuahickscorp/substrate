...
``generation1-successor-evidence-chain-v4`` parent.  It binds its own
implementation and the exact sealed v2 horizon manifest, waits for the v4
...
PROGRAM_ID = "generation1-successor-extension-chain-v1"
PARENT_LABEL = "mop-successor-extension-chain"
STATE_SCHEMA = "mop-generation1-successor-extension-chain-state/v1"
STATUS_SCHEMA = "mop-generation1-successor-extension-chain-status/v1"

PREDECESSOR_PROGRAM_ID = "generation1-successor-evidence-chain-v4"
PREDECESSOR_STATUS_SCHEMA = "mop-generation1-successor-evidence-chain-status/v4"
...
DEFAULT_PREDECESSOR_STATUS = (
    REPO_ROOT / "runs/generation1/generation1-successor-evidence-chain-v4/current_status.json"
)
...
class SuccessorExtensionChain:
    ...
    def _initial_state(self) -> dict[str, Any]:
        ...
        return {
            ...
            "capsules": {
                "predecessor_chain_v4": _empty_row(),
    ...
    def _validate_predecessor_artifact(
        ...
        value = _require_exact_fields(
            ...
            f"v4 predecessor capsule {capsule_id} artifact report",
        )
        path, payload = self._current_artifact_payload(
            value["path"],
            f"v4 predecessor capsule {capsule_id} artifact",
        )
        if (
            ...
            raise SuccessorExtensionRefused(f"v4 predecessor capsule {capsule_id} artifact report drifted")

    def _validate_predecessor_status(self, value: Mapping[str, Any]) -> None:
        _require_exact_fields(value, PREDECESSOR_STATUS_FIELDS, "v4 predecessor status")
        ...
        if (
            ...
            or value.get("supersedes") != "generation1-successor-evidence-chain-v3"
            or not isinstance(value.get("execution_enabled"), bool)
            or not isinstance(state, str)
            or state not in PREDECESSOR_STATES
        ):
            raise SuccessorExtensionRefused("v4 predecessor status identity, safety, or state drifted")
        if (
            ...
            raise SuccessorExtensionRefused("v4 predecessor status metadata drifted")
        problems = _validate_problem_list(value.get("problems"), "v4 predecessor status")
        capsules = value.get("capsules")
        if not isinstance(capsules, Mapping) or set(capsules) != set(PREDECESSOR_CAPSULE_IDS):
            raise SuccessorExtensionRefused("v4 predecessor status capsule inventory drifted")
        completed = 0
        for capsule_id in PREDECESSOR_CAPSULE_IDS:
            row = _require_exact_fields(
                capsules[capsule_id],
                PREDECESSOR_CAPSULE_FIELDS,
                f"v4 predecessor capsule {capsule_id}",
            )
            row_state = row.get("status")
            if not isinstance(row_state, str) or row_state not in PREDECESSOR_CAPSULE_STATES:
                raise SuccessorExtensionRefused(f"v4 predecessor capsule {capsule_id} state drifted")
            attempts = row.get("attempts")
            if isinstance(attempts, bool) or not isinstance(attempts, int) or attempts < 0:
                raise SuccessorExtensionRefused(f"v4 predecessor capsule {capsule_id} attempts drifted")
        ...
        if value.get("counts") != expected_counts:
            raise SuccessorExtensionRefused("v4 predecessor status counts drifted")
        if state != "complete":
            return
        if (
            problems != []
            ...
            raise SuccessorExtensionRefused("completed v4 predecessor status is not clean")
        for capsule_id in PREDECESSOR_CAPSULE_IDS:
            ...
            if (
                ...
                raise SuccessorExtensionRefused(
                    f"completed v4 predecessor capsule {capsule_id} receipt drifted"
                )
    ...
    def _reconcile_predecessor(self, row: dict[str, Any]) -> bool:
        ...
        if status is None:
            row["status"] = "pending"
            row["last_problem"] = "waiting for the sealed v4 chain status"
            return False
        state = str(status.get("state"))
        row["last_problem"] = f"observed v4 chain state {state}; signals remain forbidden"
        if state == "complete":
    ...
    def tick(self) -> dict[str, Any]:
        try:
            ...
            if predecessor.get("status") == "failure_hold":
                ...
                self._append_problem(str(predecessor.get("last_problem") or "v4 predecessor failed"))
                return self._publish()
...
def start_extension_detached(
    *,
    ...
    with FileLock(root / START_LOCK_FILE):
        entrypoint = (REPO_ROOT / "scripts/mop_generation1_successor_extension_chain.py").resolve()
        status_path = root / STATUS_FILE
        status = None
