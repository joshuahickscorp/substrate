...
The Studio Wave-0 run emits several receipts: transfer check, daemon state, encode device, and encode
schedule. This module turns them into one JSON summary plus a bounded Markdown block for
STUDIO_RUN_REPORT.md, so the report records actual s/clip and memory envelope without hand editing.
"""
