"""One-command recovery launcher (v2) for the append-only Generation 1 successor chain.

The v6 adopter went integrity_hold on the same detached-worker failure class that
killed v5, and the extension chain v3 observes that held v6 chain; the launcher v1
that started v6 is sealed and immutable.  This v2 launcher is the append-only
recovery front door: it idempotently starts (or resumes) the hardened v7 adopter
and then the fresh extension chain v4 waiter (which observes v7), each detached
under the acknowledged-waiter pattern, refusing to duplicate a live parent.  It
observes only its two successors' own roots and never signals, edits, or reads the
sealed v5/v6/extv2/extv3 held state or the live legacy campaign.
"""
...
from mop.studio.generation1_successor_extension_chain_v4 import (
...
PROGRAM_ID = "generation1-successor-chain-recovery-v2"
RECEIPT_SCHEMA = "mop-generation1-successor-chain-recovery-start/v2"
...
def _validated_status_state(
    ...
    *,
    ...
    try:
        ...
        raise SuccessorChainRecoveryRefused(
...
def _replay_complete_adopter(status: Mapping[str, Any]) -> None:
    try:
        ...
        raise SuccessorChainRecoveryRefused(
            f"v7 adopter complete acknowledgement failed durable replay: {type(exc).__name__}: {exc}"
        ) from exc
    if dict(status) != replayed:
        raise SuccessorChainRecoveryRefused(
            "v7 adopter complete acknowledgement disagrees with durable replay"
        )
...
def _replay_complete_extension(status: Mapping[str, Any]) -> None:
    ...
    if dict(status) != replayed:
        raise SuccessorChainRecoveryRefused(
            "extension complete acknowledgement disagrees with durable replay"
        )
...
def _start_until_acknowledged(
    *,
    ...
    if ack_attempts < 1:
        raise SuccessorChainRecoveryRefused("acknowledgement attempts must be positive")
    ...
    for attempt in range(1, ack_attempts + 1):
        ...
        if not isinstance(raw_result, Mapping):
            raise SuccessorChainRecoveryRefused(f"{label} starter returned no receipt")
        ...
        except SuccessorChainRecoveryRefused:
            if pending_observed is not None and attempt < ack_attempts:
                sleep_fn(ack_interval_seconds)
                continue
            raise
        return result, state, attempt
    raise SuccessorChainRecoveryRefused(f"{label} acknowledgement remained pending")
...
def start_recovery(
    *,
    ...
    """Idempotently start or resume the v7 adopter, then the extension v4 waiter.
    ...
    acknowledgements rather than launching a second supervisor.  The held
    v5/v6/extv2/extv3 runs are never signalled, edited, or read.
    """
    ...
    if adopter_state in UNSAFE_TERMINAL_STATES:
        raise SuccessorChainRecoveryRefused(f"v7 adopter is terminal in unsafe state {adopter_state}")
    ...
    if extension_state in UNSAFE_TERMINAL_STATES:
        raise SuccessorChainRecoveryRefused(f"extension is terminal in unsafe state {extension_state}")
    ...
    return {
        ...
        "extension": extension,
        "start_order": [
            "generation1-successor-evidence-chain-v7",
            "generation1-successor-extension-chain-v4",
        ],
