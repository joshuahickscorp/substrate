"""Append-only v6 adopter for the Generation 1 successor evidence chain.

Version 6 supersedes the immutable v5 integrity hold. It preserves the
observe-only legacy adoption and unchanged v1 horizon launch, keeps the bounded
re-snapshot around the exact multiprocessing spawn-title transition, and adds a
second, equally fail-closed re-snapshot class for a child-label worker that is
genuinely attached to the exact owning parent by ppid or pgid but has not yet
settled into the clean all-three (repo-cwd, parent-PGID, parent-PPID) residual.
A worker mid setpgid/reparent (the transient consolidated-final worker that
held v5) is tolerated only while it resolves into that clean residual or exits;
a process that shares only cwd, is bound to a foreign group, or never resolves
still fails closed to integrity_hold within the same bounded window.
"""
...
CHAIN_ID = "generation1-successor-evidence-chain-v6"
PARENT_LABEL = "mop-successor-evidence-chain-v6"
STATE_SCHEMA = "mop-generation1-successor-evidence-chain-state/v6"
STATUS_SCHEMA = "mop-generation1-successor-evidence-chain-status/v6"
...
def _implementation_authority(repo_root: Path) -> dict[str, Any]:
    ...
    predecessor = Path(predecessor_v5.__file__).resolve()
    return {
...
class SuccessorEvidenceChain(v3.SuccessorEvidenceChain):
    ...
    def _load_state(self) -> dict[str, Any]:
        ...
        state = _read_json(self.state_path, "successor chain v6 state")
        _validate_seal(state, "state_sha256", "successor chain v6 state")
        if state.get("schema") != STATE_SCHEMA or state.get("program_id") != CHAIN_ID:
            raise SuccessorChainRefused("successor chain v6 state identity drifted")
        ...
        if not isinstance(capsules, dict) or set(capsules) != expected_capsules:
            raise SuccessorChainRefused("successor chain v6 capsule inventory drifted")
    ...
    def _tolerable_transient_worker(
        self,
        process: ProcessSnapshot,
        parent: ProcessSnapshot,
        spec: LegacySpec,
    ) -> bool:
        """Recognize a child-label worker mid setpgid/reparent toward this parent.

        The worker must carry an exact child-label family, run inside the
        repository working tree, and already be attached to *this* exact parent
        by ppid or pgid, yet not have settled into the clean all-three residual
        (repository cwd, parent PGID, and parent PPID together).  A process that
        shares only cwd, or that is bound to a foreign process group and foreign
        parent, fails this gate and is never treated as tolerable.
        """
        ...
        if process.cwd != str(self.repo_root):
            return False
        attached = process.ppid == parent.pid or process.pgid == parent.pid
        clean_residual = (
            process.cwd == str(self.repo_root)
            and process.pgid == parent.pid
            and process.ppid == parent.pid
        )
        return attached and not clean_residual
    ...
    def _snapshot_provisionals(
        ...
        boundary is already exact); the second holds child-label workers mid
        setpgid/reparent toward the exact owning parent.  Any other hard
        boundary error still fails closed immediately.
        """
        ...
        for spec in self.specs:
            try:
                ...
                message = str(exc)
                title_prefix = f"{spec.stage_id} process group has unexpected members: "
                boundary_prefix = (
                    f"{spec.stage_id} matching worker label is outside the exact "
                    "repository-cwd, parent-PGID, and parent-PPID boundary: "
                )
                is_title = message.startswith(title_prefix)
                is_boundary = message.startswith(boundary_prefix)
                if not (is_title or is_boundary):
                    raise
                matched_prefix = title_prefix if is_title else boundary_prefix
                raw_pids = message.removeprefix(matched_prefix).split(", ")
                try:
                ...
                if len(parents) != 1 or len(candidates) != len(pids):
                    raise
                if is_title:
                    if any(
                        ...
                        raise
                    title[spec.stage_id] = (parents[0], candidates)
                else:
                    if any(
                        not self._tolerable_transient_worker(candidate, parents[0], spec)
                        for candidate in candidates
                    ):
                        raise
                    boundary[spec.stage_id] = (parents[0], candidates)
    ...
    def _stable_process_table(self) -> tuple[ProcessSnapshot, ...]:
        ...
        workers keep a fixed title while a child moves through setpgid/reparent
        toward the exact owning parent; they are pinned only by (pid,
        create_time) plus the exact owning parent, and are tolerated solely
        while they resolve into the clean all-three residual or exit exactly.
        """

        current = tuple(self.process_table_fn())
        title_provisional, boundary_provisional = self._snapshot_provisionals(current)
        if not title_provisional and not boundary_provisional:
            return current

        parent_bindings = {
            stage_id: parent.identity()
            for stage_id, (parent, _candidates) in title_provisional.items()
        }
        for stage_id, (parent, _candidates) in boundary_provisional.items():
            parent_bindings.setdefault(stage_id, parent.identity())
        candidate_bindings: dict[tuple[int, float], tuple[str, dict[str, Any]]] = {
            ...
            for stage_id, (_parent, candidates) in title_provisional.items()
            for candidate in candidates
        }
        boundary_bindings: dict[int, tuple[str, dict[str, Any]]] = {
            candidate.pid: (
                stage_id,
                {"pid": candidate.pid, "create_time": candidate.create_time},
            )
            for stage_id, (_parent, candidates) in boundary_provisional.items()
            for candidate in candidates
        }
        clean_samples = 0
        for _attempt in range(PROCESS_TRANSITION_ATTEMPTS):
            ...
            for pid, (stage_id, recorded) in boundary_bindings.items():
                spec = next(spec for spec in self.specs if spec.stage_id == stage_id)
                parent = next(process for process in current if process.label == spec.process_label)
                visible = by_pid.get(pid)
                if visible is None:
                    identity_state = self.identity_probe_fn(recorded)
                    if identity_state != "gone":
                        raise SuccessorChainRefused(
                            f"provisional worker {pid} disappeared without exact gone identity"
                        )
                    continue
                if not math.isclose(
                    visible.create_time,
                    ...
                    raise SuccessorChainRefused(
                        f"provisional worker {pid} changed identity during stabilization"
                    )
                clean_residual = (
                    visible.cwd == str(self.repo_root)
                    and visible.pgid == parent.pid
                    and visible.ppid == parent.pid
                )
                if not (clean_residual or self._tolerable_transient_worker(visible, parent, spec)):
                    raise SuccessorChainRefused(
                        f"provisional worker {pid} resolved to an unauthorized process class"
                    )

            title_provisional, boundary_provisional = self._snapshot_provisionals(current)
            for stage_id, (parent, candidates) in title_provisional.items():
            ...
            for stage_id, (parent, candidates) in boundary_provisional.items():
                bound = parent_bindings.get(stage_id)
                if bound is None:
                    parent_bindings[stage_id] = parent.identity()
                elif not self._same_snapshot_identity(parent, bound):
                ...
                for candidate in candidates:
                    prior = boundary_bindings.get(candidate.pid)
                    if prior is None:
                        boundary_bindings[candidate.pid] = (
                            stage_id,
                            {"pid": candidate.pid, "create_time": candidate.create_time},
                        )
                    elif prior[0] != stage_id or not math.isclose(
                        prior[1]["create_time"],
                        candidate.create_time,
                        rel_tol=0.0,
                        abs_tol=PROCESS_TIME_TOLERANCE_SECONDS,
                        ...
                        raise SuccessorChainRefused(
                            f"provisional worker {candidate.pid} boundary changed during stabilization"
                        )

            if title_provisional or boundary_provisional:
                clean_samples = 0
                continue
            clean_samples += 1
            if clean_samples == 2:
        ...
        unresolved = sorted(
            {
                candidate.pid
                for _parent, candidates in title_provisional.values()
                for candidate in candidates
            }
            | {
                candidate.pid
                for _parent, candidates in boundary_provisional.values()
                for candidate in candidates
            }
        )
    ...
    def _write_adoption_receipt(
        ...
        if path.exists():
            receipt = _read_json(path, f"{spec.stage_id} v6 adoption receipt")
            _validate_seal(receipt, "receipt_sha256", f"{spec.stage_id} v6 adoption receipt")
            if (
                ...
                raise SuccessorChainRefused(f"{spec.stage_id} v6 adoption receipt identity drifted")
        else:
    ...
    def _control_requested(self) -> bool:
        ...
        if not path.exists():
            return False
        value = _read_json(path, "successor chain v6 control")
        _validate_seal(value, "control_sha256", "successor chain v6 control")
        if (
            value.get("schema") != CONTROL_SCHEMA
            or value.get("program_id") != CHAIN_ID
            or value.get("action") != "drain"
        ):
            raise SuccessorChainRefused("successor chain v6 control identity drifted")
        return True
...
def validate_chain_status(
    ...
    *,
    ...
    if set(status) != STATUS_FIELDS:
        raise SuccessorChainRefused("successor chain v6 status fields drifted")
    _validate_seal(status, "status_sha256", "successor chain v6 status")
    state = status.get("state")
    problems = status.get("problems")
    if (
        ...
        or status.get("supersedes") != "generation1-successor-evidence-chain-v5"
        ...
        raise SuccessorChainRefused("successor chain v6 status identity or authority drifted")
    if (state == "complete" or state not in TERMINAL_STATES) and (
        status.get("execution_enabled") is not True or problems != []
    ):
        raise SuccessorChainRefused("successor chain v6 safe acknowledgement is not clean")
    ...
    if created_at is None or updated_at is None or created_at > updated_at:
        raise SuccessorChainRefused("successor chain v6 status timestamps drifted")
    if state in TERMINAL_STATES:
        if finished_at is None or finished_at > updated_at:
            raise SuccessorChainRefused("successor chain v6 terminal timestamp drifted")
    elif status.get("finished_at") is not None:
        raise SuccessorChainRefused("successor chain v6 nonterminal status is marked finished")
    supervisor = status.get("supervisor")
    ...
    if (
        ...
        raise SuccessorChainRefused("successor chain v6 supervisor authority drifted")
    capsules = status.get("capsules")
    ...
    if not isinstance(capsules, Mapping) or set(capsules) != expected_ids:
        raise SuccessorChainRefused("successor chain v6 status capsule inventory drifted")
    complete = 0
    for capsule_id, raw_row in capsules.items():
        if not isinstance(raw_row, Mapping) or set(raw_row) != CAPSULE_FIELDS:
            raise SuccessorChainRefused(f"successor chain v6 capsule {capsule_id} fields drifted")
        ...
        if (
            ...
            raise SuccessorChainRefused(f"successor chain v6 capsule {capsule_id} receipt shape drifted")
        if isinstance(process, Mapping):
            ...
            if (
                ...
                raise SuccessorChainRefused(
                    f"successor chain v6 capsule {capsule_id} process identity drifted"
                )
        if row_state == "complete":
            complete += 1
            if returncode != 0 or finished_at is None or not artifacts or last_problem is not None:
                raise SuccessorChainRefused(f"successor chain v6 capsule {capsule_id} completion drifted")
        elif (
            ...
            raise SuccessorChainRefused(f"successor chain v6 capsule {capsule_id} noncompletion drifted")
    expected_counts = {
        "complete": complete,
    ...
    if status.get("counts") != expected_counts:
        raise SuccessorChainRefused("successor chain v6 status counts drifted")
    ...
    if not coherent or (state not in TERMINAL_STATES and all_complete):
        raise SuccessorChainRefused("successor chain v6 state and capsule progression drifted")
    if state not in TERMINAL_STATES and any(row.get("status") == "failure_hold" for row in capsules.values()):
        raise SuccessorChainRefused("successor chain v6 safe status contains a failed capsule")
    return state
...
def _validate_complete_state_shape(
    ...
    if set(state) != STATE_FIELDS:
        raise SuccessorChainRefused("successor chain v6 state fields drifted")
    _validate_seal(state, "state_sha256", "successor chain v6 state")
    if (
        ...
        raise SuccessorChainRefused("successor chain v6 complete state boundary drifted")
    ...
    if (
        ...
        raise SuccessorChainRefused("successor chain v6 complete state timestamps drifted")
    ...
    if not isinstance(capsules, Mapping) or set(capsules) != expected_ids:
        raise SuccessorChainRefused("successor chain v6 complete capsule inventory drifted")
    for capsule_id, raw_row in capsules.items():
        if not isinstance(raw_row, Mapping) or set(raw_row) != CAPSULE_FIELDS:
            raise SuccessorChainRefused(f"successor chain v6 capsule {capsule_id} fields drifted")
        attempts = raw_row.get("attempts")
        if (
            ...
            raise SuccessorChainRefused(f"successor chain v6 capsule {capsule_id} completion receipt drifted")
        if capsule_id == "successor_horizon" and raw_row.get("adoption_receipts") != []:
            raise SuccessorChainRefused("successor chain v6 horizon capsule has unexpected adoption receipts")
    for spec in owner.specs:
        ...
        if len(receipt_paths) != len(set(receipt_paths)):
            raise SuccessorChainRefused(
                f"successor chain v6 {spec.stage_id} adoption receipt inventory contains duplicates"
            )
        ...
        for raw_path in receipt_paths:
            ...
            if relative.is_absolute() or ".." in relative.parts:
                raise SuccessorChainRefused(
                    f"successor chain v6 {spec.stage_id} adoption receipt path is unsafe"
                )
            ...
            if (
                ...
                raise SuccessorChainRefused(
                    f"successor chain v6 {spec.stage_id} adoption receipt is missing "
                    "or is not a canonical regular non-symlink file"
                )
            path = candidate.resolve()
            if not path.is_relative_to(receipt_root):
                raise SuccessorChainRefused(
                    f"successor chain v6 {spec.stage_id} adoption receipt path drifted"
                )
            file_stat = candidate.stat()
            file_identity = (file_stat.st_dev, file_stat.st_ino)
            if path in resolved_receipts or file_identity in receipt_files:
                raise SuccessorChainRefused(
                    f"successor chain v6 {spec.stage_id} adoption receipt resolves to a duplicate file"
                )
            ...
            if set(receipt) != ADOPTION_FIELDS:
                raise SuccessorChainRefused(
                    f"successor chain v6 {spec.stage_id} adoption receipt fields drifted"
                )
            _validate_seal(
                receipt,
                "receipt_sha256",
                f"{spec.stage_id} v6 adoption receipt",
            )
            ...
            if (
                ...
                raise SuccessorChainRefused(
                    f"successor chain v6 {spec.stage_id} adoption receipt authority drifted"
                )
            pid = process.get("pid")
            ...
            if (
                ...
                raise SuccessorChainRefused(
                    f"successor chain v6 {spec.stage_id} adopted process identity drifted"
                )
            if observed_status is not None and (
                ...
                raise SuccessorChainRefused(
                    f"successor chain v6 {spec.stage_id} observed status authority drifted"
                )
            identity_sha = canonical_sha256(process)[:16]
            if candidate.name != f"{process.get('pid')}-{identity_sha}.json":
                raise SuccessorChainRefused(
                    f"successor chain v6 {spec.stage_id} adoption receipt filename drifted"
                )
            validated_processes.append(process)
        if not validated_processes and row.get("process") is not None:
            raise SuccessorChainRefused(
                f"successor chain v6 {spec.stage_id} has process identity without an adoption receipt"
            )
        if validated_processes and row.get("process") != validated_processes[-1]:
            raise SuccessorChainRefused(f"successor chain v6 {spec.stage_id} adopted process history drifted")
...
def _read_stable_state_status(
    ...
    for attempt in range(SNAPSHOT_ATTEMPTS):
        before = _read_json(state_path, "successor chain v6 state")
        first_status = _read_json(status_path, "successor chain v6 status")
        after = _read_json(state_path, "successor chain v6 state")
        second_status = _read_json(status_path, "successor chain v6 status")
        try:
    ...
    raise SuccessorChainRefused("successor chain v6 state/status snapshot did not become coherent")
...
def _validate_bound_artifact_hashes(
    ...
    for capsule_id, row in capsules.items():
        if not isinstance(row, Mapping):
            raise SuccessorChainRefused(f"successor chain v6 capsule {capsule_id} is not an object")
        artifacts = row.get("artifacts")
        if not isinstance(artifacts, list):
            raise SuccessorChainRefused(f"successor chain v6 capsule {capsule_id} artifacts drifted")
        for report in artifacts:
            if not isinstance(report, Mapping):
                raise SuccessorChainRefused(
                    f"successor chain v6 capsule {capsule_id} artifact report drifted"
                )
            raw_path = report.get("path")
            if not isinstance(raw_path, str) or not raw_path:
                raise SuccessorChainRefused(f"successor chain v6 capsule {capsule_id} artifact path drifted")
            relative = Path(raw_path)
            ...
            if (
                ...
                raise SuccessorChainRefused(
                    f"successor chain v6 capsule {capsule_id} artifact path is unsafe"
                )
            path = candidate.resolve()
            if not path.is_relative_to(repo_root) or report.get("sha256") != sha256_file(path):
                raise SuccessorChainRefused(
                    f"successor chain v6 capsule {capsule_id} final artifact hash drifted"
                )
...
def read_validated_complete_chain_status(
    *,
    ...
    if not root.is_dir() or not root.is_relative_to(repo_root):
        raise SuccessorChainRefused("successor chain v6 root is absent or outside the repository")
    if acquire_lock:
        for attempt in range(LOCK_ATTEMPTS):
            try:
                ...
                if attempt + 1 < LOCK_ATTEMPTS:
                    ...
                    continue
                raise SuccessorChainRefused(
                    "successor chain v6 lifetime lock remained held while validating a complete snapshot"
                ) from exc
        raise SuccessorChainRefused("successor chain v6 lifetime lock acquisition exhausted")
    if horizon_program_path is None:
        relative = DEFAULT_HORIZON_PROGRAM.resolve().relative_to(REPO_ROOT.resolve())
        horizon_program_path = repo_root / relative
    horizon_program_path = horizon_program_path.resolve()
    ...
    _validate_seal(stable_status, "status_sha256", "successor chain v6 status")
    configured_specs = tuple(specs) if specs is not None else legacy_specs(repo_root)
    owner = SuccessorEvidenceChain(
        root=root,
        repo_root=repo_root,
    ...
    if validated_state != "complete":
        raise SuccessorChainRefused("successor chain v6 durable status is not complete")
    ...
    if owner.state["capsules"] != before_capsules:
        raise SuccessorChainRefused(
            "successor chain v6 evidence required reconciliation during terminal replay"
        )
    ...
    if current_state != stable_state or current_status != stable_status:
        raise SuccessorChainRefused("successor chain v6 state/status changed during terminal replay")
    owner._validate_completed_chain_artifacts(())
    if owner.state["capsules"] != before_capsules:
        raise SuccessorChainRefused("successor chain v6 evidence required reconciliation during final replay")
    ...
    if final_state != stable_state or final_status != stable_status:
        raise SuccessorChainRefused("successor chain v6 state/status changed during final evidence replay")
...
def read_chain_status(root: Path = DEFAULT_ROOT) -> dict[str, Any]:
    ...
    status = _read_json(path, "successor chain v6 status")
    _validate_seal(status, "status_sha256", "successor chain v6 status")
    if status.get("schema") != STATUS_SCHEMA or status.get("program_id") != CHAIN_ID:
        raise SuccessorChainRefused("successor chain v6 status identity drifted")
    return status
...
def _visible_parent_process(entrypoint: Path) -> ProcessSnapshot | None:
    ...
    for process in candidates:
        if (
            ...
            raise SuccessorChainRefused("visible successor chain v6 parent has inexact metadata")
    ...
    if len(labelled) > 1 or len(groups) > 1:
        raise SuccessorChainRefused("multiple successor chain v6 parent processes are visible")
    if labelled:
        return labelled[0]
...
def _revalidate_terminal_chain(root: Path) -> dict[str, Any]:
    with FileLock(root / LOCK_FILE):
        ...
        if owner.state.get("status") not in TERMINAL_STATES:
            raise SuccessorChainRefused(
                "sealed terminal status disagrees with the durable successor chain v6 state"
            )
...
def start_chain_detached(
    *,
    ...
    if not execute:
        raise SuccessorChainRefused("detached successor chain v6 start requires explicit --execute")
    root = root.resolve()
    root.mkdir(parents=True, exist_ok=True)
    with FileLock(root / START_LOCK_FILE):
        entrypoint = (REPO_ROOT / "scripts/mop_generation1_successor_chain_v6.py").resolve()
        status_path = root / STATUS_FILE
        status = None
        ...
        if _process_identity_alive(status_identity):
            if visible is None:
                raise SuccessorChainRefused("sealed successor chain v6 identity has no exact visible parent")
            if (
                ...
                raise SuccessorChainRefused(
                    "sealed successor chain v6 identity conflicts with visible parent"
                )
            if status is None:
                raise SuccessorChainRefused("live successor chain v6 identity lacks a sealed status")
            try:
        ...
        if acknowledged is None:
            raise SuccessorChainRefused(
                f"detached successor chain v6 did not acknowledge startup; inspect {stderr_path}"
            )
        return {
    ...
    try:
        if arguments.command == "run":
            if not arguments.execute:
                raise SuccessorChainRefused("successor chain v6 execution requires explicit --execute")
