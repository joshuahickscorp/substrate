...
from mop.studio.generation1_successor_chain_v5 import (
...
from mop.studio.generation1_successor_extension_chain_v2 import (
...
PROGRAM_ID = "generation1-successor-future-chain-v2"
RECEIPT_SCHEMA = "mop-generation1-successor-future-chain-start/v2"
...
def _replay_complete_adopter(status: Mapping[str, Any]) -> None:
    try:
        ...
        raise SuccessorFutureChainRefused(
            f"v5 adopter complete acknowledgement failed durable replay: {type(exc).__name__}: {exc}"
        ) from exc
    if dict(status) != replayed:
        raise SuccessorFutureChainRefused("v5 adopter complete acknowledgement disagrees with durable replay")
...
def start_future_chain(
    *,
    ...
    if adopter_state in UNSAFE_TERMINAL_STATES:
        raise SuccessorFutureChainRefused(f"v5 adopter is terminal in unsafe state {adopter_state}")
