"""One-command recovery launcher for the append-only Generation 1 successor chain.

The v5 adopter went integrity_hold and the extension chain v2 failure_hold
cascaded; both are sealed and immutable.  This launcher is the append-only
recovery front door: it idempotently starts (or resumes) the hardened v6 adopter
and then the fresh extension chain v3 waiter, each detached under the
acknowledged-waiter pattern, refusing to duplicate a live parent.  It observes
only its two successors' own roots and never signals, edits, or reads the sealed
v5/extv2 held state.
"""
...
from mop.studio.generation1_successor_chain_v6 import (
...
from mop.studio.generation1_successor_extension_chain_v3 import (
...
PROGRAM_ID = "generation1-successor-chain-recovery-v1"
RECEIPT_SCHEMA = "mop-generation1-successor-chain-recovery-start/v1"
...
def _replay_complete_adopter(status: Mapping[str, Any]) -> None:
    try:
        ...
        raise SuccessorChainRecoveryRefused(
            f"v6 adopter complete acknowledgement failed durable replay: {type(exc).__name__}: {exc}"
        ) from exc
    if dict(status) != replayed:
        raise SuccessorChainRecoveryRefused(
            "v6 adopter complete acknowledgement disagrees with durable replay"
        )
...
def start_recovery(
    *,
    ...
    if adopter_state in UNSAFE_TERMINAL_STATES:
        raise SuccessorChainRecoveryRefused(f"v6 adopter is terminal in unsafe state {adopter_state}")
