"""Durable waiter for the second Generation 1 successor horizon (append-only v4).

Version 4 supersedes the extension chain v3, rebinding its observed predecessor
from the integrity_hold ``generation1-successor-evidence-chain-v6`` to the
append-only ``generation1-successor-evidence-chain-v7`` adopter.  It remains
intentionally observation-only with respect to the live v7 parent: it binds its
own implementation and the exact sealed horizon v2 manifest, waits for the v7
chain to complete, and then starts or resumes the horizon v2 program through the
generic Generation 1 supervisor.  It never signals or edits the sealed v5/v6/v2
runs; the horizon v2 target program and every downstream observer are unchanged.
"""
...
from mop.studio import generation1_successor_chain_v7 as predecessor_v7
...
PROGRAM_ID = "generation1-successor-extension-chain-v4"
PARENT_LABEL = "mop-successor-extension-chain-v4"
STATE_SCHEMA = "mop-generation1-successor-extension-chain-state/v4"
STATUS_SCHEMA = "mop-generation1-successor-extension-chain-status/v4"

PREDECESSOR_PROGRAM_ID = "generation1-successor-evidence-chain-v7"
PREDECESSOR_STATUS_SCHEMA = "mop-generation1-successor-evidence-chain-status/v7"
...
DEFAULT_PREDECESSOR_STATUS = (
    REPO_ROOT / "runs/generation1/generation1-successor-evidence-chain-v7/current_status.json"
)
...
class SuccessorExtensionChain:
    ...
    def _initial_state(self) -> dict[str, Any]:
        ...
        return {
            ...
            "capsules": {
                "predecessor_chain_v7": _empty_row(),
    ...
    def _validate_predecessor_artifact(
        ...
        value = _require_exact_fields(
            ...
            f"v7 predecessor capsule {capsule_id} artifact report",
        )
        path, payload = self._current_artifact_payload(
            value["path"],
            f"v7 predecessor capsule {capsule_id} artifact",
        )
        if (
            ...
            raise SuccessorExtensionRefused(f"v7 predecessor capsule {capsule_id} artifact report drifted")

    def _validate_predecessor_status(self, value: Mapping[str, Any]) -> None:
        _require_exact_fields(value, PREDECESSOR_STATUS_FIELDS, "v7 predecessor status")
        ...
        if (
            ...
            or value.get("supersedes") != "generation1-successor-evidence-chain-v6"
            or not isinstance(value.get("execution_enabled"), bool)
            or not isinstance(state, str)
            or state not in PREDECESSOR_STATES
        ):
            raise SuccessorExtensionRefused("v7 predecessor status identity, safety, or state drifted")
        if (
            ...
            raise SuccessorExtensionRefused("v7 predecessor status metadata drifted")
        problems = _validate_problem_list(value.get("problems"), "v7 predecessor status")
        capsules = value.get("capsules")
        if not isinstance(capsules, Mapping) or set(capsules) != set(PREDECESSOR_CAPSULE_IDS):
            raise SuccessorExtensionRefused("v7 predecessor status capsule inventory drifted")
        completed = 0
        for capsule_id in PREDECESSOR_CAPSULE_IDS:
            row = _require_exact_fields(
                capsules[capsule_id],
                PREDECESSOR_CAPSULE_FIELDS,
                f"v7 predecessor capsule {capsule_id}",
            )
            row_state = row.get("status")
            if not isinstance(row_state, str) or row_state not in PREDECESSOR_CAPSULE_STATES:
                raise SuccessorExtensionRefused(f"v7 predecessor capsule {capsule_id} state drifted")
            attempts = row.get("attempts")
            if isinstance(attempts, bool) or not isinstance(attempts, int) or attempts < 0:
                raise SuccessorExtensionRefused(f"v7 predecessor capsule {capsule_id} attempts drifted")
        ...
        if value.get("counts") != expected_counts:
            raise SuccessorExtensionRefused("v7 predecessor status counts drifted")
        if state != "complete":
            return
        if (
            problems != []
            ...
            raise SuccessorExtensionRefused("completed v7 predecessor status is not clean")
        for capsule_id in PREDECESSOR_CAPSULE_IDS:
            ...
            if (
                ...
                raise SuccessorExtensionRefused(
                    f"completed v7 predecessor capsule {capsule_id} receipt drifted"
                )
    ...
    def _validate_complete_predecessor_snapshot(
        ...
        try:
            return predecessor_v7.read_validated_complete_chain_status(
            ...
            predecessor_v7.SuccessorChainRefused,
            Generation1Refused,
            AttributeError,
            IndexError,
            KeyError,
            ...
            raise SuccessorExtensionRefused(
                f"v7 predecessor complete snapshot is invalid: {type(exc).__name__}: {exc}"
            ) from exc
    ...
    def _validate_predecessor_launch_authority(self) -> None:
        ...
        if after != before:
            raise SuccessorExtensionRefused("v7 predecessor changed during v2 prelaunch authority replay")
    ...
    def _predecessor_status(self) -> dict[str, Any] | None:
        if not self.predecessor_status_path.exists():
            return None
        value = _read_json(self.predecessor_status_path, "v7 predecessor status")
        _validate_seal(value, "status_sha256", "v7 predecessor status")
        self._validate_predecessor_status(value)
        if value.get("state") == "complete":
            validated = self.predecessor_completion_validator_fn(value)
            if not isinstance(validated, Mapping):
                raise SuccessorExtensionRefused(
                    "v7 predecessor completion validator returned no authoritative status"
                )
            ...
            if value.get("state") != "complete":
                raise SuccessorExtensionRefused(
                    "v7 predecessor changed state during complete snapshot validation"
                )
        return value
    ...
    def _reconcile_predecessor(self, row: dict[str, Any]) -> bool:
        ...
        if status is None:
            row["status"] = "pending"
            row["last_problem"] = "waiting for the sealed v7 chain status"
            return False
        state = str(status.get("state"))
        row["last_problem"] = f"observed v7 chain state {state}; signals remain forbidden"
        if state == "complete":
    ...
    def _reconcile_target(
        ...
        predecessor_lock = self.predecessor_status_path.parent / predecessor_v7.LOCK_FILE
    ...
    def _validate_terminal_artifacts(self, program: Program) -> None:
        predecessor = self.state["capsules"]["predecessor_chain_v7"]
    ...
    def tick(self) -> dict[str, Any]:
        try:
            ...
            if predecessor.get("status") == "failure_hold":
                ...
                self._append_problem(str(predecessor.get("last_problem") or "v7 predecessor failed"))
                return self._publish()
...
def validate_extension_status(
    ...
    *,
    ...
    capsules = status.get("capsules")
    expected_ids = {"predecessor_chain_v7", "successor_horizon_v2"}
    ...
    predecessor_state = capsules["predecessor_chain_v7"].get("status")
...
def start_extension_detached(
    *,
    ...
    with FileLock(root / START_LOCK_FILE):
        entrypoint = (REPO_ROOT / "scripts/mop_generation1_successor_extension_chain_v4.py").resolve()
        status_path = root / STATUS_FILE
        status = None
