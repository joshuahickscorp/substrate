...
@dataclass(frozen=True, slots=True)
class ReproCountBedArtifact:
    """The assembled sealed reproduction artifact plus the mechanics-only demonstration receipt."""
...
def build_repro_count_bed_artifact(
    *,
    ...
    prereg_written = write_repro_prereg(prereg, prereg_path)
    ...
    body: dict[str, Any] = {
        ...
        "matched_budget_wall_note": (
            "wall_ns is a deterministic nominal at a 1 GFLOP/s reference so the artifact is byte-"
            "reproducible; the measured wall is unsealed run provenance, and the authoritative sealed "
            "compute axes are the parameter count and the FLOP ledger"
        ),
