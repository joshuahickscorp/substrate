...
VERIFIER_SCHEMA = "mop-starss23-count-repro-featurizer-estimator-verification/v1"

# Re-declared, never imported, so the verifier shares no symbol with the producer it audits.
EXPECTED_ARTIFACT_SCHEMA = "mop-starss23-escs-count-bed-repro-featurizer-estimator/v1"
EXPECTED_STAGE = 3
EXPECTED_CLAIM_SCOPE = "deterministic programmatic mechanics only; no capability or natural-data claim"
EXPECTED_REPRO_AXIS = "featurizer_estimator"
...
def _abs_error_sum(gt: list[int], emitted: list[int]) -> int:
    if len(gt) != len(emitted):
        raise ReproVerificationRefusal("ground-truth and emitted tracks differ in length")
...
def verify_repro_count_artifact(artifact: dict) -> ReproVerificationResult:
    ...
    recomputed_candidate_mae: list[float] = []
    recomputed_rmr_mae: list[float] = []
    for seed_block in per_seed:
        ...
        recomputed_candidate_mae.append(recomputed_mae[ARM_CANDIDATE])
        recomputed_rmr_mae.append(recomputed_mae[ARM_RATE_MATCHED_RANDOM])
    ...
    # 6. Independent survival readout: candidate strictly lower mean MAE than rate-matched-random, mean paired
    #    delta at least the registered SESOI, and the one-sided sign-flip at or below the 1/32 floor. This is
    #    the reproduction's own strict conjunction, re-derived here and NOT read from the producer.
    n_seeds = len(recomputed_deltas)
    mean_candidate = sum(recomputed_candidate_mae) / n_seeds if n_seeds else 0.0
    mean_rmr = sum(recomputed_rmr_mae) / n_seeds if n_seeds else 0.0
    survives = bool(
        independent_referee_reproduction
        and mean_candidate < mean_rmr
        and isinstance(sesoi, (int, float))
        and not isinstance(sesoi, bool)
        and mean_delta >= float(sesoi)
        and one_sided_p <= (1.0 / n_perm) + _TOL
    )

    # 7. Scientific confirmation is a strictly higher bar and cannot be reached on a single run.
    ...
    return ReproVerificationResult(
        ...
        detail={
            ...
            "recomputed_candidate_mean_mae": mean_candidate,
            "recomputed_rate_matched_random_mean_mae": mean_rmr,
            ...
            "sesoi_mae": sesoi,
            "mean_delta_exceeds_sesoi": exceeds,
            "candidate_strictly_beats_rate_matched_random": mean_candidate < mean_rmr,
...
def verify_sealed_repro_count_file(in_path: str) -> dict:
    ...
    return repro_verification_payload(verify_repro_count_artifact(artifact))
