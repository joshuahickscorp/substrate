...
# ---------------------------------------------------------------------------
# The native fold-respecting split and one-time featurization / estimation.
# ---------------------------------------------------------------------------
...
def _fold_respecting_split(
    ...
    if not train or not val or not test:
        raise CountProducerRefusal("the fold-respecting split produced an empty partition")
    detail = {
...
def _featurize_all(
    adapter: RealStarssAdapter, featurizer: FrozenCountFeaturizer
) -> dict[str, np.ndarray]:
    """Featurize every real clip once with the frozen count front-end. Reused across all paired seeds."""

    return {clip.clip_id: featurizer.featurize(adapter.audio(clip.clip_id)) for clip in adapter.clips()}


def _estimate_all(
    adapter: RealStarssAdapter, estimator: FrozenCountEstimator
) -> dict[str, np.ndarray]:
    """Compute the frozen estimator track once per clip. Shared across all arms and all paired seeds."""

    return {
        clip.clip_id: estimator.estimate_track(adapter.audio(clip.clip_id)) for clip in adapter.clips()
    }
...
def _build_budget_points(
    seed_runs: list[_CountSeedRun], config: RealCountBedConfig
...
def write_count_artifact(
    artifact: dict[str, Any], out_path: str | Path = DEFAULT_COUNT_ARTIFACT_PATH
) -> Path:
