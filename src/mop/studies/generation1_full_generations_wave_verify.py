...
def _root_from_result(value: Mapping[str, Any]) -> Path:
    ...
    if not isinstance(waves, list) or not waves:
        raise ValueError("full-generations-wave result has no wave bindings")
    categories = waves[0].get("categories") if isinstance(waves[0], Mapping) else None
    if not isinstance(categories, list) or not categories:
        raise ValueError("full-generations-wave result has no category bindings")
    path_value = categories[0].get("path") if isinstance(categories[0], Mapping) else None
    if not isinstance(path_value, str):
        raise ValueError("full-generations-wave result category path is invalid")
    relative = Path(path_value)
    if relative.is_absolute() or ".." in relative.parts:
        raise ValueError("full-generations-wave result category path is unsafe")
    path = (REPO_ROOT / relative).resolve()
    root = path.parents[2]
    if not root.is_relative_to(REPO_ROOT.resolve()):
        raise ValueError("full-generations-wave result root escapes the repository")
    return root
...
def _mutation_suite(result: Mapping[str, Any], root: Path) -> dict[str, Any]:
    ...
    if tuple(name for name, _mutate in mutations) != MUTATION_NAMES:
        raise ValueError("full-generations-wave mutation inventory drifted")
    rows = [
...
def _recomputation_from_result(
    ...
    return {
        ...
        "new_mechanism_lane_count": len(wave.NEW_LANE_IDS),
        "i1_dependency_count": len(wave.I1_DEPENDENCIES),
...
def build_verification(result_path: Path = wave.DEFAULT_RESULT) -> dict[str, Any]:
    ...
    if not mutation_suite["all_rejected"]:
        raise ValueError("full-generations-wave semantic mutation suite did not reject every mutation")
...
def validate_verification(value: Mapping[str, Any]) -> None:
    if set(value) != {
        ...
        raise ValueError("full-generations-wave verification fields drifted")
    _validate_seal(value, "verification_sha256", "full-generations-wave verification")
    verified_at = value.get("verified_at")
    if not isinstance(verified_at, str):
        raise ValueError("full-generations-wave verification timestamp is invalid")
    try:
        parsed = dt.datetime.fromisoformat(verified_at.replace("Z", "+00:00"))
    except ValueError as exc:
        raise ValueError("full-generations-wave verification timestamp is invalid") from exc
    if parsed.tzinfo is None or parsed.utcoffset() != dt.timedelta(0):
        raise ValueError("full-generations-wave verification timestamp is not UTC")
    ...
    if not isinstance(source, Mapping) or set(source) != {
        ...
        raise ValueError("full-generations-wave verification source fields drifted")
    path_value = source.get("path")
    if not isinstance(path_value, str) or not path_value:
        raise ValueError("full-generations-wave verification source path is invalid")
    relative = Path(path_value)
    if relative.is_absolute() or ".." in relative.parts:
        raise ValueError("full-generations-wave verification source path is unsafe")
    result_path = (REPO_ROOT / relative).resolve()
    if not result_path.is_relative_to(REPO_ROOT.resolve()):
        raise ValueError("full-generations-wave verification source path escapes the repository")
    result = _read_object(result_path)
    root = _root_from_result(result)
    wave.validate_result(result, root=root)
    if source.get("file_sha256") != sha256_file(result_path) or source.get("result_sha256") != result.get(
        ...
        raise ValueError("full-generations-wave verification source bytes drifted")
    ...
    if (
        ...
        or value.get("checks") != _EXPECTED_CHECKS
        ...
        raise ValueError("full-generations-wave verification identity or recomputation drifted")
...
def verify(
    *,
    ...
    if not output.is_relative_to(REPO_ROOT.resolve()):
        raise ValueError("full-generations-wave verification is outside the repository")
    if output.is_file():
