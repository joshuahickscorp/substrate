...
from .p5_terminal_evidence import (
...
def _p5_terminal_contract(
    ...
    problems: list[str] = []
    parse_problems, summary = _parse_p5_terminal_evidence(parsed_sources, repo_root)
    problems.extend(parse_problems)
    if parse_problems:
        return problems, {}

    if atlas.get("p5_terminal_evidence") != summary:
        problems.append("atlas P5 terminal summary disagrees with the verified evidence chain")
    if "p5_local_admission" in atlas:
        problems.append("atlas retains the superseded P5 local-admission field")
    ...
    expected_narrative = (
        "P5 final verifier returned null after three disjoint fresh-training seeds verified "
        f"{summary['verified_pattern_count']} of {summary['fresh_pattern_count']} pilot patterns; "
        "the result is programmatic-only and grants no scientific or confirmatory promotion"
    )
    ...
    if not isinstance(readiness, list) or readiness.count(expected_narrative) != 1:
        problems.append("OP3 P5 terminal narrative disagrees with the verified evidence")
    if not isinstance(evidence, list) or any(
        evidence.count(path) != 1 for path in P5_TERMINAL_EVIDENCE_PATHS
    ):
        problems.append("OP3 does not bind every terminal P5 evidence path exactly once")
    expected_local_path = (
        "run the exclusive P6 10k resource probe, full 10k replication, and independent "
        "checkpoint verifier after the terminal P5 null"
    )
    if not isinstance(local_to_10, list) or local_to_10.count(expected_local_path) != 1:
        problems.append("OP3 local path does not advance from the terminal P5 null to P6")
    ...
    if not isinstance(measured, dict) or measured.get("terminal_evidence") != summary:
        problems.append("requirements P5 terminal summary disagrees with the verified evidence")
    elif (
        measured.get("present") is not True
        or measured.get("execution_verified_at_generation") is not True
        or measured.get("ledger_snapshot_superseded_by_terminal_evidence") is not True
    ):
        problems.append("requirements P5 execution state does not supersede the stale ledger row")
    if not isinstance(p5_row, dict) or p5_row.get("classification_basis") != (
        "governed-programmatic-terminal-null"
    ):
        problems.append("requirements P5 classification basis is not terminal-null")
    evidence_refs = p5_row.get("evidence_refs") if isinstance(p5_row, dict) else None
    current_refs = {f"local:{path}" for path in P5_TERMINAL_EVIDENCE_PATHS}
    retired_ref = "local:proof/LOCAL_THROTTLE_P5_SMOKE_PREFLIGHT.json"
    if (
        not isinstance(evidence_refs, list)
        or any(evidence_refs.count(ref) != 1 for ref in current_refs)
        or retired_ref in evidence_refs
    ):
        problems.append("requirements P5 evidence does not bind the exact terminal chain")
    ...
    requirement_check = checks.get("p5_terminal_evidence") if isinstance(checks, dict) else None
    if not isinstance(requirement_check, dict) or requirement_check != {"all_ok": True, **summary}:
        problems.append("requirements P5 authoritative terminal check has drifted")
    receipts = summary["governor_receipts"]
    return problems, {
        "state": summary["state"],
        "outcome": summary["outcome"],
        "smoke_run_id": receipts["smoke"]["run_id"],
        "pilot_run_id": receipts["pilot"]["run_id"],
        "fresh_challenge_run_id": receipts["fresh_challenge"]["run_id"],
        "verifier_run_id": receipts["verifier"]["run_id"],
        "primary_seed_count": summary["primary_seed_count"],
        "fresh_training_seed_count": len(summary["fresh_training_seeds"]),
        "verified_pattern_count": summary["verified_pattern_count"],
        "scientific_promotion": summary["scientific_promotion"],
    }
...
def validate_potential_atlas(
    ...
    *,
    ...
    p5_problems, p5_detail = _guard_pair(
        "terminal P5 evidence",
        lambda: _p5_terminal_contract(atlas, requirements, parsed_sources, repo_root),
    )
    checks.append(_check("p5_terminal_evidence", p5_problems, p5_detail))
    ...
    summary = {
        ...
        "p5_terminal_state": p5_detail.get("state"),
        "p5_outcome": p5_detail.get("outcome"),
        "p5_smoke_run_id": p5_detail.get("smoke_run_id"),
        "p5_pilot_run_id": p5_detail.get("pilot_run_id"),
        "p5_fresh_challenge_run_id": p5_detail.get("fresh_challenge_run_id"),
        "p5_verifier_run_id": p5_detail.get("verifier_run_id"),
        "p5_primary_seed_count": p5_detail.get("primary_seed_count"),
        "p5_fresh_training_seed_count": p5_detail.get("fresh_training_seed_count"),
        "p5_verified_pattern_count": p5_detail.get("verified_pattern_count"),
        "p5_scientific_promotion": p5_detail.get("scientific_promotion"),
