...
FINAL_PLAN = Path("/Users/scammermike/Downloads/SUBSTRATE_FINAL_AUTONOMOUS_PROGRAM.md")
MASTER_PLAN = Path(P.PLAN)
...
ANCESTRY = (
    ...
    ("mop-experimental-method-reformation-v1", "the experiment validity kernel",
     "the admission gate every Substrate experiment passes"),
    ("mop-fast-state-plasticity-forge-v1", "fast state and plasticity",
     "immutable inherited nulls including the learned plasticity closure"),
    ("mop-temporal-core-mechanism-v1", "the temporal core factorial",
     "terminal, and no core was scientifically licensed"),
    ("mop-substrate-master-v1", "the Substrate entity program", "live"),
)
...
def _rollback(item) -> str:
    ...
    if item.evidence:
        return (f"delete {', '.join(e for e in item.evidence if ':' not in e)} and rerun "
                "mop.cognition.deliverables write; the sealed inputs are immutable and are not touched")
...
def master_authority(st: dict) -> dict:
    ...
    return {
        ...
        "supersedes": "SUBSTRATE_MASTER_PLAN.md as the top level program structure, without "
                      "invalidating anything it sealed",
        "plans": {
            "final": {"path": str(FINAL_PLAN), "sha256": _sha(FINAL_PLAN),
                      "resolved": FINAL_PLAN.is_file()},
            "previous": {"path": str(MASTER_PLAN), "sha256": _sha(MASTER_PLAN),
                         "resolved": MASTER_PLAN.is_file()},
        },
        "source": {"commit": io.commit(), "tree": _git("rev-parse", "HEAD^{tree}"),
                   "branch": _git("rev-parse", "--abbrev-ref", "HEAD"),
                   "remote_branch": "agent/mop-temporal-core-mechanism",
                   "pull_request": 35},
        "proof_roots": {k: str(v.relative_to(io.ROOT)) for k, v in D.ROOTS.items() if k},
        "inherited_authorities": [{"program": prog, **D._bind(ref), "role": why}
                                  for prog, ref, why in D.INHERITED_AUTHORITIES],
        "naming_authority": D.NAMING,
        "claim_boundary": D.CLAIM_BOUNDARY,
        "goal_authority": D.GOAL_AUTHORITY,
        "single_infrastructure": {
            "scheduler": "mop.cognition.campaign over mop.cognition.graph",
            "engine": "fastforge.engine",
            "admission_gate": "mop.method.gate through mop.cognition.admission",
            "registry": "mop.cognition.program.ITEMS",
            "configuration_authority": "mop.cognition.graph.NODES",
            "cli": "python -m mop.cognition.<module>",
            "evidence_fabric": "proof/substrate/mop-substrate-master-v1 plus integrated/evidence_store",
        },
        "program_graph": {"nodes": graph_doc["node_count"], "valid": graph_doc["valid"],
                          "externally_blocked": graph_doc["externally_blocked"],
                          "buildable_prerequisites": [b["node"] for b in
                                                      graph_doc["buildable_prerequisites"]]},
        "resume_without_history": ("every field a successor needs is in SUBSTRATE_FINAL_STATE.json and "
...
def ancestry() -> dict:
    return {
        ...
        "programs": [{"program": name, "contribution": what, "role": role,
                      "proof_root": str((io.ROOT / "proof" / "substrate" / name).relative_to(io.ROOT))
                      if (io.ROOT / "proof" / "substrate" / name).is_dir() else None}
                     for name, what, role in ANCESTRY],
        "rule": ("historical evidence is preserved in place. This authority binds it and never rewrites "
                 "it, and a superseded claim keeps its original sealed bytes"),
        "temporal_core_verdict": {
            "terminal": True, "licensed": False,
...
def final_scorecard(st: dict) -> dict:
    ...
    return {
        ...
        "evidence_target_note": ("section 43 sets fifty to seventy five percent before a subsystem is "
...
def value_queue(st: dict) -> dict:
    ...
    graph_doc = G.declaration()
    ready = [r for r in graph_doc["nodes"]
             if not r["exit_passed"] and not r["missing_implementation"] and not r["external_blocker"]
    ...
    return {
        ...
        "selection_rule": ("expected decision information, scientific headroom, relevance to entity "
...
def ledger_markdown(st: dict) -> str:
    ...
    for r in rows:
        lines.append(f"| {r['id']} | {r['category']} | {r['status']} | {r['classification'] or '-'} | "
...
def write_all() -> dict:
    ...
    written = {
        ...
        "SUBSTRATE_FINAL_SCORECARD.json": io.seal("SUBSTRATE_FINAL_SCORECARD.json",
                                                  final_scorecard(st)),
