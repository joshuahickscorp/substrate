...
from mop.cognition import io, runtime as R, sessions as S

SESOI = 0.05

GATES = ("grounded_closed_loop", "endogenous_allocation", "cross_domain_continuity",
         "world_self_control_value", "procedural_transfer", "unity_under_conflict")

# what the long run may conclude, weakest first. None of these is a claim about experience.
CLASSIFICATIONS = ("certified_cognitive_scaffold", "persistent_developmental_cognition",
                   "reflective_cognitive_organization", "functional_or_proto_nous_candidate")
...
def _stream(n: int, flip: int = 2, confidence: float = 0.7, delayed: bool = True) -> list[dict]:
    ...
    return [{"observation": {"label": labels[i], "label_confidence": confidence},
...
def _accuracy(entity: R.Substrate, fixture) -> float:
    hits = 0
    for t, row in zip(entity.traces[-len(fixture):], fixture):
...
def grounded_closed_loop() -> dict:
    ...
    links["proposal_is_sandboxed"] = (entity.ws.read("decision", "gate") or {}).get(
        "activation") is False
    links["outcome_reached_the_self_model"] = len(entity.self_model.history) == len(fixture)
    links["belief_written_with_provenance"] = all(
        b.provenance for b in entity.beliefs.beliefs.values())
    links["belief_revised_by_outcome"] = any(b.retracted for b in entity.beliefs.beliefs.values()) or \
        any(b.verification_status == "verified" for b in entity.beliefs.beliefs.values())
    links["memory_updated"] = len(entity.episodes.store) == len(fixture)
    links["episode_carries_the_outcome"] = all(
        e.outcome is not None for e in entity.episodes.store.values())
    ...
    activity = {
        ...
        "beliefs_revised_without_outcomes": sum(
            1 for b in blind.beliefs.beliefs.values()
            if b.retracted or b.verification_status == "verified"),
    }
    mechanism_active = (activity["self_model_history_without_outcomes"] == 0
    ...
    return {"gate": "grounded_closed_loop", "links": links, "broken_links": broken,
            "mechanism_activity": {"active": mechanism_active, "detail": activity,
                                   "test": "removing the outcome must break the downstream links"},
            "oracle_headroom": {"applicable": False,
            ...
            "reading": ("a closed loop is a chain of receipts from observation to memory. A verbal "
...
def endogenous_allocation() -> dict:
    ...
    def cost_adjusted(entity, fx):
        spent = sum((t["stages"].get("run_perspectives") or {}).get("compute_spent", 0.0)
                    for t in entity.traces)
    ...
    fixed = _run(fixture, cycle_budget=bind,
                 ablate=frozenset({"attend"}))  # no drivers, everything offered in order
    maximum = _run(fixture, cycle_budget=1e6)  # no resource limit at all

    rows = {}
    for name, entity in (("endogenous", endogenous), ("fixed_schedule", fixed),
                         ("maximum_compute", maximum)):
        score, spent = cost_adjusted(entity, fixture)
        rows[name] = {"accuracy": round(_accuracy(entity, fixture), 6),
                      "compute": round(spent, 6), "cost_adjusted": round(score, 6)}
    ...
    oracle = (statistics.fmean(a for a, _ in per_cycle)
              - lam * sum(c for _, c in per_cycle))
    best_control = max(rows["fixed_schedule"]["cost_adjusted"],
                       rows["maximum_compute"]["cost_adjusted"])
    ...
    return {"gate": "endogenous_allocation", "lambda_per_compute_unit": lam, "arms": rows,
            "mechanism_activity": {"active": mechanism_active,
                                   "test": "the three arms must actually spend different compute"},
            "oracle_headroom": {"applicable": True, "oracle": round(oracle, 6),
            ...
            "saturation_note": ("every arm reaches the same accuracy on this fixture, so allocation can "
                                ...
                                "would need to say more" if saturated else ""),
            "passes": mechanism_active and headroom > SESOI and margin > SESOI,
            "reading": ("endogenous allocation earns a pass by beating both a fixed schedule and "
...
def cross_domain_continuity() -> dict:
    ...
    return {"gate": "cross_domain_continuity",
            "sequence": "A to B to A to held out B, one entity, nothing reinitialized",
            "scores": {"a_first": round(a_first, 6), "b_first": round(b_first, 6),
                       "a_return": round(a_return, 6), "b_heldout": round(b_heldout, 6),
                       "fresh_b_control": round(fresh_b_score, 6)},
            "retention": round(retention, 6), "transfer": round(transfer, 6),
            "interference": round(interference, 6),
            "identity_continuity": {"checkpoints": len(identities), "all_distinct": identity_unbroken,
                                    "steps": entity.step_index},
            "mechanism_activity": {"active": mechanism_active,
                                   "test": "the phases must not all score identically"},
            "oracle_headroom": {"applicable": True, "oracle": round(oracle, 6),
                                "clears_sesoi": oracle > SESOI},
            "passes": (mechanism_active and identity_unbroken and oracle > SESOI
                       and transfer > SESOI and interference <= SESOI),
            "reading": ("continuity means A survives B and B benefits from A, on one entity whose "
...
def world_self_control_value() -> dict:
    ...
    changed_paths["resource_allocation"] = (
        ...
        != [t["stages"].get("run_perspectives", {}).get("compute_spent")
            for t in without_self.traces])
    ...
    changed_paths["verification"] = (
        sum(1 for b in with_self.beliefs.beliefs.values() if b.verification_status == "verified")
        != sum(1 for b in without_self.beliefs.beliefs.values()
    ...
    return {"gate": "world_self_control_value", "control_paths": changed_paths,
            "paths_changed": changed,
            "world_model": {"verdict": world["verdict"],
            ...
            "oracle_headroom": {"applicable": False,
                                "reason": ("this gate asks whether model information changes a control "
            ...
            "reading": ("the models earn a pass by changing verification, deferral, selection, "
...
def procedural_transfer() -> dict:
    ...
    return {"gate": "procedural_transfer",
            "arms": {"carried_procedure": round(carried_score, 6),
                     "fresh": round(fresh_score, 6),
                     "retrieval_only": round(retrieval_score, 6),
                     "more_compute": round(richer_score, 6)},
            "best_baseline": round(best_baseline, 6), "margin": round(margin, 6),
            "mechanism_activity": {"active": mechanism_active,
                                   "test": "the reliability table must have moved off its prior"},
            "oracle_headroom": {"applicable": True, "oracle": round(oracle, 6),
                                "clears_sesoi": oracle > SESOI},
            "passes": mechanism_active and oracle > SESOI and margin > SESOI,
            "reading": ("a procedure transfers only if carrying it beats a fresh entity, a retrieval "
...
def unity_under_conflict() -> dict:
    ...
    entity.ws.write("uncertainty", "arbiter",
                    {"interval": report["confidence_interval"],
                     "unresolved": [c["alternative"] for c in report["unresolved_contradictions"]]},
                    provenance="conflict_probe", confidence=1.0)
    checks["globally_available"] = bool((entity.ws.read("uncertainty", "any_reader") or {})
                                        .get("unresolved"))
    ...
    return {"gate": "unity_under_conflict", "checks": checks, "failed": failed,
            "arbitration": {"decision": report["decision"], "deferred": report["deferred"],
            ...
            "reading": ("one entity under conflict keeps the contradiction globally available, keeps the "
...
def classify(results: dict) -> dict:
    ...
    return {
        "passed": sorted(passed), "failed": sorted(set(GATES) - passed),
        ...
        "requires_for_next_level": {
            ...
            "reflective_cognitive_organization": sorted((structural | developmental | reflective)
                                                        - passed),
            "functional_or_proto_nous_candidate": ("all six gates plus terminal long run evidence. "
                                                   "Not available from a closure pass"),
        },
        "never_claimed": list(FORBIDDEN),
        "claim_rule": ("these are functional and architectural classifications. None of them is a claim "
...
def run() -> dict:
    ...
    for name, row in results.items():
        ...
        if not row["mechanism_activity"]["active"]:
            gated.append({"gate": name, "reason": "no mechanism activity",
                          "classification": "instrumentation_failure_not_a_null"})
        elif headroom["applicable"] and not headroom["clears_sesoi"]:
            gated.append({"gate": name, "reason": "no oracle headroom on this bed",
            ...
            nulls.append({"gate": name, "oracle": headroom.get("oracle"),
                          "headroom": headroom.get("headroom", headroom.get("oracle")),
                          "classification": "mechanism_null_on_this_bed"})
    verdict = classify(results)
    return {"schema": "substrate-nous-closure/v1", "gates": list(GATES), "results": results,
            "terminally_gated": gated, "mechanism_nulls": nulls, "verdict": verdict, "sesoi": SESOI,
            "null_versus_failure": ("a gate with an active mechanism and real headroom that measures zero "
            ...
            "not_a_new_layer": ("every gate runs the existing runtime over existing beds and the sealed "
                                "session. Nothing was added to the architecture to pass one"),
            "architecture_freeze": {
                "frozen": True,
                "scope": "src/mop/cognition and tests/cognition",
                "rule": ("further expansion has to be generated by terminal long run evidence. A gate "
                ...
                "what_may_still_change": ("repairs to a defect reproduced by a test, and the beds "
