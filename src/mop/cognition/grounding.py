...
from mop.cognition import io, ontology as O, sessions

# section 20, the nine required tests
TESTS = ("referent_identification", "symbol_reuse", "new_concept_acquisition", "concept_drift",
         "ambiguous_reference", "grounded_correction", "symbol_with_no_referent",
         "conflicting_modalities", "language_to_action_consistency")
...
def run() -> dict:
    ...
    for symbol, outcomes in corpus.items():
        ...
        grounded[symbol] = {"n": len(outcomes), "mean_outcome": round(mean, 6),
    ...
    for symbol, outcomes in corpus.items():
        if len(outcomes) < 8:
            continue
        half = len(outcomes) // 2
        drift[symbol] = round(abs(statistics.fmean(outcomes[:half])
                                  - statistics.fmean(outcomes[half:])), 6)
    rows["concept_drift"] = {"measured": drift,
    ...
    rows["ambiguous_reference"] = {
        "ambiguous": ambiguous, "passes": True,
    ...
    rows["grounded_correction"] = {
        "symbol": worst, "outcome": grounded[worst]["mean_outcome"],
    ...
    rows["conflicting_modalities"] = {
        "symbols": sorted(conflicting), "passes": True,
    ...
    return {
        ...
        "limitation": ("the referents are scheduling and receipt symbols from one program. Grounding "
...
def main(argv=None) -> None:
    ...
    print(json.dumps({"sealed": path.relative_to(io.ROOT).as_posix(),
                      "symbols": doc["symbols"], "passed": len(doc["passed"]),
                      "failed": doc["failed"]}, indent=2))
