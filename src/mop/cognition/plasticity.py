...
LEVEL_NAMES = ("state_update", "memory_write", "memory_consolidation", "head_update", "adapter_update",
               "projection_update", "core_update", "routing_update", "reliability_update",
               "structural_slot_activation")

# section 11.2 and 11.3
FAST_MECHANISMS = ("persistent_state", "working_memory", "temporary_bindings", "prototypes",
                   "domain_local_adapters", "cached_procedures", "active_readout_adaptation")
SLOW_TARGETS = ("adapters", "projections", "procedural_memory", "semantic_memory",
                "reliability_estimates", "selected_core_groups")
...
PERMITTED_REORGANIZATIONS = (
    "activate_dormant_perspective", "retire_ineffective_perspective", "alter_routing_weights",
    "alter_perspective_reliability", "assign_domain_local_adapter", "change_memory_ownership",
    "freeze_or_reopen_declared_component", "activate_preallocated_latent_slot",
...
class Level:
    ...
    def violations(self) -> list[str]:
        v = [f"{self.name}: {f} not declared" for f in safety.ADAPTATION_FIELDS
...
LEVELS: tuple[Level, ...] = (
    _l("state_update", "the current observation and the temporal core state", "workspace temporal region",
       "reversible", 0.01, "carries a stale context across a boundary",
       "misaligned reset control must lose the effect", "reset the region", "fast", False),
    _l("memory_write", "one episode", "episodic store", "reversible", 0.02,
       "writes an unverified generated episode", "promotion requires a verification receipt",
       "quarantine the episode", "fast", False),
    _l("memory_consolidation", "a window of episodes", "semantic and procedural stores",
       "reversible_from_checkpoint", 0.2, "consolidates a coincidence into a rule",
       "held out transfer test", "restore the pre consolidation checkpoint", "slow", False),
    _l("head_update", "the current domain only", "the task head", "reversible_from_checkpoint", 0.3,
       "overfits the domain that triggered it", "held out accuracy on the same domain",
       "restore the previous head", "fast", False),
    _l("adapter_update", "the current domain only", "a domain local adapter",
       "reversible_from_checkpoint", 0.4, "leaks a domain specific bias into shared use",
       "retention on prior domains", "detach the adapter", "fast", False),
    _l("projection_update", "several domains", "the shared projection", "reversible_from_checkpoint", 0.8,
       "shifts the representation every domain depends on", "cross domain retention",
       "restore the previous projection", "slow", True),
    _l("core_update", "several domains and repeated evidence", "selected temporal core groups",
       "reversible_from_checkpoint", 1.0, "destabilizes the component the whole substrate reads",
       "cross domain retention and an objective drift check", "restore the core checkpoint", "slow",
       True),
    _l("routing_update", "measured perspective utility", "the routing weights",
       "reversible_from_checkpoint", 0.3, "locks in a routing that suited one distribution",
       "utility beyond fixed and simple routing after cost", "restore the previous weights", "slow",
       False),
    _l("reliability_update", "the paired prediction and outcome history", "the reliability table",
       "reversible", 0.05, "an early streak becomes a permanent prior",
       "calibration against held out outcomes", "recompute from the sealed history", "fast", False),
    _l("structural_slot_activation", "repeated failure of every active component", "a preallocated slot",
       "reversible_from_checkpoint", 1.0, "grows capacity instead of fixing the cause",
       "the slot must beat the unactivated arm after cost", "deactivate the slot", "slow", True),
)
...
def _proposal(level: Level, adaptation: Adaptation) -> dict:
    return {"information_used": level.information_used, "affected_state": level.affected_state,
            "reversibility": "reversible" if level.reversibility == "reversible" else "irreversible",
            "cost": level.cost, "risk": level.risk, "verification": level.verification,
            "rollback": level.rollback, "checkpoint": adaptation.checkpoint}
...
def slow_adapt(adaptation: Adaptation, *, repetitions: int, held_out: dict, retention: dict,
               min_repetitions: int = 3) -> dict:
    ...
    return {"level": level.name, "applied": adaptation.applied, "refusals": list(gaps),
            "repetitions": repetitions, "held_out": held_out, "retention": retention,
...
def learning_to_learn(results_by_task: dict, *, min_tasks: int = 2, sesoi: float = SESOI) -> dict:
    ...
    for rule, tasks in results_by_task.items():
        ...
        rows[rule] = {
            ...
            "reason": "" if len(helped) >= min_tasks else
                      f"helped on {len(helped)} held out tasks, {min_tasks} required",
        }
    return {"rules": rows, "generalizing_rules": sorted(r for r, v in rows.items() if v["generalizes"]),
...
def reorganize(change: str, *, measured: dict, cost: float, sesoi: float = SESOI) -> dict:
    ...
    if change in safety.FORBIDDEN_REORGANIZATIONS:
        return {"change": change, "permitted": False, "applied": False,
    ...
    if None in (fixed, simple, after):
        return {"change": change, "permitted": True, "applied": False,
    ...
    earned = net > sesoi
    return {"change": change, "permitted": True, "applied": earned,
            "baseline": round(baseline, 6), "reorganized": round(float(after), 6),
            "cost_charged": float(cost), "net_after_cost": round(net, 6), "sesoi": sesoi,
            "reason": "" if earned else
                      "does not beat the stronger of fixed and simple routing once cost is charged"}
...
def declaration() -> dict:
    ...
    return {
        ...
        "levels": [{"name": lv.name, "information_used": lv.information_used,
                    "affected_state": lv.affected_state, "reversibility": lv.reversibility,
                    "cost": lv.cost, "risk": lv.risk, "verification": lv.verification,
                    "rollback": lv.rollback, "speed": lv.speed,
                    "touches_shared_parameters": lv.touches_shared_parameters} for lv in LEVELS],
        ...
        "slow_rule": ("repeated evidence, a held out improvement above the SESOI, retention on prior "
                      "domains and a checkpoint, all four"),
        "policies": [{"name": p.name, "trigger": p.trigger,
                      "information_used": sorted(p.information_used), "learned": p.learned}
                     for p in POLICIES],
...
def reorganization_declaration() -> dict:
    return {
        ...
        "evidence_rule": ("reorganization is credited only when it beats the stronger of fixed and simple "
                          "routing after its own cost is charged"),
        "permitted_is_not_sufficient": ("appearing on the permitted list bounds a change, it does not "
                                        "earn it"),
        "activation": False,
    }
...
def developmental_history() -> dict:
    return {
        ...
        "generalization_rule": ("an adaptation rule counts as generalizing only after it helps on held "
                               "out tasks it was not derived from"),
        "min_held_out_tasks": 2,
        "measured_rules": {},
        "honest_state": ("no adaptation rule has yet been evaluated across tasks, so no developmental "
