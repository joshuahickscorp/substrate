...
from mop.cognition import io, memory as M, perspectives as PS, selfmodel as SM, workspace as W

SESOI = 0.05

# section 14, what thinking has to beat, and what does not count as evidence for it
ALTERNATIVES = ("larger_static_model", "stronger_readout", "longer_context", "more_samples",
                "more_tokens", "tool_only_system")
NOT_EVIDENCE = ("latency", "hidden_activations")
THINKING_ROUTES = ("maintaining_state", "transforming_representations", "generating_intermediate_structures",
                   "comparing_alternatives", "correcting_errors", "allocating_compute",
                   "reusing_abstractions", "transferring_strategies")

# section 15.1, what continuity has to preserve and what it has to survive
CONTINUITY_SURFACES = ("goals", "unresolved_questions", "memory", "world_state", "self_state",
                       "commitments", "uncertainty", "project_context")
STRESSORS = ("interruption", "checkpoint_restore", "context_removal", "session_change",
             "model_body_replacement", "long_delay")

# section 15.2
UNITY_MEASURES = ("global_availability", "shared_goals", "cross_perspective_memory",
                  "conflict_resolution", "consistent_action", "preservation_of_alternatives")

# section 15.3
REFLECTIVE_QUESTIONS = ("what_is_known", "what_is_not_known", "where_a_belief_came_from",
                        "what_evidence_supports_it", "confidence", "failure", "what_would_change_it")
...
class Entity:
    ...
    def observe(self, text: str, **regions):
        ...
        for region, payload in regions.items():
            ...
            self.ws.write(region, spec.writers[0], payload, provenance=f"observation:{len(self.transcript)}",
                          confidence=0.8 if spec.confidence else None)
        return self

    def checkpoint(self) -> dict:
        return {"workspace": self.ws.checkpoint(),
...
def thinking_battery(claim: dict) -> dict:
    ...
    if named_non_evidence and not [k for k in beaten if k in ALTERNATIVES]:
        return {"schema": "substrate-thinking-battery/v1", "scored": False, "supported": False,
                "reason": ("the claim rests on latency or hidden activations, which section 14 excludes "
                           "as evidence of thinking"),
                "alternatives_declared": list(ALTERNATIVES), "alternatives_compared": [],
                "alternatives_missing": missing, "non_evidence_named": named_non_evidence}

    margins = {k: round(float(v.get("substrate", 0.0)) - float(v.get("alternative", 0.0)), 6)
               for k, v in beaten.items() if k in ALTERNATIVES}
...
def continuity_battery(entity: Entity, probes: dict) -> dict:
    ...
    for surface in CONTINUITY_SURFACES:
        ...
        if probe is None:
            rows[surface] = {"probed": False, "owned": False, "replay": False, "replay_full": False,
                             "reason": "no probe declared for this surface"}
            continue
        expected = probe["expected"]
        rows[surface] = {"probed": True, "owned": answer_from_owned(surface) == expected,
                         "replay": str(expected) in replayed, "replay_full": str(expected) in full}
    ...
    return {
        ...
        "stressors_not_applied": [s for s in STRESSORS
                                  if s not in ("context_removal", "checkpoint_restore")],
        ...
        "support_rests_on": ("the contested margin only. The surfaces no replay can reach are reported "
        ...
        "why_the_second_control": ("the unbounded replay arm exists to prove the matched arm lost to the "
...
def unity_battery(entity: Entity, outputs: list[PS.Output]) -> dict:
    ...
    rows = {
        "global_availability": {"regions_readable_by_any_component": readable,
                                "regions_writable_by_any_component": writable_by_anyone,
                                "passes": readable > 0 and writable_by_anyone == 0},
        "shared_goals": {"goal_region_populated": "goal" in view,
                         "passes": "goal" in view},
        "cross_perspective_memory": {"episodes_visible": len(entity.episodes.store),
                                     "passes": len(entity.episodes.store) > 0},
        "conflict_resolution": {"contradictions_named": len(report["unresolved_contradictions"]),
                                "deferred": report["deferred"],
                                "passes": bool(report["unresolved_contradictions"]) or
                                          report["decision"] is not None},
        "consistent_action": {"decision": report["decision"], "deferred": report["deferred"],
                              "passes": report["decision"] is not None or report["deferred"]},
        "preservation_of_alternatives": {"minority_preserved": report["minority_preserved"],
                                         "passes": report["minority_preserved"] > 0 or
                                                   len(outputs) <= 1},
    }
    return {"schema": "substrate-unified-cognition-battery/v1", "measures": rows,
            ...
            "note": ("global availability is scored as broad read plus narrow write. A battery that "
...
def reflective_report(entity: Entity, belief_id: str) -> dict:
    ...
    if fact is None:
        return {"schema": "substrate-reflective-access-battery/v1", "belief": belief_id,
                "answered": False, "failed_closed": True,
    ...
    return {
        ...
        "answers": {
            ...
            "what_is_not_known": [k for k in M.EPISODE_FIELDS
                                  if k not in ("observation", "outcome")][:3],
            ...
            "what_would_change_it": ("a measurement on the same bed that contradicts "
                                     f"{fact.statement!r} above the SESOI"),
        },
...
def reflective_battery(entity: Entity, belief_ids: list[str]) -> dict:
    ...
    return {"schema": "substrate-reflective-access-battery/v1",
            "questions": list(REFLECTIVE_QUESTIONS),
            "reports": reports,
            "answered": len(answered), "failed_closed": len(reports) - len(answered),
...
def _demo_entity() -> tuple[Entity, dict]:
    e = Entity()
    e.observe("the operator set the goal to finish the temporal graph",
              goal=["finish the temporal graph"])
    ...
    e.semantic.assert_(M.Fact("f1", "two of three beds are temporal", 0.8,
    ...
    e.self_model.record(SM.SelfFact("unfinished_tasks", ["speech_stream principal"],
                                    source="runs/.../e2_principal"))
    ...
    probes = {
        "goals": {"expected": ["finish the temporal graph"],
                  "read": lambda x: x.ws.read("goal", "reader")},
        "uncertainty": {"expected": {"open": ["speech_stream"]},
                        "read": lambda x: x.ws.read("uncertainty", "reader")},
        "world_state": {"expected": {"beds": 3}, "read": lambda x: x.ws.read("world", "reader")},
        "memory": {"expected": "har_stream principal terminal",
                   "read": lambda x: x.episodes.store["ep1"].outcome},
        "self_state": {"expected": ["speech_stream principal"],
...
def declaration() -> dict:
    ...
    return {
        ...
        "thinking": thinking_battery({"routes": ["maintaining_state"],
                                      "beats": {"latency": {"substrate": 1, "alternative": 0}}}),
        ...
        "honest_state": ("the thinking battery is shown refusing a latency only claim, which is what it "
