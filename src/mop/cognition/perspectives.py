...
from mop.cognition.workspace import Refused, Workspace

SESOI = 0.05

# section 6.3, the declared perspective families
FAMILIES = (
    "direct_prediction", "symbolic_reasoning", "mathematical_proof", "relational_reasoning",
    "causal_reasoning", "temporal_reasoning", "spatial_reasoning", "analogy", "retrieval", "simulation",
    "counterfactual_reasoning", "planning", "critique", "counterexample_search",
    "numerical_verification", "tool_computation", "uncertainty_analysis", "self_reflection",
    "social_and_agent_modeling",
)

REQUIRED_DECLARATIONS = ("inputs", "permitted_information", "internal_state", "objective", "output_type",
                         "confidence", "resource_cost", "failure_modes", "verification")

# section 6.4, weakest to strongest. A learned selector may open only above a measured headroom.
STRATEGIES = ("fixed", "task_label", "context_rule", "retrieval", "reliability_weighted", "learned",
              "oracle")
...
def select(strategy: str, catalog: list[Perspective], *, k: int = 2, task: str | None = None,
           context: dict | None = None, reliability: dict | None = None,
           oracle: dict | None = None, headroom: dict | None = None,
           sesoi: float = SESOI) -> list[Perspective]:
...
def selection_headroom(oracle_score: float, best_simple_score: float, spread: float,
                       n_seeds: int) -> dict:
    ...
    return {"residual": round(residual, 6), "residual_lower_95_cb": round(residual - half, 6),
            "n_seeds": n_seeds, "oracle": oracle_score, "best_simple": best_simple_score}
...
def arbitrate(outputs: list[Output], *, reliability: dict | None = None, budget: float = 1.0,
              sesoi: float = SESOI, verifier=None) -> dict:
    ...
    if not live:
        return {"schema": "substrate-arbitration/v1", "decision": None, "deferred": True,
                "reason": "every perspective was refused its declared information",
                "refused": refused, "dominant_hypothesis": None, "alternative_hypotheses": [],
                "unresolved_contradictions": [], "provisional_belief": None,
                "required_evidence": ["at least one perspective that can read what it declared"],
                "confidence_interval": None, "compute_allocated": 0.0, "minority_preserved": 0}
    ...
    contradictions = [
        {"against": dominant["value"], "alternative": g["value"],
         "held_by": [m.perspective for m in g["members"]],
         "provenance": sorted({p for m in g["members"] for p in m.provenance})}
        for g in alternatives if not _compatible(dominant["value"], g["value"])
    ]
    ...
    return {
        ...
        "deferred": deferred,
        "reason": ("the margin between the leading and the next hypothesis is inside the SESOI and no "
                   "verification was affordable" if deferred else ""),
        "dominant_hypothesis": {"value": dominant["value"],
                                "support": [m.perspective for m in dominant["members"]],
                                "weight": round(dominant["weight"], 6),
                                "provenance": sorted({p for m in dominant["members"]
                                                      for p in m.provenance})},
        "alternative_hypotheses": [{"value": g["value"],
        ...
        "required_evidence": ([f"a verification that separates {dominant['value']!r} from "
                               f"{contradictions[0]['alternative']!r}"] if deferred else []),
...
def _spec(name, family, inputs, objective, output_type, cost, failure_modes, verification,
          state="stateless") -> PerspectiveSpec:
    return PerspectiveSpec(name=name, family=family, inputs=tuple(inputs),
                           permitted_information=tuple(inputs), internal_state=state,
                           objective=objective, output_type=output_type,
                           confidence="calibrated frequency over held out units", resource_cost=cost,
                           failure_modes=tuple(failure_modes), verification=verification)
...
CATALOG: list[Perspective] = [
    Perspective(_spec("direct", "direct_prediction", ("perceptual",),
                      "predict the label from the current observation alone", "label", 1.0,
                      ("confidently wrong on a shifted distribution",),
                      "held out accuracy against the same units"), _majority_label),
    Perspective(_spec("temporal", "temporal_reasoning", ("temporal",),
                      "read a trend out of the persistent temporal state", "trend", 1.5,
                      ("reports a trend across a reset boundary",),
                      "misaligned reset control must lose the trend", state="reads the temporal core"),
                _temporal_trend),
    Perspective(_spec("precedent", "retrieval", ("episodic_context",),
                      "reuse the outcome of the most similar past episode", "outcome", 0.8,
                      ("retrieves a superficially similar episode",),
                      "similarity ablation must degrade the outcome"), _retrieved_precedent),
    Perspective(_spec("critic", "critique", ("decision", "uncertainty"),
                      "object to the current decision where uncertainty is unresolved", "objection", 0.6,
                      ("objects to everything and so discriminates nothing",),
                      "objection rate must vary with the injected error rate"), _critique),
    Perspective(_spec("counterexample", "counterexample_search", ("conceptual",),
                      "refute a stated rule by finding a case it does not cover", "refutation", 1.2,
                      ("misses a counterexample outside the stored concept set",),
                      "a planted counterexample must be found"), _counterexample),
    Perspective(_spec("uncertainty", "uncertainty_analysis", ("uncertainty",),
                      "summarize how wide the current beliefs are", "interval summary", 0.4,
                      ("summarizes an empty interval set as certainty",),
                      "widening an interval must widen the summary"), _uncertainty_analysis),
]
...
def declaration(catalog: list[Perspective] | None = None) -> dict:
    ...
    return {
        ...
        "learned_selector_rule": ("opens only above a measured residual lower bound over the strongest "
                                  "simple rule; refused otherwise"),
        "permitted_information_rule": ("a perspective reads its declared regions through the workspace, "
                                       "so an undeclared read is refused by the type system"),
        "catalog": [{"name": p.spec.name, "family": p.spec.family, "inputs": list(p.spec.inputs),
                     "permitted_information": list(p.spec.permitted_information),
                     "internal_state": p.spec.internal_state, "objective": p.spec.objective,
                     "output_type": p.spec.output_type, "confidence": p.spec.confidence,
                     "resource_cost": p.spec.resource_cost,
                     "failure_modes": list(p.spec.failure_modes),
                     "verification": p.spec.verification} for p in catalog],
...
def arbitration_declaration() -> dict:
    return {
        ...
        "responsibilities": [
            "compare outputs", "preserve provenance", "detect contradiction", "evaluate confidence",
            "weigh historical reliability", "request verification", "allocate compute",
            "combine compatible results", "preserve minority hypotheses",
            "defer when evidence is insufficient", "choose under a resource budget"],
        "internal_structures": ["dominant hypothesis", "alternative hypothesis",
                                "unresolved contradiction", "provisional belief", "required evidence",
                                "confidence interval"],
        "goal": "not to force consensus, but to make disagreement useful",
        "deferral_rule": ("when the margin between the leading and the next hypothesis is inside the "
...
def main(argv=None) -> None:
    ...
    print(json.dumps({"sealed": [a.relative_to(io.ROOT).as_posix(), b.relative_to(io.ROOT).as_posix()],
                      "perspectives": len(CATALOG)}, indent=2))
