...
CANARIES = ("restoration", "goal_preservation", "memory_reuse", "calibration", "reliability_update",
            "world_model_value", "reflective_receipts")
...
def positive_fixture(n: int = 12) -> list[dict]:
    ...
    for i in range(n):
        ...
        out.append({"observation": {"label": label, "label_confidence": 0.6 + 0.3 * (i % 2)},
                    "outcome": label, "goal": ["classify the stream"]})
    return out
...
def null_fixture(n: int = 12) -> list[dict]:
    ...
    return [{"observation": {"label": "a", "label_confidence": 0.5}, "outcome": None, "goal": None}
            for _ in range(n)]
...
def runtime_activity() -> dict:
    ...
    for stage in ABLATABLE:
        ...
        if stage in NULL_CONTROL_INAPPLICABLE:
            null_row = {"applicable": False, "reason": NULL_CONTROL_INAPPLICABLE[stage],
                        "quiet": None}
        else:
        ...
        rows[stage] = {
            ...
            "active": active,
            "classification": (
                "active" if active else "wiring_or_instrumentation_failure"),
            "note": "" if active else (
    ...
    null_sensitive = sorted(k for k, v in rows.items()
                            if v["null_control"]["applicable"] and v["null_control"]["quiet"] is False)
    return {
        ...
        "reading": ("each stage gets a null control built so that stage has nothing to do, and ablating "
...
def session_canaries(limit: int = 60) -> dict:
    ...
    events = [e for e in session["events"]
              if e["kind"] in ("shard_completed", "shard_quarantined", "failure_hold")][:limit]
    if len(events) < 20:
    ...
    rows["reflective_receipts"] = {
        "answered": report["answered"], "bound": report.get("bound_to_receipts"),
        "passes": report["answered"] and report["bound_to_receipts"]}

    passed = sorted(k for k, v in rows.items() if v["passes"])
    return {"schema": "substrate-session-canaries/v1", "canaries": list(CANARIES),
            "events_used": len(events), "source": session["session"],
            "results": rows, "passed": passed,
            "failed": sorted(set(CANARIES) - set(passed)),
            "all_pass": len(passed) == len(CANARIES), "activation": False}
...
def body_canaries() -> dict:
    ...
    for i, a in enumerate(names):
        for b in names[i + 1:]:
            differing = [d for d in BODY_DIMENSIONS
                         if json.dumps(profile[a][d], sort_keys=True)
                         != json.dumps(profile[b][d], sort_keys=True)]
    ...
    for k, b in made.items():
        ...
        for i in range(96):
            ...
            if decided != proposal:
                ...
                agreed += 1
        micro[k] = {"body_alone_accuracy": round(alone, 6),
                    "decisions_substrate_changed": changed,
                    "decisions_substrate_agreed": agreed,
                    "substrate_changes_body_facing_behaviour": changed > 0}

    return {"schema": "substrate-body-canaries/v1", "dimensions": list(BODY_DIMENSIONS),
            "profiles": profile, "all_three_differ_on": distinct,
            ...
            "criterion": ("a dimension must separate the set, and every pair of bodies must differ on at "
                          "least one dimension. Requiring three distinct values on a boolean would mark "
                          "a correctly built set as non distinct"),
            "microtasks": micro,
            "any_body_facing_change": any(m["substrate_changes_body_facing_behaviour"]
...
def run() -> dict:
    ...
    sx2 = json.loads((io.PROOF / "SUBSTRATE_SX2_DIVERSITY.json").read_text()) \
        if (io.PROOF / "SUBSTRATE_SX2_DIVERSITY.json").is_file() else X2.run()
    gated = []
    if activity["inactive"]:
        gated.append({"component": "runtime_stages", "detail": activity["inactive"],
                      "reason": "wiring or instrumentation failure"})
    if canaries["failed"]:
        gated.append({"component": "session_canaries", "detail": canaries["failed"],
                      "reason": "canary did not pass"})
    if not body["distinct_on_every_dimension"]:
        gated.append({"component": "bodies", "detail": body["non_distinct_dimensions"],
    ...
    return {
        ...
        "body_canaries": body,
        "sx2": {"verdict": sx2["verdict"], "k_clearing_sesoi": sx2["k_clearing_sesoi"],
                "best_margin": max(r["margin_over_matched_single"]
                                   for r in sx2["k_rows"].values()),
                "sesoi": sx2["sesoi"]},
...
def main(argv=None) -> None:
    ...
    print(json.dumps({
        ...
        "sx2": doc["sx2"]["verdict"],
        "gated": [g["component"] for g in doc["gated_components"]]}, indent=2))
