...
MESSAGE_KINDS = ("inference", "hidden_state", "selected_activations", "tool_request", "memory_request",
                 "verification_request", "adaptation_proposal", "resource_report", "checkpoint")

INTEGRATION_MODES = ("external_cognitive_shell", "sidecar_temporal_core", "adapter_layer",
                     "internal_recurrent_layer", "multi_model_orchestrator", "end_to_end_owned_model")
...
REQUIRED_FIELDS = {
    ...
    "adaptation_proposal": tuple(("information_used", "affected_state", "reversibility", "cost", "risk",
...
def conformance(contract: BodyContract) -> dict:
    ...
    return {"body": contract.name, "mode": contract.mode,
            "implements": sorted(contract.implements), "missing": missing,
...
def declaration(bodies: list[BodyContract] | None = None) -> dict:
    ...
    return {
        ...
        "why_three_bodies": ("running the same Substrate against a compact specialist, a larger general "
        ...
        "honest_state": ("no model body is attached. The contract exists and is testable; the comparison "
