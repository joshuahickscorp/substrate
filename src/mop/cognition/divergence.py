...
from mop.cognition import io, runtime as R, sessions

# section 29, the nine dimensions
DIMENSIONS = ("memory", "procedures", "world_models", "self_models", "perspective_reliability",
              "specialization", "transfer", "robustness", "goal_progress")
...
def _histories(session: dict) -> tuple[list, list, list]:
    ...
    events = [e for e in session["events"] if e["kind"] in
              ("shard_completed", "shard_quarantined", "failure_hold")]
    if len(events) < 60:
...
def _observation(event: dict) -> dict:
    ...
    return {"label": "ok" if ok else "bad",
            "label_confidence": 0.9 if ok else 0.6,
            "stage": event.get("stage", ""), "kind": event["kind"]}
...
def _measure(entity: R.Substrate, held_out: list) -> dict:
    ...
    for event in held_out:
        obs = _observation(event)
        trace = entity.step(obs, outcome=obs["label"])
    ...
    return {
        ...
        "specialization": round(
            statistics.pstdev(list(entity.reliability.values())) if len(entity.reliability) > 1 else 0.0,
            6),
...
def run() -> dict:
    ...
    return {
        ...
        "control_rule": ("two instances given the same history must not diverge, or the divergence below "
        ...
        "verdict": (
            "invalid_control" if not control_clean else
            "no_divergence" if not any(v > 0 for v in divergence.values()) else
            "divergence_with_transfer_difference" if useful else
            "divergence_without_transfer_value"),
        "reading": (
            ...
            if control_clean and not any(v > 0 for v in divergence.values()) else
            "divergence was measured; whether it carries transfer value is reported separately"),
        "claim_boundary": ("this measures developmental individuality as differences in owned state that "
...
def main(argv=None) -> None:
    ...
    print(json.dumps({"sealed": path.relative_to(io.ROOT).as_posix(),
                      ...
                      "transfer_gap": doc["transfer_gap"], "verdict": doc["verdict"]}, indent=2))
