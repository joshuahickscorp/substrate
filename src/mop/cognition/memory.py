...
EPISODE_FIELDS = ("context", "observation", "internal_state", "goal", "action", "outcome", "error",
                  "perspectives_used", "verification", "confidence", "cost", "later_usefulness")
...
class WorkingMemory:
    ...
    def measure(self, probe: list[tuple[str, object]]) -> dict:
        ...
        return {"declared_capacity": self.capacity, "measured_held": held,
...
class ProceduralMemory:
    ...
    def transfer_test(self, procedure_id: str, evaluated_on: list[str], score: float,
                      baseline: float) -> dict:
        ...
        p.transfer = {"evaluated_on": list(evaluated_on), "score": score, "baseline": baseline,
                      "improves": score > baseline, "held_out": True}
        return p.transfer
...
POLICIES: tuple[ConsolidationPolicy, ...] = (
    ...
    ConsolidationPolicy("boundary_triggered", frozenset({"episode_index", "context_boundary"}), True,
                        _boundary),
    ConsolidationPolicy("performance_triggered", frozenset({"episode_index", "confidence"}), True,
                        _performance),
    ConsolidationPolicy("verification_triggered", frozenset({"episode_index", "verification"}), True,
                       _verification),
    ConsolidationPolicy("repetition_triggered", frozenset({"episode_index", "action_history"}), True,
                        _repetition),
...
def compare_policies(episodes: list[Episode], state: dict | None = None) -> dict:
    ...
    return {"selected": chosen, "distinct_selections": distinct, "policies": len(POLICIES),
...
def hygiene(records: dict, *, audit_required: set, requests: list[tuple[str, str]]) -> dict:
    ...
    for record_id, action in requests:
        ...
        if action == "delete" and record_id in audit_required:
            refused.append({"record": record_id, "action": action,
...
def declaration() -> dict:
    ...
    probe = [Episode(f"e{i}", action="a" if i % 2 else "b", confidence=0.3 + 0.1 * (i % 4),
    ...
    return {
        ...
        "working_memory": {"declared_capacity": wm.capacity, "decay_per_tick": wm.decay,
                           "measured": measured,
                           "measures": ["capacity", "interference", "decay", "refresh",
                                        "prioritization", "downstream value"]},
        "episodic_memory": {"episode_fields": list(EPISODE_FIELDS), "classes": list(EPISODE_CLASSES),
                            "promotion_rule": ("a generated episode needs a verification receipt before "
                                               "it can become training material")},
        "semantic_memory": {"kinds": ["concept", "fact", "relation", "rule", "abstraction", "exception"],
                            "supersession_rule": ("a superseded fact stays in the store marked with its "
                                                  "successor, so a belief keeps a traceable chain")},
        "procedural_memory": {"transfer_rule": ("a procedure is transferable only after evaluation on "
                                                "episodes it did not come from")},
        "consolidation": {"policies": [{"name": p.name, "information_used": sorted(p.information_used),
                                        "available_at_decision_time": p.available_at_decision_time}
                                       for p in POLICIES],
                          "comparison_on_a_probe_stream": comparison},
        "hygiene": {"actions": list(HYGIENE_ACTIONS),
