...
REQUIRED_DECLARATIONS = ("shape", "persistence", "timescale", "readers", "writers", "provenance",
                         "confidence", "cost", "reset", "update_rule")
...
def _r(name, shape, persistence, timescale, readers, writers, provenance, confidence, cost, reset,
       update_rule) -> RegionSpec:
    return RegionSpec(name, shape, persistence, timescale, tuple(readers), tuple(writers), provenance,
                      confidence, cost, reset, update_rule)
...
REGIONS: tuple[RegionSpec, ...] = (
    _r("perceptual", "observation vector", "step", "immediate", ALL, ("sensor",), True, False, 0.1,
       "step", "overwrite from the current observation"),
    _r("temporal", "core hidden state", "episode", "fast", ALL, ("temporal_core",), True, False, 0.2,
       "episode", "recurrent update from the selected temporal core"),
    _r("ontological", "typed item graph", "persistent", "slow", ALL, ("ontology",), True, True, 0.3,
       "persistent", "merge only on evidence, and a merge stays reversible"),
    _r("epistemic", "belief store with justification edges", "persistent", "medium", ALL,
       ("epistemology",), True, True, 0.3, "persistent",
       "retraction propagates to every dependant before any confidence is read"),
    _r("conceptual", "concept bindings", "episode", "medium", ALL, ("semantic_memory", "perspective"),
       True, True, 0.2, "episode", "merge on higher confidence, keep the loser as an alternative"),
    _r("goal", "goal stack", "goal", "slow", ALL, ("goal_authority",), True, False, 0.1, "goal",
       "push and pop only through an authorized goal decomposition"),
    _r("working_memory", "bounded slot table", "episode", "fast", ALL, ("perspective", "arbiter"), True,
       True, 0.3, "episode", "priority eviction when the slot budget is exceeded"),
    _r("episodic_context", "recent episode window", "episode", "medium", ALL, ("episodic_memory",), True,
       False, 0.3, "episode", "append with decay, never rewrite a sealed episode"),
    _r("world", "entity and relation graph", "persistent", "slow", ALL, ("world_model",), True, True, 0.4,
       "persistent", "bayesian style update against measured outcomes"),
    _r("self", "measured internal facts", "persistent", "slow", ALL, ("self_model",), True, True, 0.4,
       "persistent", "replace only from a measured comparison against actuals"),
    _r("uncertainty", "per belief interval", "episode", "fast", ALL, ("arbiter", "self_model"), True,
       True, 0.1, "episode", "widen on contradiction, never narrow without new measurement"),
    _r("decision", "chosen action and rationale", "step", "immediate", ALL, ("arbiter",), True, True, 0.1,
       "step", "written once per decision, superseded rather than edited"),
)
...
class Workspace:
    ...
    def broadcast(self) -> dict:
        ...
        """
        return {name: {"value": e.value, "writer": e.writer, "provenance": e.provenance,
                       "confidence": e.confidence, "step": e.step}
    ...
    def reset(self, trigger: str):
        ...
        cleared = [n for n, s in self.specs.items()
                   if n in self.store and PERSISTENCE.index(s.reset) <= order]
        for name in cleared:
            del self.store[name]
        return cleared

    def checkpoint(self) -> dict:
        return {"step": self.step, "spent": self.spent, "writes": self.writes,
...
def capacity(ws: Workspace) -> dict:
    ...
    return {"regions": len(ws.specs), "slot_cost_total": round(sum(s.cost for s in ws.specs.values()), 4),
            "budget": ws.budget}
...
def declaration() -> dict:
    ...
    return {
        ...
        "regions": [{"name": s.name, "shape": s.shape, "persistence": s.persistence,
                     "timescale": s.timescale, "readers": list(s.readers), "writers": list(s.writers),
                     "provenance_required": s.provenance, "confidence_required": s.confidence,
                     "cost": s.cost, "reset": s.reset, "update_rule": s.update_rule}
                    for s in REGIONS],
        "all_regions_fully_declared": not violations,
        "declaration_violations": violations,
        "access_rule": ("reading is broad and writing is narrow. Global availability is a read property "
                        "and does not widen who may write"),
        "control": {"class": "UntypedWorkspace",
...
def main(argv=None) -> None:
    ...
    print(json.dumps({"sealed": path.relative_to(io.ROOT).as_posix(),
                      ...
                      "capacity_matched": doc["capacity_matched"]}, indent=2))
