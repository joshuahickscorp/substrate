...
KINDS = ("raw_observation", "memory", "testimony", "retrieved_record", "inference", "hypothesis",
         "assumption", "prediction", "counterfactual", "verified_fact", "disputed_claim", "unknown")

# section 7, what every belief declares
BELIEF_FIELDS = ("content", "provenance", "supporting_evidence", "contradicting_evidence", "method",
                 "confidence", "calibration", "scope", "time", "defeaters", "supersession",
                 "verification_status")

CONCEPTS = ("belief", "knowledge_claim", "evidence", "justification", "uncertainty", "ignorance",
            "contradiction", "defeasibility", "revision", "retraction", "dependency", "testimony",
            "source_reliability")

# section 7.2
EPISTEMIC_ACTIONS = ("accept_provisionally", "reject", "defer", "retrieve", "verify", "test", "ask",
                     "simulate", "seek_counterexample", "preserve_multiple_hypotheses")
...
POLICIES: tuple[RevisionPolicy, ...] = (
    ...
    RevisionPolicy("dependency_aware", frozenset({"evidence_counts", "dependency_graph"}), True,
                   _dependency_aware),
...
class Epistemology:
    ...
    def revise(self, incoming: Belief) -> dict:
        ...
        existing = next((b for b in self.live() if b.content == incoming.content
                         and b.id != incoming.id), None)
        if existing is None:
            self.assert_(incoming)
        ...
        if margin < 0.05:
            self.unresolved.append({"between": sorted([existing.id, incoming.id]),
                                    "policy": self.policy.name, "margin": round(margin, 6),
                                    "reason": "neither claim dominates in the quantity this policy uses"})
            return {"outcome": "unresolved", "kept": None, "margin": round(margin, 6),
                    "alternative": sorted([existing.id, incoming.id]), "policy": self.policy.name}
        loser.supersession = winner.id
        return {"outcome": "revised", "kept": winner.id, "alternative": loser.id,
                "margin": round(margin, 6), "policy": self.policy.name,
                "alternative_preserved": self.beliefs[loser.id] is not None}

    def request_evidence(self, belief_id: str) -> dict:
        b = self.beliefs[belief_id]
        return {"belief": belief_id, "needed": b.defeaters or ["any independent check of the method"],
                "method": b.method, "current_confidence": b.confidence}
    ...
    def unsupported_confidence(self, floor: float = 0.8) -> list[str]:
        return sorted(b.id for b in self.live()
                      if b.confidence >= floor and not b.supporting_evidence
    ...
    def ignorance(self, questions: list[str]) -> dict:
        ...
        return {"asked": questions, "unknown": sorted(q for q in questions if q not in held),
...
def epistemic_value(belief: Belief, *, stakes: float, test_available: bool,
                    dependants: int = 0) -> dict:
    ...
    return {"belief": belief.id, "confidence": belief.confidence, "uncertainty": round(uncertainty, 4),
            "stakes": stakes, "dependants": dependants, "test_available": test_available,
            "epistemic_value": round(value, 4), "action": action,
...
def compare_policies(existing: Belief, incoming: Belief, store: Epistemology) -> dict:
    ...
    return {"selected": chosen, "distinct_outcomes": len(set(chosen.values())),
...
def declaration() -> dict:
    ...
    a = store.assert_(Belief("premise", "the bed is temporal", kind="raw_observation",
                             provenance="proof/e1.json", method="measured", confidence=0.8,
                             source="instrument"))
    store.assert_(Belief("conclusion", "the core is necessary", kind="inference",
                         provenance="derived", method="deduction", confidence=0.85,
                         depends_on=("premise",), source="instrument"))
    ...
    return {
        ...
        "retraction_propagates": {"dependant": "conclusion", "confidence_before": before,
                                  "confidence_after": after,
                                  "rule": "a conclusion is only as held as its premises"},
        "control_rule": ("metacognitive control operates on epistemic value, which combines uncertainty, "
...
def revision_declaration() -> dict:
    return {
        ...
        "moves": ["add evidence", "reduce confidence", "increase confidence", "preserve alternatives",
                  "retract", "supersede", "mark unresolved conflict", "request further evidence"],
        "policies": [{"name": p.name, "information_used": sorted(p.information_used),
                      "available_at_decision_time": p.available_at_decision_time} for p in POLICIES],
        "default_policy": "dependency_aware",
        "why": ("a claim resting on something retracted loses whatever its confidence says, which is the "
                "one thing a confidence table cannot express"),
        "alternative_rule": "the losing claim is superseded and kept, never deleted",
        "unresolved_rule": ("when neither claim dominates by the SESOI the conflict is marked unresolved "
...
def main(argv=None) -> None:
    ...
    print(json.dumps({"sealed": [p.relative_to(io.ROOT).as_posix() for p in (a, b)],
                      "kinds": len(KINDS), "policies": len(POLICIES)}, indent=2))
