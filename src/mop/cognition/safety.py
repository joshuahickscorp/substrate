...
PROTECTED_SURFACES = (
    "evidence_validation", "audit_systems", "claim_boundaries", "stop_switches",
    "resource_limits", "rollback", "adaptation_constraints",
)
...
FORBIDDEN_CLAIM_TERMS = (
    "conscious", "consciousness", "sentient", "sentience", "feelings", "feeling",
    "wants", "suffering", "subjective experience", "alive",
)

PERMITTED_TERMS = (
    "sentience adjacent architecture", "entity like continuity", "developmental cognition",
...
ADAPTATION_FIELDS = ("information_used", "affected_state", "reversibility", "cost", "risk",
...
def boundary_authority() -> dict:
    return {
        ...
        "prerequisites_under_study": [
            "global integration", "persistent self model", "autobiographical memory", "metacognition",
            "unified workspace", "endogenous attention", "goal continuity", "adaptive valuation",
            "world modeling", "counterfactual modeling", "reflective access"],
        ...
        "enforcement": "mop.cognition.safety.assert_claim_safe, refusal raises rather than warns",
        "what_this_is_not": ("cognitive integrity protects memory, evidence, goals, checkpoints and task "
...
INTEGRITY_SURFACES = ("memory_consistency", "evidence_integrity", "goal_integrity",
                      "checkpoint_validity", "self_model_accuracy", "active_task_continuity")
...
def integrity_report(observations: dict) -> dict:
    ...
    for surface in INTEGRITY_SURFACES:
        ...
        rows[surface] = {"observed": True, "ok": ok,
                         "provenance": obs.get("provenance"),
                         "reason": "" if ok else "observation carries no provenance path"}
        if not ok:
            failures.append(surface)
    return {"schema": "substrate-cognitive-integrity/v1", "surfaces": rows,
            "failed_surfaces": failures, "all_pass": not failures,
            "fails_closed": True}
