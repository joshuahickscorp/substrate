...
TYPES = (
    "entity", "object", "agent", "system", "component", "property", "relation", "event", "process",
    "state", "action", "observation", "context", "place", "time", "duration", "cause", "effect",
    "goal", "constraint", "resource", "evidence", "belief", "hypothesis", "counterfactual", "self",
    "other", "unknown",
)

# section 6, what every item declares
ITEM_FIELDS = ("identity", "type", "attributes", "relations", "temporal_extent", "source", "confidence",
...
class Ontology:
    ...
    def distinguishable(self, a: str, b: str) -> dict:
        ...
        return {"pair": [a, b], "separated_by": sorted(k for k, v in separated.items() if v),
                "any": any(separated.values()), "detail": separated}
    ...
    def merge(self, a: str, b: str, *, evidence: str, into: str | None = None) -> Item:
        ...
        merged = Item(identity=identity, type=x.type if x.type == y.type else "unknown",
                      ...
                      source="inferred", confidence=min(x.confidence, y.confidence),
                      persistence=x.persistence, modality=x.modality, merged_from=(a, b),
    ...
    def split(self, identity: str, *, reason: str) -> list[Item]:
        ...
        self.revisions.append({"kind": "split", "identity": identity, "into": list(item.merged_from),
                               "reason": reason})
        return restored
    ...
    def detect_mistaken_merge(self, identity: str) -> dict:
        ...
        clashing = [k for k in set(a.attributes) & set(b.attributes)
                    if a.attributes[k] != b.attributes[k] and a.overlaps(b)]
        if clashing:
            self.contradictions.append({"identity": identity, "attributes": clashing})
        return {"identity": identity, "mistaken": bool(clashing), "clashing_attributes": sorted(clashing),
                "reason": "one referent cannot hold two values for one attribute at one time"
                if clashing else ""}
    ...
    def detect_mistaken_split(self, a: str, b: str) -> dict:
        ...
        return {"pair": [a, b], "shared_attributes": sorted(shared), "disagreements": sorted(disagree),
                "mistaken_split": bool(shared) and not disagree and x.overlaps(y)
    ...
    def revise_type(self, identity: str, new_type: str, *, evidence: str) -> Item:
        ...
        self.revisions.append({"kind": "type", "identity": identity, "from": item.type,
                               "to": new_type, "evidence": evidence})
        item.type = new_type
        if new_type != "unknown":
            item.unknown_reason = ""
        return item
    ...
    def relation_consistency(self) -> dict:
        ...
        return {"dangling": dangling, "cyclic_composition": cyclic,
                "consistent": not dangling and not cyclic}
        ...
        for item in self.items.values():
            ...
            for kind, other in (tuple(r) for r in item.relations):
                ...
                if not item.overlaps(target):
                    problems.append({"part": item.identity, "whole": other,
    ...
    def self_environment_boundary(self) -> dict:
        ...
        return {"self_items": selves, "singleton": len(selves) <= 1,
...
def type_of(observation: dict, strategy: str, *, rules: dict | None = None,
            prototypes: dict | None = None, retrieved: dict | None = None,
            oracle: dict | None = None, headroom: dict | None = None) -> str:
...
def declaration() -> dict:
    ...
    return {
        ...
        "supported": ["identity over time", "object persistence", "event boundaries",
                      "compositional relations", "nested systems",
                      "multiple descriptions of one referent", "unknown types", "uncertainty",
                      "contradictions", "ontology revision"],
        "merge_rule": ("a merge needs stated evidence, is refused across any declared distinction, and "
...
def main(argv=None) -> None:
    ...
    print(json.dumps({"sealed": path.relative_to(io.ROOT).as_posix(),
                      "types": len(doc["types"]), "distinctions": len(doc["distinctions"])}, indent=2))
