...
FACT_KINDS = ("active_goals", "available_capabilities", "known_limitations", "current_state",
              "active_perspectives", "historical_reliability", "memory_integrity", "resource_limits",
              "unfinished_tasks", "recent_errors", "checkpoint_identity", "developmental_history")

# section 9, what the predictions are compared against
PREDICTION_KINDS = ("accuracy", "failure_probability", "time", "cost", "tool_competence",
                    "memory_confidence", "perspective_reliability", "task_progress")
...
def compare_against_fixed_prior(outcomes: dict[str, list[float]],
    ...
    gains = {k: round(b[k]["mean_absolute_error"] - a[k]["mean_absolute_error"], 6)
             for k in outcomes if a[k]["n"] and b[k]["n"]}
    return {"updating_model": a, "fixed_prior": b, "calibration_gain": gains,
...
def usefulness_report(measures: dict) -> dict:
    ...
    for use in USEFULNESS:
        m = measures.get(use)
        if m is None:
            rows[use] = {"measured": False, "improves": False,
                         "reason": "no comparison against the no self model arm was run"}
            continue
        rows[use] = {"measured": True, "with_model": m["with_model"],
    ...
    return {"uses": rows, "earned_uses": earned, "any_use_earned": bool(earned),
...
def declaration(model: SelfModel | None = None) -> dict:
    ...
    return {
        ...
        "not_a_narrative": ("a self fact carries a source artifact or code path or it is refused. No self "
