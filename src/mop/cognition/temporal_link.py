...
PROVIDES = ("current_temporal_state", "state_confidence", "transition_signal", "history_summary",
            "reset_authority", "checkpoint", "restore")

# section 9, the five sources that must stay distinguishable
SOURCES = ("current_observation", "explicit_history", "temporal_core_state", "memory_retrieval",
...
class TemporalCore:
    ...
    def current_temporal_state(self) -> Reading:
        ...
        value = sum(v * w for v, w in zip(self.state, weights)) / total
    ...
    def transition_signal(self) -> Reading:
        if len(self.state) < 2:
            return Reading(0.0, "temporal_core_state", 0.0)
        return Reading(round(abs(self.state[-1] - self.state[-2]), 6), "temporal_core_state",
    ...
    def reset_authority(self) -> dict:
        return {"who_may_reset": ["goal_authority", "episode_boundary"],
    ...
    def checkpoint(self) -> dict:
        return {"version": self.version, "is_control": self.is_control, "state": list(self.state),
                "history": list(self.history), "resets": self.resets, "horizon": self.horizon,
                "decay": self.decay, "identity": io.sha_obj(
                    {"s": self.state, "h": self.history, "r": self.resets})}
...
class DeclaredControl(TemporalCore):
    ...
    is_control = True
    limitation = ("no temporal core was scientifically licensed. The factorial is terminal and its "
...
def merged_view(core: TemporalCore, *, observation, retrieved=None, predicted=None) -> dict:
    ...
    readings = {
        ...
        "world_model_prediction": Reading(predicted, "world_model_prediction",
                                          0.0 if predicted is None else 0.6),
    }
    return {"readings": {k: {"value": r.value, "source": r.source, "confidence": r.confidence}
...
def declaration() -> dict:
    ...
    return {
        ...
        "licensing": {"authority": TEMPORAL_AUTHORITY,
        ...
        "checkpoint_roundtrip": {"identity": snapshot["identity"][:16],
...
def main(argv=None) -> None:
    ...
    print(json.dumps({"sealed": path.relative_to(io.ROOT).as_posix(),
                      "implementation": doc["implementation"], "is_control": doc["is_control"],
                      "sources": len(doc["information_sources"])}, indent=2))
