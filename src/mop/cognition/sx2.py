...
def diversity_statistics(scores: dict) -> dict:
    ...
    return {"n_cells": len(cells), "n_units": len(units),
            ...
            "complementary_pairs": complementary, "total_pairs": len(pairs),
...
def set_value(scores: dict, cells: list, units: list, rule: str) -> float:
    ...
    if rule == "reliability_weighted":
        ...
        return statistics.fmean(
            sum(scores[c][u] * weights[c] for c in cells) / total for u in units)
...
def matched_single(scores: dict, costs: dict, units: list, budget: float) -> dict:
    ...
    return {"cell": best, "value": round(_mean_over_units(scores, best, units), 6),
...
def run(bed: str = "harth_stream", seed: int = 0) -> dict:
    ...
    best_single = {"cell": ranked[0], "value": round(_mean_over_units(scores, ranked[0], units), 6),
    ...
    for k in [k for k in K_VALUES if k <= len(cells)] + ([max_k] if max_k not in K_VALUES else []):
        ...
        while len(diverse) < k:
            ...
            pick = min(rest, key=lambda c: statistics.fmean(
        ...
        rows[k] = {
            ...
            "reliability_weighted_top_k": round(
                set_value(scores, top, units, "reliability_weighted"), 6),
    ...
    return {
        ...
        "why_admissible": ("each cell is a different architecture, capacity, readout, horizon and reset "
                           "schedule, each already scored on the same held out units, and none was built "
                           "to be a perspective set"),
        "n_cells": len(cells), "n_units": len(units), "units": units,
        ...
        "verdict": verdict,
        "compute_matching": ("selecting among k cells costs k times one cell, so every comparison is "
        ...
        "reading": ("oracle selection is an upper bound that no selector can exceed. If the upper bound "
                    ...
                    "the upper bound clears the SESOI, so one bounded selector and arbitration canary is "
                    "licensed"),
        "activation": False,
    }
...
def main(argv=None) -> None:
    ...
    print(json.dumps({"sealed": path.relative_to(io.ROOT).as_posix(),
                      "n_cells": doc["n_cells"], "n_units": doc["n_units"],
