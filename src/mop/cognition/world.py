...
REPRESENTED = ("entities", "objects", "relations", "events", "causes", "affordances", "agents",
               "goals", "uncertainty", "time", "counterfactual_alternatives")

DISTINCTIONS = ("predictive_accuracy", "decision_usefulness", "causal_validity",
                "simulation_reliability")

TESTS = ("next_state", "event", "transition", "intervention", "counterfactual_consistency",
         "missing_observation", "return_to_context", "long_horizon_consistency",
         "decision_improvement")

TEST_GROUP = {
    "next_state": "predictive_accuracy", "event": "predictive_accuracy",
    "transition": "predictive_accuracy", "missing_observation": "predictive_accuracy",
    "intervention": "causal_validity", "counterfactual_consistency": "causal_validity",
...
def synthetic_world(seed: int = 0, n: int = 600) -> dict:
    ...
    parents = {"season": ("season",), "weather": ("season",), "road": ("weather",), "tyre": ("tyre",),
               "speed": ("road", "tyre"), "arrive": ("speed",)}
    ...
    def step(state: dict) -> dict:
        ...
        return {"season": season, "weather": weather, "road": road, "tyre": tyre, "speed": speed,
                "arrive": arrive}

    state = {"season": 0, "weather": "dry", "road": "grip", "tyre": "summer", "speed": "fast",
             "arrive": "ontime"}
    transitions = []
    for _ in range(n):
        nxt = step(state)
    ...
    return {"parents": parents, "train": transitions[:split], "test": transitions[split:],
            "actions": {"tyre": ("summer", "winter")}, "truth": step}
...
def evaluate(model: WorldModel, bed: dict) -> dict:
    ...
    scores["missing_observation"] = _rate(
        model.infer_missing({k: v for k, v in s.items() if k != "road"}, "road") == n["road"]
        for s, n in test)
    ...
    detour = {"season": 3, "weather": "wet", "road": "slick", "tyre": "winter", "speed": "fast",
              "arrive": "ontime"}
    restored = []
    for s, n in test[:50]:
    ...
    scores["long_horizon_consistency"] = _rate(
        tuple(sorted(st.items())) in model.seen_states for st in traj)
    ...
    return {
        ...
        "limited_instrument_reason": ("predicts well and does not improve any decision, which section 8 "
                                      "calls a limited instrument" if limited else ""),
...
def limited_instrument(distinctions: dict) -> bool:
    ...
    return (distinctions["predictive_accuracy"] >= 0.9
            and distinctions["decision_usefulness"] <= 0.0)
...
def declaration() -> dict:
    ...
    return {
        ...
        "operator_rule": ("intervening on a variable cuts it from its parents for that step, observing it "
                          "does not. A model that treats them identically passes the predictive tests and "
                          "fails the causal ones"),
        "limited_instrument_rule": ("high predictive accuracy with no decision improvement is reported as "
                                    "a limited instrument, never as a world model result"),
        "calibration_bed": {"kind": "synthetic world with a known generative truth",
                            "parents": {k: list(v) for k, v in bed["parents"].items()},
                            "train": len(bed["train"]), "test": len(bed["test"])},
        "battery": battery,
        "activation": False,
    }
...
def main(argv=None) -> None:
    ...
    print(json.dumps({"sealed": path.relative_to(io.ROOT).as_posix(),
                      ...
                      "limited_instrument": doc["battery"]["limited_instrument"]}, indent=2))
