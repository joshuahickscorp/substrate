...
def _orchestration_events() -> list[dict]:
    ...
    for path in sorted(root.glob("*.json")) if root.is_dir() else []:
        ...
        out.append(
            {
                "kind": kind,
                "stage": doc.get("stage") or doc.get("classification", ""),
        ...
        out.append(
            {
                ...
                "stage": "supervisor",
                "identity": f"t{len(out)}",
                "detail": {"workers": workers, "cap": cap, "resource_class": klass, "remaining": remaining},
...
def build() -> dict:
    ...
    return {
        ...
        "why_it_is_real": (
            "a supervisor ran for days under changing resource pressure and recorded every "
        ...
        "provenance": {
            "runs_root": TEMPORAL_RUNS.relative_to(io.ROOT).as_posix(),
        ...
        "usable_for": [
            ...
            "grounding referents and outcomes",
            "world model state and decision pairs",
            "developmental divergence histories",
        ],
        "activation": False,
    }
