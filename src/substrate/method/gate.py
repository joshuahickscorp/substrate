...
from substrate.method.contracts import Contract
...
class Preregistration:
    ...
    stop_switch: str = "~/.substrate/stop"
...
def classify_result(
    *,
    ...
    baseline_valid: bool,
    estimator_sufficient: bool,
    verifier_agrees: bool,
    mutations_rejected: bool,
    implementations_agreeing: int = 1,
    cost_adjusted_pass: bool = True,
) -> dict:
    ...
    if v == "harm":
        return {
            "classification": "harm",
            "reason": "effect below the harm boundary",
            "scientific": True,
            "effect": effect,
        }
    if v == "wrong_direction_failure":
        return {
            "classification": "failure_wrong_direction",
            "reason": "",
            "scientific": True,
            "effect": effect,
        }
...
def coverage_gate(
    ...
    *,
    scope: list[str],
    excluded: list[str],
    target_statement: float = 92.0,
    target_branch: float = 82.0,
) -> dict:
