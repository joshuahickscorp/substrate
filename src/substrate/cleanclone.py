...
def run(keep: bool = False) -> dict:
    ...
    try:
        ...
        results["graph_valid"] = {
            "ok": code == 0 and '"valid": true' in out.replace("'", '"').lower(),
        ...
        code, out = _run([PY, "-m", "substrate.deliverables", "seal-modules"], clone, env)
        once = {p.name: content(p) for p in sorted(proof.glob("SUBSTRATE_*.json"))}
        code2, _ = _run([PY, "-m", "substrate.deliverables", "seal-modules"], clone, env)
        twice = {p.name: content(p) for p in sorted(proof.glob("SUBSTRATE_*.json"))}

        drifted = sorted(k for k in before if k in once and before[k] != once[k] and k not in bootstrap)
        ...
        results["artifacts_regenerate_identically"] = {
            ...
            "why_bootstrap": (
                "both report on the tree as it stands after the commit, so neither can be "
            ...
            "note": (
                "content is compared with the commit stamp and its digest excluded, because a "
        ...
        code, out = _run([PY, "-m", "substrate.verification", "recompute"], clone, env)
        results["independent_recomputation"] = {
            "ok": code == 0 and "0 disagreements" in out,
            "detail": out.strip()[-200:],
        }
    ...
    return {
        "schema": "substrate-clean-clone/v1",
        "commit": commit,
        "checks": checks,
        "detail": results,
