...
POLICIES: tuple[Policy, ...] = (
    ...
    Policy("contradiction_first", frozenset({"contradiction", "confidence", "budget"}), _contradiction_first),
...
def headroom(situations: list[Situation], budget: float = 1e9) -> dict:
    ...
    return {
        "oracle": oracle,
        "per_simple_policy": simple,
        "best_simple": best,
        "residual": round(oracle - best, 6),
...
def _probe(n: int = 60) -> list[Situation]:
    ...
    for i in range(n):
        ...
        out.append(
            Situation(
                confidence=0.9 if not wrong else 0.4,
                novelty=(i % 7) / 7,
                contradiction=i % 5 == 0,
                correct_without_verification=not wrong,
                verification_cost=1.0,
                verification_fixes=i % 6 != 0,
            )
        )
    return out
        ...
        "policies": [
            {
                ...
                "learned": p.learned,
                "implemented": p.decide is not None,
            }
            for p in POLICIES
        ],
        ...
        json.dumps(
            {
                ...
                "learned_licensed": doc["learned_currently_licensed"],
            },
            indent=2,
        )
    )
