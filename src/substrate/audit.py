...
SRC = io.ROOT / "src" / "substrate"

CHECKS = (
    "exclusive_producers",
    "no_stale_outputs",
    "no_duplicate_stages",
    "causal_paths_present",
    "every_node_actionable",
    "runtime_stages_reachable",
    "no_activation_path",
)
...
def producers() -> dict:
    ...
    for f in sorted(SRC.glob("*.py")):
        ...
        for m in re.finditer(r"io\.seal(?:_md)?\(\s*\{([^}]*)\}", text, re.S):
...
def no_stale_outputs() -> dict:
    ...
    return {
        ...
        "note": (
            "an artifact reporting a failure is not stale. Evidence still refuses to count it, "
            "which is substrate.program.evidence_state, a different question from whether "
...
def no_duplicate_stages() -> dict:
    ...
    return {
        "nodes": len(ids),
        "duplicate_identities": [i for i in ids if ids.count(i) > 1],
...
def every_node_actionable() -> dict:
    ...
    return {
        "nodes": rows,
        "not_actionable": [r["node"] for r in rows if not r["actionable"]],
...
def runtime_stages_reachable() -> dict:
    ...
    return {
        "declared": list(R.STAGES),
        "written_in_the_loop": sorted(recorded),
...
def no_activation_path() -> dict:
    ...
    return {
        "source_hits": hits,
        "sealed_artifacts_with_activation_true": sealed,
...
def run() -> dict:
    ...
    return {
        "schema": "substrate-structural-audit/v1",
        "checks": list(CHECKS),
        "results": results,
        "failed": failed,
        "all_pass": not failed,
        ...
        json.dumps(
            {
                ...
                "failed": doc["failed"],
                "summary": {k: v["ok"] for k, v in doc["results"].items()},
            },
            indent=2,
        )
    )
