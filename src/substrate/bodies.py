...
ABLATIONS = (
    "body_alone",
    "body_plus_memory",
    "body_plus_temporal_core",
    "body_plus_arbitration",
    "body_plus_bounded_adaptation",
    "full_substrate",
)

EXTERNAL_BLOCKER = (
    "no frontier model weights and no inference credential are present on this machine, "
...
def _views(x):
    ...
    return {
        "static": x.mean(axis=1),
        "dynamic": np.abs(np.diff(x, axis=1)).mean(axis=1),
        "spread": x.std(axis=1),
    }
...
class Body:
    ...
    def selected_activations(self) -> dict:
        return {
            "selector": "per view distance",
            "shape": [len(self.views)],
    ...
    def adaptation_proposal(self) -> dict:
        return {
            "information_used": "held out units",
            "affected_state": "the readout centroids",
            "reversibility": "reversible",
            "cost": 0.1,
            "risk": "overfits one domain",
            "verification": "retention on prior units",
            "rollback": "restore previous centroids",
        }
    ...
    def checkpoint(self) -> dict:
        return {
            ...
            "sha256": io.sha_obj({v: m["centroids"].tolist() for v, m in self.models.items()}),
        }
...
def conformance(klass: str) -> dict:
    ...
    for kind in CONTRACT.MESSAGE_KINDS:
        handler = {
            "inference": lambda: b.inference(bed["Xte"][:4]),
            "hidden_state": b.hidden_state,
            "selected_activations": b.selected_activations,
            ...
            "resource_report": b.resource_report,
...
def compare() -> dict:
    ...
    return {
        ...
        "reading": (
            "no rung of the ladder beats the body alone on this bed. The substrate components "
...
def main(argv=None) -> None:
    ...
    if command in ("compact", "general", "tool"):
        ...
        path = io.seal(
            {
                "compact": "SUBSTRATE_BODY_COMPACT.json",
                "general": "SUBSTRATE_BODY_GENERAL.json",
                "tool": "SUBSTRATE_BODY_TOOL.json",
            }[command],
            doc,
        )
        print(
            json.dumps(
                {
                    ...
                    "accuracy": doc["held_out_accuracy"],
                },
                indent=2,
            )
        )
            ...
            json.dumps(
                {
                    ...
                    "any_gain": doc["any_substrate_gain"],
                },
                indent=2,
            )
        )
