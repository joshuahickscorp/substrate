...
COG = "src/substrate"
D, X, V = "substrate.deliverables", "substrate.experiments", "substrate.verification"
...
NODES: tuple[Node, ...] = (
    ...
    _n(
        "tests",
        "verification",
        module="substrate.program",
        args=("tests",),
        dependencies=("declarations",),
        exit_gate="every declared test node is recorded passing",
        resource_profile="light",
    ),
    _n(
        "authority",
        "report",
        module="substrate.authority",
        args=("seal",),
        dependencies=("tests",),
        ...
        requires_impl=(f"{COG}/authority.py",),
        resource_profile="trivial",
    ),
    # ---------------------------------------------------------------- category 2, the built blockers
    _n(
        "temporal_core_integration",
        "integration",
        module="substrate.temporal_link",
        args=("seal",),
        dependencies=("declarations",),
    ...
    _n(
        ...
        requires_impl=(f"{COG}/worldbed.py",),
    ...
    _n(
        ...
        requires_impl=(f"{COG}/sessions.py",),
        resource_profile="light",
    ),
    # ---------------------------------------------------------------- category 3 to 5
    _n(
        ...
        requires_impl=(f"{COG}/goals.py",),
        resource_profile="light",
    ),
    _n(
        "grounding",
        "implementation",
        module="substrate.grounding",
        args=("seal",),
        ...
        requires_impl=(f"{COG}/grounding.py",),
        resource_profile="light",
    ),
    _n(
        "perspective_expansion",
        "implementation",
        module="substrate.perspectives",
        args=("seal",),
        dependencies=("declarations",),
        ...
        produces=("SUBSTRATE_PERSPECTIVE_SYSTEM.json",),
        resource_profile="light",
    ),
    _n(
        ...
        requires_impl=(f"{COG}/divergence.py",),
        resource_profile="moderate",
    ),
    _n(
        "entity_batteries",
        "experiment",
        module="substrate.batteries",
        args=("seal",),
    ...
    _n(
        ...
        module=D,
        args=("write",),
        dependencies=("sx1b", "sx5", "authority"),
        exit_gate="every master artifact is regenerated from the tree",
        produces=("SUBSTRATE_STATE.json", "SUBSTRATE_NEXT_FRONTIER.json"),
        resource_profile="trivial",
    ),
    _n(
        "recompute",
    ...
    _n(
        ...
        requires_impl=(f"{COG}/cleanclone.py",),
        resource_profile="moderate",
    ),
    _n(
        "consolidate",
        ...
        produces=("SUBSTRATE_CURRENT_ENTITY_SPEC.json",),
        resource_profile="trivial",
    ),
)
...
def entry_open(node: Node) -> dict:
    ...
    return {
        "open": not missing_impl and not node.external_blocker,
...
def exit_passed(node: Node) -> dict:
    ...
    return {
        "passed": (not missing and condition_met and not entry_open(node)["missing_implementation"]),
    ...
    for n in NODES:
        ...
        if gate["missing_implementation"] and not n.external_blocker:
            rows.append(
                {
                    "node": n.identity,
                    "missing": gate["missing_implementation"],
...
def declaration() -> dict:
        ...
        rows.append(
            {
                ...
                "entry_gate": n.entry_gate,
                "exit_gate": n.exit_gate,
                ...
                "successors": successors(n.identity),
                "entry_open": gate["open"],
                "exit_passed": exit_["passed"],
    ...
    return {
        ...
        "rule": (
            "a blocker that can be built with available data, code and compute is a node in this "
        ...
        json.dumps(
            {
                ...
                "valid": doc["valid"],
                "terminal": len(doc["terminal_nodes"]),
                "buildable_prerequisites": [b["node"] for b in doc["buildable_prerequisites"]],
                "externally_blocked": doc["externally_blocked"],
            },
            indent=2,
        )
    )
