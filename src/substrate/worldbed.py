...
def integrate(bed: dict | None = None) -> dict:
    ...
    return {
        ...
        "bed": {
            "admissible": bed["admissible"],
            "n_transitions": bed["n_transitions"],
            ...
            json.dumps(
                {
                    "sealed": path.relative_to(io.ROOT).as_posix(),
                    "verdict": doc["verdict"],
                    "decision_gain": doc["decision_gain"],
                    "decisions_changed": doc["decisions_changed_by_the_model"],
                },
                indent=2,
            )
        )
