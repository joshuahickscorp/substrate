...
CLAIM_BOUNDARY = {
    ...
    # substrate.safety, which is wider because it also carries the adjective forms. Two lists for one
    # concept is how a boundary drifts, so this one names the other rather than restating it.
    "forbidden_claims_as_written_in_the_plan": [
        "consciousness",
        "sentience",
        "feelings",
        "wants",
    ...
    "enforced_vocabulary": "substrate.safety.FORBIDDEN_CLAIM_TERMS",
...
HYPOTHESES = (
    dict(
        ...
        dependent_hypotheses=["H_arbitration_minority"],
        blocking_null=None,
    ),
    dict(
        ...
        dependent_hypotheses=["H_learned_selector"],
        blocking_null=None,
    ),
    dict(
        id="H_learned_selector",
        items=["C4"],
        premise="a learned perspective selector beats the strongest simple selection rule",
        predecessor="H_perspective_diversity",
        state="unopened",
    ...
    dict(
        ...
        dependent_hypotheses=["H_selfmodel_calibration"],
        blocking_null=None,
    ),
    dict(
        id="H_selfmodel_calibration",
        items=["S1", "E3"],
        premise="a measured self model improves decisions over a fixed prior of the same form",
        predecessor="H_owned_continuity",
        state="unopened",
    ...
    dict(
        id="H_learned_plasticity_policy",
        items=["P4"],
...
def null_map(st: dict) -> dict:
    inherited = [{"program": prog, **_bind(ref), "role": why} for prog, ref, why in INHERITED_AUTHORITIES]
    ...
    return {
        "measurement_boundaries": (
            ...
            if screen and not screen.get("any_candidate")
            else []
        ),
        "boundary_rule": (
            "a hypothesis nothing under custody can measure at the declared effect size is "
        ...
        "binding_rule": (
            "a null is immutable. It may be superseded only by an appended authority that "
...
def master_authority(st: dict) -> dict:
    ...
    return {
        ...
        "single_infrastructure": {
            "scheduler": "substrate.execution",
            "experiment_engine": "substrate.experiments",
...
COMPONENTS = (
    (
        "temporal_control",
        "src/substrate/temporal_link.py",
        "the declared control inherited from the terminal temporal factorial",
    ),
    ("typed_workspace", "src/substrate/workspace.py", "broad reads, narrow writes, ten typed regions"),
    ("perspective_system", "src/substrate/perspectives.py", "processes, a selection ladder, arbitration"),
    (
        "memory_hierarchy",
        "src/substrate/memory.py",
        "working, episodic, semantic, procedural, consolidation, hygiene",
    ),
    ("world_model", "src/substrate/world.py", "causal graph plus tabular dynamics, four distinctions"),
    ("self_model", "src/substrate/selfmodel.py", "measured facts, predictions paired to outcomes"),
    ("metacognition", "src/substrate/metacog.py", "eleven governed actions, eight measures, attention"),
    ("plasticity", "src/substrate/plasticity.py", "ten levels, fast and slow bars, bounded reorganization"),
    ("model_body_interface", "src/substrate/body.py", "nine message kinds, six integration modes"),
    ("batteries", "src/substrate/batteries.py", "thinking, continuity, unity, reflective access"),
    (
        "verification_fabric",
        "src/substrate/verification.py",
        "recomputation from sealed bytes, mutation attacks",
    ),
    ("admission_gate", "src/substrate/method", "the compact experiment validity kernel"),
    ("scheduler", "src/substrate/execution.py", "the single DAG executor and supervisor"),
)
...
def architecture(st: dict) -> dict:
    ...
    return {
        "schema": "substrate-architecture/v1",
        "shape": [
            "model body or bodies",
            "owned temporal core",
            "typed workspace",
            "memory hierarchy",
            "world model",
...
def _temporal_progress(counts: dict) -> dict:
    ...
    return {
        ...
        "assumption": (
            "each running shard now gets close to a full core, which holds while the worker "
...
def temporal_core_record() -> dict:
    ...
    return {
        ...
        "honest_state": (
            "Substrate Temporal Core v1 is not named yet. The factorial is still executing "
            ...
            if not terminal
            else "the selection is terminal and independently verified"
        ),
...
def entity_spec(st: dict, arch: dict, caps: dict) -> dict:
    ...
    return {
        ...
        "not_claimed_as_written_in_the_plan": list(CLAIM_BOUNDARY["forbidden_claims_as_written_in_the_plan"]),
        "activation": False,
    }
...
def entity_report(st: dict, spec: dict, arch: dict, caps: dict, temporal: dict) -> str:
    ...
    lines += [
        ...
        "zero in every category that has not produced a scientific classification, and raising it would",
        "require an admitted experiment on a valid bed, not more code.",
        "",
