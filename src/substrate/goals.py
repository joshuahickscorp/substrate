...
AUTHORIZED_WEIGHTS = {
    ...
    "time": -0.2,
    "resource_use": -0.3,
    "harm_constraints": -5.0,
    "goal_priority": 0.6,
    "uncertainty": -0.1,
    "reversibility": 0.5,
}
WEIGHT_AUTHORITY = "operator declared in substrate.goals.AUTHORIZED_WEIGHTS, auditable in place"
...
class GoalSystem:
    ...
    def authorize(self, goal: Goal, *, external_authority: str) -> Goal:
        ...
        if not external_authority or external_authority.startswith("substrate.goals"):
    ...
    def observe_progress(self, goal_id: str, value: float) -> dict:
        ...
        if g.progress <= before and g.state == "active":
            ...
            g.state = "active"
        return {"goal": goal_id, "progress": g.progress, "state": g.state, "blocked": g.state == "blocked"}
...
def declaration() -> dict:
    ...
    root = gs.authorize(
        Goal(
            ...
            rollback="revert the campaign commits",
            authority="pending",
        ),
    ...
    child = gs.decompose(
        ...
        Goal(
            ...
            constraints=("activation stays false",),
            resources="local compute",
            progress_measure="tests passing",
            termination="ontology terminal",
            rollback="revert",
            authority="pending",
        ),
    )
